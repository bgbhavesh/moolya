...
export default class MlUsersFavourites extends Component {
  ...
  async getAllConnections() {
    ...
    const response = await fetchFavouritesHandler(registrationId, communityCode)
    console.log(response)
    if (response && response.length > 0) { this.setState({ data: response }) } else { toastr.error('No Favourites Available') }
  }
