...
export default MlAccounts = class MlAccounts {
  ...
  static sendVerificationEmail(regId, emailOptions, address, customEmailComponent) {
    ...
    const user = mlDBController.findOne('MlRegistration', { _id: regId }, emailOptions.context || {});
  ...
  static verifyEmail(token) {
    ...
    let user = mlDBController.findOne('MlRegistration', { 'services.email.verificationTokens.token': token }, {})
    if (!user) {
      return {
        email: null, error: true, reason: 'EMAIL ALREADY VERIFIED', code: 403
      };
    }
    ...
    if (!user) {
      return {
        email: null, error: true, reason: 'REGISTRATION INACTIVATED', code: 403
      };
    }// throw new Error(403, "Verify email link expired");

    const tokenRecord = _.find(user.services.email.verificationTokens, t => t.token == token);
    ...
    const emailsRecord = _.find(user.emails, e => e.address == tokenRecord.address);

    if (!emailsRecord) {
      return {
        userId: user._id, error: true, reason: 'Verify email link is for unknown address', code: 403
      };
    }
    ...
    const verficationTokensArray = user && user.services && user.services.email && user.services.email.verificationTokens ? user.services.email.verificationTokens : []
    const verficationTokenObject = _.findWhere(verficationTokensArray, { token });
    const linkGenerateDate = verficationTokenObject && verficationTokenObject.when ? verficationTokenObject.when : null
    const hours = moment().diff(linkGenerateDate, 'hours')
    const registrationCommunityName = user && user.registrationInfo && user.registrationInfo.communityDefCode &&
    ...
    // }
    if (hours > 72) {
      MlRegistration.update({ _id: user._id, 'emails.address': tokenRecord.address }, {
      ...
      return {
        email: null, error: true, reason: 'LINK EXPIRED', code: 403
      };
    }

    const emailVerified = MlRegistration.update({ _id: user._id, 'emails.address': tokenRecord.address }, {
      $set: { 'emails.$.verified': true },
      $pull: { 'services.email.verificationTokens': { address: tokenRecord.address } }
    });

    if (emailVerified) {
      const emailSent = MlEmailNotification.onEmailVerificationSuccess(user);
  ...
  static sendVerificationSmsOtp(regId, numbr, customEmailComponent) {
    ...
    if (otps) {
      ...
      otps = [{
        num: otpNumber, time: new Date(), verified: false, mobileNumber, countryId: countryCode
      }];
      otp = otpNumber;
    }

    const otpNum = otp || `${Math.floor(1000 + Math.random() * 9000)}`;
    if (otp) {
      // DO WE NEED TO UPDATE THE TIME?
    } else {
      otps.push({
        num: otpNum, time: new Date(), verified: false, mobileNumber, countryId: countryCode
      });
    }

    MlRegistration.update({ _id: regId }, { $set: { otps } });
  ...
  static sendUserVerificationSmsOtp(userId, customEmailComponent) {
    ...
    if (user && user.profile) {
      ...
      if (!mobileNumber) {
        return {
          mobileNumber: '', error: true, reason: 'Mobile Number is not available', code: 403
        };
      }
    }
    ...
    const countryCode = externalProfile.countryId;
    let msg = null;
    // if(!mobileNumber){
    //   mobileNumber=(regDetails.registrationInfo||{}).contactNumber;
    // }
    ...
    Meteor.users.update({ _id: userId }, { $set: { otps } });
  ...
  static resendVerificationSmsOtp(numbr, customEmailComponent) {
    const regDetails = mlDBController.findOne('MlRegistration', { 'registrationInfo.contactNumber': numbr, status: { $nin: ['REG_ADM_REJ', 'REG_USER_REJ'] } });

    if (!regDetails) {
      throw new Error(403, 'Mobile Number entered is not registered');
    }

    const to = (regDetails.registrationInfo || {}).contactNumber;
    const countryCode = (regDetails.registrationInfo || {}).countryId;
    ...
    MlRegistration.update({ 'registrationInfo.contactNumber': numbr, status: { $nin: ['REG_ADM_REJ', 'REG_USER_REJ'] } }, { $set: { otps } });
    // send SMS
    if (typeof customEmailComponent === 'function') {
      msg = customEmailComponent(regDetails, otpNum);
    } else {
  ...
  static verifyUserMobileNumberOtp(userId, otp) {
    ...
    if (user.otps && user.otps.length > 0) {
      ...
      for (let i = 0; i < user.otps.length; i++) {
        const otpRec = user.otps[i];
        ...
        const otpNum = otpRec.num;
        if (otpNum && Number(otpNum) === otp && otpRec.verified === false) {
          const otpTime = 15;// todo: configure it as the settings
          ...
          const mobileArr = user.mobileNumbers;
          if (mobileArr) {
            const mobileObj = _.find(mobileArr, { mobileNumber })
            if (mobileObj) {
              mobileObj.verified = true
              _lodash.remove(mobileArr, { mobileNumber })
              mobileArr.push(mobileObj);
            }
            user.mobileNumbers = mobileArr;
          } else {
            const mobileNumbers = [{
              num: otpNum, time: new Date(), verified: otpRec.verified, mobileNumber, countryId: otpRec.countryId
            }];
            user.mobileNumbers = mobileNumbers;
          }

          const updatedCount = Meteor.users.update({ _id: userId }, { $set: { otps: user.otps, mobileNumbers: user.mobileNumbers } });
          otpFound = true;
          break;
        }
      }
      ...
      if (otpVerified) {
        return {
          mobileNumber, error: true, reason: 'Mobile Number entered has already been verified', code: 403
        };
      } else if (otpExpired) {
        return {
          mobileNumber, error: true, reason: `OTP ${otp} has expired, generate new one with resend option`, code: 403
        };
      } else if (!otpFound) {
        return {
          mobileNumber, error: true, reason: 'OTP is Invalid, enter a correct one or try resend option', code: 403
        };
      }
      return {
        mobileNumber, recordId: user._id, error: false, reason: 'Mobile Number has been verified.', code: 200
      };
    ...
    return {
      mobileNumber, error: true, reason: 'Invalid otp,Please provide a valid otp to verify your number', code: 403
    };
  }

  static verifyMobileNumberOtp(mobileNumber, otp) {
    /** added a check to verify the mobile Number if registration is not rejected. */
    const regDetails = mlDBController.findOne('MlRegistration', { 'registrationInfo.contactNumber': mobileNumber, status: { $nin: ['REG_ADM_REJ', 'REG_USER_REJ'] } });

    if (!regDetails) {
      // throw new Error(403, "Mobile Number entered is not registered");
      return {
        mobileNumber, error: true, reason: 'Mobile Number entered is not registered', code: 403
      };
    }
    ...
    if (regDetails.otps && regDetails.otps.length > 0) {
      ...
      let otpVerified = false;
      let otpExpired = false;
      for (let i = 0; i < regDetails.otps.length; i++) {
        const otpRec = regDetails.otps[i];
        ...
        if (otpNum && Number(otpNum) === otp && otpRec.verified === false) {
          ...
          otpRec.verified = true;
          const updatedCount = MlRegistration.update({ _id: regDetails._id }, { $set: { otps: regDetails.otps } });
          otpFound = true;
          break;
        }
      }
      ...
      return {
        mobileNumber, recordId: regDetails._id, error: false, reason: 'Mobile Number has been verified.', code: 200
      };
      // These commented lines are to generate token for user to login when the user verifies his mobile/email
      // var stampedLoginToken = Accounts._generateStampedLoginToken();
      // Accounts._insertLoginToken(user._id, stampedLoginToken);
  ...
  static sendForgotPasswordEamil(email, context) {
    const user = mlDBController.findOne('users', { $or: [{ username: email }, { 'emails.address': email }] });
    if (!user) {
      return {
        email, error: true, reason: 'Invalid Email, User not register with this email', code: 404
      };
    }
    ...
    const res = mlDBController.update('users', user._id, { 'services.password.reset.token': token }, { $set: true }, context);
    /* let regObj = {
    ...
    if (res) {
      const emailSent = MlEmailNotification.forgotPassword(context, `${Meteor.absoluteUrl('reset')}/${token}`);
  ...
  static resetPasswordWithToken(token, password, context) {
    ...
    if (!user) {
      return {
        token, error: true, reason: 'Reset link Expired/Used', code: 404
      };
    }
    context.userId = user._id;
    context.browser = 'Reset Password API';
    context.url = Meteor.absoluteUrl('') || 'moolya';
    const salted = passwordUtil.hashPassword(password);
    const res = mlDBController.update('users', user._id, { 'services.password.bcrypt': salted, 'services.password.reset': {} }, { $set: true }, context);
    if (res) {
      if (user._id) {
  ...
  static resetSwitchProfile(userId, context) {
    try {
      ...
      let result = null;
      const hasSwitchedProfile = userProfile.hasSwitchedProfile;
      const switchedProfileDefaultId = userProfile.switchedProfileDefaultId;
      ...
      if (userProfile.isInternaluser) {
        ...
        result = mlDBController.update(
          ...
          },
          {
            ...
            'profile.hasSwitchedProfile': false,
        ...
        if (result === 1) {
          result = mlDBController.update(
            'users', { _id: userId, 'profile.InternalUprofile.moolyaProfile.userProfiles': { $elemMatch: { clusterId: { $ne: switchedProfileDefaultId } } } },
        ...
        if (result === 1) {
          result = mlDBController.update(
            'users', { _id: userId, 'profile.externalUserProfiles': { $elemMatch: { profileId: { $ne: switchedProfileDefaultId } } } },
...
Accounts.validateLoginAttempt(function (details) {
  if (details && details.user && details.user.profile.InternalUprofile && details.user.profile.InternalUprofile.moolyaProfile && details.user.profile.InternalUprofile.moolyaProfile.assignedDepartment && details.user.profile.InternalUprofile.moolyaProfile.assignedDepartment.length == 1) {
    const departmentId = details.user.profile.InternalUprofile.moolyaProfile.assignedDepartment[0].department;
    if (departmentId != 'all') {
      mlDBController = new MlDBController();
      const department = mlDBController.findOne('MlDepartments', { _id: departmentId }, this);
...
Accounts.onLogin((details) => {
  if (details.type == 'password') {
    ...
    const transactionDetails = `User logged in to application at ${new Date()} `;
...
Accounts.onLogout((details) => {
  const userId = (details.user || {})._id;
  const context = { ip: details.connection.clientAddress, browser: details.connection.httpHeaders['user-agent'], url: details.connection.httpHeaders.host };
  const transactionDetails = `User logged out of application at ${new Date()} `;
  if (details && details.user) {
    new MlTransactionsHandler().recordTransaction({
      activity: 'logout', transactionType: 'system', transactionTypeId: 'system', userId, transactionDetails, context
    });
  }
});
