...
export const createApolloServer = (customOptions = {}, customConfig = {}) => {
  ...
  graphQLServer.use(config.path, bodyParser.json(), graphqlExpress(async (req, res) => {
    try {
      ...
      const ret = mlserviceCardHandler.validateResource(req.body.query, context, req.body.variables);
      if (!ret.success) {
        const response = new MlRespPayload().errorPayload(ret.msg, 400);
  ...
  if (config.paymentReturnUrlPath) {
    ...
    graphQLServer.post(config.paymentReturnUrlPath, bodyParser.json(), Meteor.bindEnvironment((req, res) => {
      if (req) {
        let body = '';
        req.on('data', (data) => {
          body += data;
        ...
        req.on('end', async () => {
          try {
            ...
            const data_string = post.TxId + post.TxStatus + post.amount
                  + post.pgTxnNo + post.issuerRefNo + post.authIdCode
                  + post.firstName + post.lastName + post.pgRespCode + post.addressZip;
            const signature = crypto.createHmac('sha1', '22b7cc7ea856cd35e42c2ed6a76eff8e2f27a470').update(data_string).digest('hex');
            if (signature == post.signature) {
              const payObj = {
                transID: post.TxId,
                paymentStatus: post.TxStatus
              };
              const response = MlResolver.MlMutationResolver.updatePayment(null, payObj, context, null);
              if (response) {
              ...
              const code = 403;
              const error = { error: 'Transaction Failed', message: 'Signature Verification Failed' };
  ...
  if (config.conversationPath) {
    ...
    graphQLServer.post(config.conversationPath, bodyParser.json(), Meteor.bindEnvironment((req, res) => {
      const context = getContext({ req });
      mlConversationsRepo.login(context, (ret) => {
        res.send(ret)
      });
    }))
  }
