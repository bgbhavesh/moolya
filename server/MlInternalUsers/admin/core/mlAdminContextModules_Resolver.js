...
MlResolver.MlQueryResolver.ContextSpecSearch = (obj, args, context, info) => {
  ...
  // Authorization layer
  const transactionModules = ['registrationInfo', 'registrationApprovedInfo', 'registrationRejectedInfo', 'internalRequests', 'share', 'userTransaction', 'ConversationsLog', 'portfolioApproved', 'portfolioRequests', 'internalRejectedRequests', 'internalApprovedRequests', 'officeTransaction']
  const isTransactModule = includes(transactionModules, context.module)
  // Context Specific Search layer
  let contextQuery = {};
  contextQuery = new MlAdminContextQueryConstructor(context.userId, { module: args.module, action: args.action, isTransactModule }).contextQuery();
  ...
  switch (moduleName) {
    ...
    case 'MASTERSETTINGS':
      ...
      break;
    case 'AUDIT_LOG':
      const auditParams = args.context;
      const userSpecificSearch = !!args.fieldsData;
