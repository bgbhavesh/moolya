...
MlResolver.MlMutationResolver.updateHierarchyAssignment = (obj, args, context, info) => {
  ...
  if (hierarchy.id) {
    const hierarchyAssignment = mlDBController.findOne('MlHierarchyAssignments', { _id: hierarchy.id }, context)
    const id = hierarchy.id
    const hierarchyRoles = hierarchy.teamStructureAssignment; // client
    if (hierarchyRoles.length >= 1) {
      ...
      if (!_.isEmpty(oldAssignments)) {
        oldAssignments.map((role, oldkey) => {
          hierarchyRoles.map((newRole, key) => {
            if ((newRole.roleId == role.roleId) && newRole.isAssigned) {
              ...
              oldAssignments.splice(oldkey, 1)
            } else if (newRole.isAssigned === true) {
  ...
  const hierarchyRoles = hierarchy.teamStructureAssignment;
  const roles = []
  hierarchyRoles.map((role, key) => {
    if (role.isAssigned === true) {
      roles.push(role)
    }
  })
  hierarchy.teamStructureAssignment = roles;
  const id = mlDBController.insert('MlHierarchyAssignments', hierarchy, context)
  if (id) {
    const code = 200;
    const result = { appovalId: id }
    const response = new MlRespPayload().successPayload('Hierarchy added successfully', code);
    return response
  }
};
...
MlResolver.MlQueryResolver.fetchHierarchyRoles = (obj, args, context, info) => {
  ...
  if (department && department.isActive) {
    ...
    if (subChapter) {
      ...
      const userRole = mlAssignHierarchy.getUserRoles(context.userId);
      let isDefaultSubChapter = true;
  ...
  if (response) {
    ...
    if (isParentRole === false) {
      ...
      /*
         ...
         */
      const userHierarchy = mlDBController.findOne('MlHierarchy', { code: (currentRole.assignedLevel).toUpperCase() }, context)

      let reportingRole = currentRole.reportingRole;
      const sameLevelRoles = []
      /*    Looping through all the team assignment in the hierarchy    */
      teamStructureAssignment.map((role, key) => {
        const hierarchy = mlDBController.findOne('MlHierarchy', { code: (role.assignedLevel).toUpperCase() }, context)
      ...
      let finalArr = sameLevelRoles;

      for (let i = 0; i < sameLevelRoles.length; i++) {
        const role = _.find(sameLevelRoles, { roleId: reportingRole });
        if (role) {
          reportingRole = role.reportingRole;
...
MlResolver.MlQueryResolver.fetchHierarchyUsers = (obj, args, context, info) => {
  ...
  if (hierarchy) {
    ...
    if (args.clusterId && args.departmentId && args.subDepartmentId && args.roleId && args.subChapterId) {
      ...
      usersList = mlDBController.find('users', {
        $and:
        [
