 ...
 */
const actions = ['create', 'read', 'update', 'delete', 'like', 'connect',
...
const processActions = ['like', 'compare', 'share', 'wishlist', 'comment', 'onboard', 'shortlist', 'review', 'assess', 'discuss', 'promote', 'negotiate',
...
for (i = 0; i < actions.length; i++) {
  ...
  if (!action) {
    ...
    const code = actions[i].toUpperCase();
    action = {
      name: actions[i], displayName: actions[i], code, isActive: true, isAdmin: true
    };
    // check if its process setup action
    const isProcessAction = !(_.lastIndexOf(processActions, actions[i]) < 0);
