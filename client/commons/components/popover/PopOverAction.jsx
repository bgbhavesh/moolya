...
export default class PopOverAction extends React.Component {
  ...
  clickHandler(data) {
    console.log(data);
    this.setState({ data, popoverOpen: !this.state.popoverOpen });
  }


  render() {
    const PopOverComponent = React.cloneElement(this.props.popOverComponent, {
      config: this.props, data: this.state.data, closePopOver: this.toggle, toggle: this.toggle
    });
