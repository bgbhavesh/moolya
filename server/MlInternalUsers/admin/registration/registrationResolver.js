...
MlResolver.MlMutationResolver.createRegistration = (obj, args, context, info) => {
  ...
  if (!args.registration.registrationType) {
    const code = 409;
    const response = new MlRespPayload().errorPayload("'Registration Type' is mandatory", code);
    return response;
  } else if (!args.registration.userName) {
    const code = 409;
    const response = new MlRespPayload().errorPayload('Username is mandatory!', code);
    return response;
  }
  ...
  const communityDetails = mlDBController.findOne('MlCommunity', {
    subChapterId: (subChapterDetails._id || null),
    communityDefCode: args.registration.registrationType
  }, context) || {};
  const communityDef = mlDBController.findOne('MlCommunityDefinition', { code: (args.registration.registrationType || null) }, context) || {};
  ...
  const emails = [{ address: args.registration.email, verified: false }];
  ...
  args.registration.createdBy = createdBy || user.username;
  const id = mlDBController.insert('MlRegistration', {
  ...
  if (id) {
    ...
    const updatedResponse = mlDBController.update('MlRegistration', id, updateRecord, { $set: true }, context)
    ...
    // send email and otp;
    const code = 200;
    const result = { registrationId: id }
...
MlResolver.MlMutationResolver.registerAs = (obj, args, context, info) => {
  ...
  let validationCheck = MlRegistrationPreCondition.validateRegisterAsActiveCommunity(args.registration);
  if (validationCheck && !validationCheck.isValid) {
    return validationCheck.validationResponse;
  }
  const userInfo = mlDBController.findOne('MlRegistration', args.registrationId, context) || {};
  const userRegisterInfo = userInfo.registrationInfo;
  let registrationInfo = args.registration;

  const clusterInfo = MlClusters.findOne({ _id: registrationInfo.clusterId });
  const communityDef = mlDBController.findOne('MlCommunityDefinition', { code: (args.registration.registrationType || null) }, context) || {};
  ...
  const regDetails = _lodash.pick(userRegisterInfo, ['countryId', 'cityId', 'password', 'accountType', 'institutionAssociation', 'companyname', 'companyUrl', 'remarks', 'referralType']);
  ...
  if (registrationInfo.subChapterId) {
    const mlSubChapterAccessControl = MlSubChapterAccessControl.getAccessControl('TRANSACT', context, registrationInfo.subChapterId);
    ...
    const subChapterDetails = mlDBController.findOne('MlSubChapters', { _id: registrationInfo.subChapterId }) || {}
  ...
  orderNumberGenService.assignRegistrationId(args.registration)
  const emails = [{ address: args.registration.email, verified: true }];
  // create transaction
  ...
  if (id) {
    ...
    const updatedResponse = mlDBController.update('MlRegistration', id, updateRecord, { $set: true }, context)
    const communityName = communityDef && communityDef.name ? communityDef.name : ''
    if (updatedResponse) {
...
MlResolver.MlMutationResolver.createRegistrationAPI = (obj, args, context, info) => {
  ...
  const requestedSubChapterId = args.registration && args.registration.subChapterId ? args.registration.subChapterId.trim() : null;
  args.registration = _lodash.omit(args.registration, 'subChapterId');
  const countryId = args.registration && args.registration.countryId ? args.registration.countryId : null;
  const cityId = args.registration && args.registration.cityId ? args.registration.cityId : '';
  const communityType = args.registration && args.registration.registrationType ? args.registration.registrationType : null;
  ...
  const sysAdmin = mlDBController.findOne('users', { 'profile.email': 'systemadmin@moolya.global' }, context) || {};
  const sysAdminProfile = sysAdmin && sysAdmin.profile && sysAdmin.profile.InternalUprofile && sysAdmin.profile.InternalUprofile.moolyaProfile ? sysAdmin.profile.InternalUprofile.moolyaProfile : {};
  ...
  /** fix:MOOLYA-3391 */
  const registrationExist = MlRegistration.findOne({
  ...
  const userExist = mlDBController.findOne('users', { 'profile.email': args.registration.email }, context) || {};
  if (registrationExist || userExist._id) {
    ...
    const isActiveOFB = MlRegistrationPreCondition.checkActiveOfficeBearer(args)
    if (isActiveOFB) { result = { message: 'Sorry, your request requires Office admin attention. Please contact your Office Admin.' } }
    const errResp = new MlRespPayload().errorPayload(result, code);
    return errResp;
  } else if (args.registration) {
    // Community type and name
    const communityDef = mlDBController.findOne('MlCommunityDefinition', { code: (communityType || null) }, context) || {};
     ...
     * */
    let subChapterData = null; let clusterData = null; let chapterData = null; let chapterId = null;
    if (requestedSubChapterId) { /** Registration request for specific sub chapter */
      ...
      clusterData = mlDBController.findOne('MlClusters', { countryId, isActive: true }, context) || {}
      chapterData = mlDBController.findOne('MlChapters', { cityId, isActive: true }, context) || {}
      chapterId = chapterData && chapterData._id ? chapterData._id : ''
      subChapterData = mlDBController.findOne('MlSubChapters', { chapterId, isActive: true, isDefaultSubChapter: true }, context) || {}
    }
    ...
    const transactionId = args.registration.registrationId;
    ...
    /* Assign UserAgent to registration object - Fix for Issue: MOOLYA-769 */
    const {
      source, deviceName, deviceNumber, ipAddress, ipLocation
    } = setUserAgentForRegistration({}, userAgent(context.browser), context.ip);
    ...
    const updateCount = mlDBController.update('MlRegistration', { 'registrationInfo.email': args.registration.email, status: { $nin: ['REG_ADM_REJ', 'REG_USER_REJ'] } }, registrationRecord, {
      $set: true, upsert: true, setOnInsert: true, setOnInsertObject: { transactionId }
    }, context);

    if (updateCount == 1) {
      // mlRegistrationRepo.updateStatus(updateRecord,'REG_EMAIL_P');
      //  let updatedResponse = mlDBController.update('MlRegistration',response,updateRecord, {$set: true}, context)
      const regRec = mlDBController.findOne('MlRegistration', { transactionId }, context);
      if (regRec) {
        const updatedResponse = mlDBController.update('MlRegistration', regRec._id, { 'registrationInfo.registrationId': transactionId }, { $set: true }, context);
        ...
        const code = 200;
        const result = { message: 'You have successfully registered on moolya. Please check your email inbox for our email with the verification link', registrationId: args.registration.registrationId }
        const succResp = new MlRespPayload().successPayload(result, code);
        return succResp;
      }
...
MlResolver.MlQueryResolver.findRegistrationInfo = (obj, args, context, info) => {
  let response = null
  if (args.registrationId) {
    const id = args.registrationId;
    response = MlRegistration.findOne({ _id: id }) || {}
    const countryId = response.registrationInfo && response.registrationInfo.countryId ? response.registrationInfo.countryId : ''
    const country = countryId ? mlDBController.findOne('MlCountries', { _id: countryId }, context) : {};
    if (response && response.registrationInfo) { response.registrationInfo.countryCode = country ? country.countryCode : '' }
    return response
...
MlResolver.MlQueryResolver.findRegistrationInfoForUser = (obj, args, context, info) => {
  ...
  if (userId) {
    ...
    if (profile) {
      const registerId = profile.registrationId
      const username = mlDBController.findOne('users', { _id: userId }, context).username
      if (registerId) {
        const response = MlRegistration.findOne({ _id: registerId }) || {}

        const isAllowRegisterAs = mlDBController.findOne('MlRegistration', { // MOOLYA_2792
        ...
        if (_lodash.isEmpty(isAllowRegisterAs)) { response.isAllowRegisterAs = true } else {
        ...
        const communityCode = response && response.registrationInfo && response.registrationInfo.registrationType ? response.registrationInfo.registrationType : ''
...
MlResolver.MlMutationResolver.updateRegistrationInfo = (obj, args, context, info) => {
  if (args.registrationId) {
    ...
    let subChapterDetails = null;
    const id = args.registrationId;

    /** Get the registration Details */
    const registerDetails = mlDBController.findOne('MlRegistration', id, context) || {};
    ...
    if (args && args.registrationDetails && !args.registrationDetails.identityType) {
      const code = 401;
      const response = new MlRespPayload().errorPayload('Identity Type is required', code);
      return response;
    }
    ...
    if (args.registrationDetails) {
      ...
      if ((registrationInfo.countryId != details.countryId) || (registrationInfo.clusterId != details.clusterId) || (registrationInfo.chapterId != details.chapterId) || (registrationInfo.subChapterId != details.subChapterId) || (registrationInfo.registrationType != details.registrationType) || (registrationInfo.userType != details.userType) || (registrationInfo.identityType != details.identityType) || (registrationInfo.profession != details.profession) || (registrationInfo.industry != details.industry)) {
        const updatedResp = MlRegistration.update({ _id: id }, { $unset: { kycDocuments: '' } })
      }
      ...
      const {
        source, ipAddress, ipLocation, deviceName, deviceNumber
      } = registrationInfo;
      ...
      const communityDef = mlDBController.findOne('MlCommunityDefinition', { code: (details.registrationType || null) }, context) || {};
      /**
      ...
      const user = Meteor.users.findOne({ username: details.userName });
      details.userId = user ? user._id : null;

      const registrationDetails = { identityType: details.identityType, userType: details.userType };
      ...
      const profile = {
        ...
        email: details.email,
        mobileNumber: details.contactNumber,
        isActive: (userProfile.communityDefCode == 'OFB'),
      ...
      if (existingUser) {
        ...
        const externalUserProfiles = existingUser.profile.externalUserProfiles
        const userExProfile = _lodash.find(externalUserProfiles, { registrationId: id })
        ...
        if (userId) {
          ...
          MlNotificationController.createNewUser({ userId, userName: userObject.username })

          /** Email & MobileNumber verification updates to user */
          mlDBController.update(
            'users', { username: userObject.username },
            {
              $set: {
          ...
          if (_lodash.isMatch(details, { communityDefCode: 'OFB' })) {
            const officeObj = {
              registrationId: id,
              officeMember: { userId, profileId: userProfile.profileId, isActive: true }
            }
      ...
      if (updateCount === 1 || userId) {
        ...
        const resp = MlResolver.MlMutationResolver.updateRegistrationTransaction(obj, { transactionInfo }, context, info);
      ...
      const aprvStatus = mlRegistrationRepo.updateStatus({}, 'REG_SOFT_APR');
      ...
      if (existingUser) {
        const dob = args && args.details && args.details.dateOfBirth ? moment(args.details.dateOfBirth).startOf('day').toDate() : null
        const gender = args && args.details && args.details.gender ? args.details.gender : null
        result = mlDBController.update(
...
MlResolver.MlMutationResolver.updateRegistrationUploadedDocumentUrl = (obj, args, context, info) => {
  ...
  if (args.docUrl && args.documentId && args.docTypeId) {
    ...
    const documentDetails = MlDocumentMapping.findOne(args.documentId)
    ...
    const updatedResponse = mlDBController.update('MlRegistration', {
    ...
    if (updatedResponse) {
      const statusResponse = mlDBController.update('MlRegistration', {
        _id: args.registrationId,
        kycDocuments: { $elemMatch: { documentId: args.documentId, docTypeId: args.docTypeId } }
      }, { 'kycDocuments.$.status': 'Pending Verification' }, { $set: true }, context)
      if (statusResponse) {
        let allManditoryKYCuploaded;
        const registrationDetails = MlRegistration.findOne(args.registrationId)
        const registrationKYCDocs = registrationDetails && registrationDetails.kycDocuments && registrationDetails.kycDocuments.length ? registrationDetails.kycDocuments : []
        ...
        if (allManditoryKYCuploaded) {
          ...
          const updatedResponse = mlDBController.update('MlRegistration', args.registrationId, updateRecord, { $set: true }, context)
...
MlResolver.MlMutationResolver.ApprovedStatusForUser = (obj, args, context, info) => {
  ...
  if (args.registrationId) {
    let temp = 0
    const registrationRecord = MlRegistration.findOne(args.registrationId)

    const addressDetails = registrationRecord && registrationRecord.addressInfo ? registrationRecord.addressInfo : []
    ...
    if (addressDetails && addressDetails.length < 1) {
      const code = 401;
      const response = new MlRespPayload().errorPayload("'Default Address' is mandatory", code);
      return response;
       ...
       */
      const found = addressDetails.some(el => el.isDefaultAddress == true);
      if (!found) {
        /**
         * If registration contains address array
         ...
         */
        const addressData = _.filter(addressDetails, { isDefaultAddress: true });
        if (addressData && addressData.length > 1) {
          const code = 401;
          const response = new MlRespPayload().errorPayload('Only one default address should exist', code);
          return response;
        }
      }
    }
    if (registrationRecord && registrationRecord.emails.length > 0) {
      const email = registrationRecord.emails;
      emailVerified = _.find(email, mail => mail.verified == true)
      if (emailVerified) {
        if (registrationRecord && registrationRecord.status != 'REG_USER_APR' && registrationRecord.status != 'REG_USER_REJ') {
          const kycDocuments = registrationRecord.kycDocuments && registrationRecord.kycDocuments.length > 0 ? registrationRecord.kycDocuments : []
          if (kycDocuments && kycDocuments.length >= 1) {
            // mandatory doc exist or not
            mandatorykycDoc = kycDocuments.filter(item => item.isMandatory == true);
            if (mandatorykycDoc.length >= 1) {
              // mandatory document should approved and upload
              kycDoc = mandatorykycDoc.filter(item => item.status == 'Approved' && item.docFiles.length >= 1 && item.isMandatory == true);
              if (kycDoc && kycDoc.length == mandatorykycDoc.length) {
                // nonmandatory document exist should approved and upload
                nonMandatorykycDoc = kycDocuments.filter(item => item.docFiles.length >= 1 && item.isMandatory == false);
                if (nonMandatorykycDoc.length >= 1) {
                  status = nonMandatorykycDoc.filter(item => item.status == 'Approved');
          ...
          const response = new MlRespPayload().errorPayload(getRegStatus, code);
          // let response = new MlRespPayload().errorPayload("User already approved", code);
          return response;
        }
      } else {
        const code = 556;
        const response = new MlRespPayload().errorPayload('Email verification not completed', code);
        return response;
      }
    }
    ...
    if (updatedResponse === 1) {
      ...
      const regRecord = mlDBController.findOne('MlRegistration', { _id: args.registrationId }, context) || { registrationInfo: {} };
    ...
    if (updatedResponse === 1) {
      const code = 200;
      const result = { registrationId: updatedResponse }
      const response = new MlRespPayload().successPayload(result, code);
      return response
    }
    const code = 401;
...
MlResolver.MlMutationResolver.RejectedStatusForUser = (obj, args, context, info) => {
  if (args.registrationId) {
    const isRejected = mlDBController.findOne('MlRegistration', { _id: args.registrationId, status: { $in: ['REG_ADM_REJ', 'REG_USER_REJ'] } }, context)
    const statusCode = args.regType === '1' ? 'REG_USER_REJ' : 'REG_ADM_REJ';
    const updateRecord = {};
    // Update the Status
    ...
    if (!isRejected) {
      const updatedResponse = mlDBController.update('MlRegistration', args.registrationId, updateRecord, { $set: true }, context)
      if (updatedResponse) {
        const resp = mlDBController.update('users', {
        ...
        const user = mlDBController.findOne('MlRegistration', { _id: args.registrationId }, context)
        MlEmailNotification.UserRejectionByAdmin(user);
        const code = 200;
        const response = new MlRespPayload().successPayload('Registration rejected successfully', code);
        return response
      }
    } else {
      const code = 401;
      const response = new MlRespPayload().errorPayload('Registration already rejected', code);
      return response;
    }
  }
}
...
MlResolver.MlMutationResolver.ApprovedStatusOfDocuments = (obj, args, context, info) => {
  // TODO : Authorization
  if (args.registrationId) {
    const documentList = args.documentId;
    const doctypeList = args.docTypeId
    let updatedResponse;
    const updateRecord = {}
    if (documentList.length > 0) {
      for (let i = 0; i < documentList.length; i++) {
        ...
        const user = MlRegistration.findOne({ _id: args.registrationId }) || {}
        const kyc = user.kycDocuments && user.kycDocuments.length > 0 ? user.kycDocuments : []
        const kycDoc = _.find(kyc, item => item.documentId == documentList[i] && item.docTypeId == doctypeList[i]);
        if (kycDoc.docFiles && kycDoc.docFiles.length > 0) {
          const response = mlDBController.update('MlRegistration', {
            _id: args.registrationId,
          ...
          if (response) {
            ...
            */
            ...
            const code = 200;
            const result = { registrationId: response }
...
MlResolver.MlMutationResolver.RemoveFileFromDocuments = (obj, args, context, info) => {
  ...
  if (args.registrationId) {
    ...
    const user = MlRegistration.findOne({ _id: args.registrationId })
    const kyc = user.kycDocuments
    let kycDoc = _.find(kyc, item => item.documentId == args.documentId && item.docTypeId == args.docTypeId);
    if (kycDoc.docFiles.length > 0 && kycDoc.status != 'Approved') {
      ...
      if (response) {
        const registrationRecord = MlRegistration.findOne(args.registrationId);
        const kycDocuments = registrationRecord.kycDocuments
        if (kycDocuments && kycDocuments.length >= 1) {
          // if doc not available
          kycDoc = _.find(kycDocuments, item => item.docFiles.length < 1 && item.docTypeId == args.docTypeId && item.documentId == args.documentId);
        ...
        const updateRecord = {}
        const existingStatus = registrationDetails && registrationDetails.status ? registrationDetails.status : 'REG_KYC_U_KOFF'
        if (allManditoryKYCuploaded) {
...
MlResolver.MlMutationResolver.createGeneralInfoInRegistration = (obj, args, context, info) => {
  let id = ' '
  const registrationDetails = MlRegistration.findOne({ _id: args.registrationId }) || {};
  const fut = new Future();
  if (args && args.registration) {
    if (args.type == 'CONTACTTYPE') {
      const dbData = _.pluck(registrationDetails.contactInfo, 'numberType') || [];
      ...
      if (contactExist) {
        const code = 409;
        const response = new MlRespPayload().errorPayload("'Contact type' already exists!", code);
        return response;
      }
      ...
      const dbData = _.pluck(registrationDetails.addressInfo, 'addressType') || [];
      ...
      if (addressExist) {
        const code = 409;
        const response = new MlRespPayload().errorPayload("'Address type' already exists!", code);
        return response;
      }
      if (registrationDetails.addressInfo) {
        ...
        // )
        const city = args.registration.addressInfo[0].addressCity
        const area = args.registration.addressInfo[0].addressArea
        const locality = args.registration.addressInfo[0].addressLocality
        const pin = args.registration.addressInfo[0].addressPinCode
        geocoder.geocode(`${locality},${area},${city},${pin}`, Meteor.bindEnvironment((err, data) => {
          if (err) {
          ...
          try {
            ...
            const id = mlDBController.update('MlRegistration', args.registrationId, { addressInfo: args.registration.addressInfo[0] }, { $push: true }, context)
            if (id) {
              const code = 200;
              const result = { addressId: id }
        ...
        geocoder.geocode(`${locality},${area},${city},${pin}`, Meteor.bindEnvironment((err, data) => {
          ...
          try {
            // let id = MlClusters.insert(cluster);
            const id = mlDBController.update('MlRegistration', args.registrationId, { addressInfo: args.registration.addressInfo }, { $set: true }, context)
            if (id) {
              const code = 200;
              const result = { addressId: id }
              const response = JSON.stringify(new MlRespPayload().successPayload(result, code));
      ...
      const dbData = _.pluck(registrationDetails.socialLinksInfo, 'socialLinkType') || [];
      let addressExist = null;
      ...
      if (addressExist) {
        const code = 409;
        const response = new MlRespPayload().errorPayload("'Social link' type already exists!", code);
        return response;
      }
      ...
      /* if(args.registration.addressInfo && args.registration.addressInfo[0]){ */
      const dbData = _.pluck(registrationDetails.emailInfo, 'emailIdType') || [];
      ...
      if (emailTypeExist) {
        const code = 409;
        const response = new MlRespPayload().errorPayload("'Email type' already exists!", code);
        return response;
      }
      ...
      /* if(args.registration.addressInfo && args.registration.addressInfo[0]){ */
      const newKYCDocs = args.registration && args.registration.kycDocuments ? args.registration.kycDocuments : []
      if (registrationDetails.kycDocuments) {
        const existisngKYCDocs = registrationDetails && registrationDetails.kycDocuments && registrationDetails.kycDocuments.length > 0 ? registrationDetails.kycDocuments : []
        const registrationKYCUpdatedDocs = []
        _.map(existisngKYCDocs, (arr2obj) => {
          _.find(newKYCDocs, (arr1obj) => {
  ...
  if (id) {
    ...
    const insertedData = MlRegistration.findOne(id) || {};
...
MlResolver.MlQueryResolver.findRegistration = (obj, args, context, info) => {
  ...
  const resp = mlDBController.findOne('MlRegistration', { _id: args.registrationId }, context)
  return resp;
}
...
MlResolver.MlMutationResolver.sendSmsVerificationForRegistration = (obj, args, context, info) => {
  ...
  if (args.registrationId) {
    ...
    // if ( userId ) {
    const result = MlResolver.MlMutationResolver.sendSmsVerification(obj, { registrationId: args.registrationId }, context, info);
    // return result;
    if (result) {
      const code = 200;
      const result = { registrationId: args.registrationId }
      const response = new MlRespPayload().successPayload(result, code);
      return response
    }
    //  }else{
...
MlResolver.MlMutationResolver.sendEmailVerificationForRegistration = (obj, args, context, info) => {
  ...
  if (args.registrationId) {
    ...
    // const userId=regDetails&&regDetails.registrationInfo&&regDetails.registrationInfo.userId?regDetails.registrationInfo.userId:null;
    // if ( userId ) {
    const result = MlResolver.MlMutationResolver.sendEmailVerification(obj, { registrationId: args.registrationId }, context, info);
    if (result) {
      const code = 200;
      const result = { registrationId: args.registrationId }
      const response = new MlRespPayload().successPayload(result, code);
...
MlResolver.MlMutationResolver.sendUserSmsVerification = (obj, args, context, info) =>
  // TODO : Authorization
  MlAccounts.sendUserVerificationSmsOtp(context.userId)


MlResolver.MlMutationResolver.resendUserSmsVerification = (obj, args, context, info) => {
  // TODO : Authorization
  const resp = MlAccounts.resendUserVerificationSmsOtp(context.userId);
  if (resp && resp.otp) {
    return {
      mobileNumber: resp.mobileNumber, success: true, reason: 'Successfully resend OTP', code: 200
    };
  }
  return {
    mobileNumber: resp.mobileNumber, error: true, reason: 'Resend OTP failed', code: 403
  };
}
...
MlResolver.MlMutationResolver.verifyUserMobileNumber = (obj, args, context, info) => {
  ...
  if (mobileNumber && args.otp) {
    ...
    if (result && result.error) {
      const response = new MlRespPayload().errorPayload(result.reason || '', result.code);
      return response;
    }
    const succResp = { mobileNumber, mobileNumberVerified: true };
...
MlResolver.MlMutationResolver.resendSmsVerification = (obj, args, context, info) => {
  ...
  if (args.mobileNumber) {
    const resp = MlAccounts.resendVerificationSmsOtp(args.mobileNumber);
    if (resp && resp.otp) {
      return {
        mobileNumber: resp.mobileNumber, success: true, reason: 'OTP resent successfully', code: 200
      };
    }
    return {
      mobileNumber: resp.mobileNumber, error: true, reason: 'OTP could not be resent. Please check the mobile number or try after 30 seconds.', code: 403
    };
  }
}
MlResolver.MlMutationResolver.verifyLaterUserMobileNumber = (obj, args, context, info) =>
  // TODO : Authorization
  MlAccounts.verifyLaterUserMobileNumber(args.mobileNumber)
...
MlResolver.MlMutationResolver.verifyMobileNumber = (obj, args, context, info) => {
  ...
  if (args.mobileNumber && args.otp) {
    const otp = args.otp ? Number(args.otp) : null;
    const result = MlAccounts.verifyMobileNumberOtp(args.mobileNumber, otp);
    if (result && result.error) {
      const response = new MlRespPayload().errorPayload(result.reason || '', result.code);
      return response;
    }
    const succResp = { mobileNumber: args.mobileNumber, mobileNumberVerified: true };
    return new MlRespPayload().successPayload(succResp, 200);
  }
  return new MlRespPayload().errorPayload('Mobile Number and OTP are mandatory', 403);
}
...
MlResolver.MlQueryResolver.fetchContextClusters = (obj, args, context, info) => {
  ...
  const listData = [];
  const clusterIds = userProfile && userProfile.defaultProfileHierarchyRefId ? userProfile.defaultProfileHierarchyRefId : [];
...
MlResolver.MlMutationResolver.verifyEmail = (obj, args, context, info) => {
  if (args.token) {
    const updateRecord = {};
    const user = mlDBController.findOne('users', { 'profile.email': 'systemadmin@moolya.global' }, context) || {};
...
MlResolver.MlMutationResolver.createKYCDocument = (obj, args, context, info) => {
  ...
  if (documentDetails.allowableFormat) {
    const documentFormat = MlDocumentFormats.find({ _id: { $in: documentDetails.allowableFormat } }).fetch();
    const documentFormatArray = _.pluck(documentFormat, 'docFormatName') || [];
    if (documentFormatArray) {
      const allowableFormatArray = documentFormatArray.join();
  ...
  const sameKYCEXist = MlRegistration.findOne({ _id: args.registrationId, 'kycDocuments.documentId': kycDocumentObject.documentId });
...
MlResolver.MlQueryResolver.findUserPendingRegistration = (obj, args, context, info) => {
  const user = mlDBController.findOne('users', { _id: context.userId }, context) || {}
  const resp = mlDBController.find('MlRegistration', {
...
MlResolver.MlQueryResolver.findRegistrationInfoUser = (obj, args, context, info) => {
  if (args.registrationId) {
    const id = args.registrationId;
    const response = MlRegistration.findOne({ _id: id }) || {}
    const userId = response.registrationInfo && response.registrationInfo.userId ? response.registrationInfo.userId : ''
    let users = ''
    if (userId) {
...
headerCommunityDisplay = (registrationInfo, context) => {
  const subChapter = mlDBController.findOne('MlSubChapters', { _id: registrationInfo.subChapterId }) || {}
  const isMoolya = subChapter.isDefaultSubChapter
  let subChapterName = registrationInfo.subChapterName
  let chapterName = registrationInfo.chapterName
  const communityDefCode = registrationInfo.communityDefCode
  if (subChapterName && subChapterName.length > 12) { subChapterName = `${subChapterName.substr(0, 12)}...` }
  chapterName = chapterName.substr(0, 4)
  let returnName = registrationInfo.communityName
  if (!isMoolya) { returnName = `${subChapterName}/${chapterName}/${registrationInfo.communityName}` }
  return returnName
};

registrationUserApprovalResponse = (status) => {
  let response = 'User is already approved by admin';
  if (status === 'REG_USER_APR') { response = 'User is already approved by admin'; } else if (status === 'REG_USER_REJ') { response = 'Rejected user cannot be approved'; }
  return response
};
