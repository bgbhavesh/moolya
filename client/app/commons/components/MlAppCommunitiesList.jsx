...
export default class MlAppCommunitiesList extends Component {
  ...
  setSelectedCommunity(selCommunity, idx, e) {
    ...
    this.setState({
      selectedCommunity: selCommunity, popoverOpen: !(this.state.popoverOpen), identity: null, registrationType: null, clusterId: null
    })
  }
  ...
  async registerAs() {
    ...
    if (response.success) {
      const registrtionId = response.result
      const registrtion = JSON.parse(registrtionId)
  ...
  async fetchCommunities() {
    const communities = await fetchCommunitiesHandlerReg();
    const userCommunity = this.state.communityId
    const status = this.state.status
    if (communities && communities.length) {
      if (status == 'REG_USER_APR') {
        if (userCommunity == 'CMP') {
          const communitilist = _.filter(communities, community => community.code === 'CMP');
          this.setState({ communities: communitilist })
        } else if (userCommunity == 'INS') {
          const communitilist = _.filter(communities, community => community.code === 'INS');
          this.setState({ communities: communitilist })
        } else if (userCommunity == 'BRW') {
          const communitilist = _.filter(communities, community => community.code != 'BRW' && community.code != 'OFB');
          this.setState({ communities: communitilist })
        } else {
          const communitilist = _.filter(communities, community => community.code === 'FUN' || community.code === 'IDE' || community.code === 'STU' || community.code === 'SPS');
          this.setState({ communities: communitilist })
        }
      } else {
        const communitilist = _.filter(communities, community => community.code != 'BRW' && community.code != 'OFB');
  ...
  render() {
    ...
    const clusterQuery = gql`query($subChapterId: String){data:fetchClustersForMap(subChapterId:$subChapterId){label:displayName,value:_id}}`;

    const identityOptions = { options: { variables: { communityId: this.state.selectedCommunity } } };
    const clusterOptions = { options: { variables: { subChapterId: this.state.subChapterId } } };
