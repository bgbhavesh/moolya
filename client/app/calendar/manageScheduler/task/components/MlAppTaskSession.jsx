...
export default class MlAppTaskSession extends Component {
  ...
  async findTaskDetails() {
    ...
    if (response) {
      const userSession = _.range(response ? response.noOfSession : 0);
      console.log(userSession);
      const sessionData = [];
      _.each(userSession, (item, value) => {
        sessionData.push({
      ...
      if (response && !this.props.editMode) {
        this.setState({ loading: false, data: response, sessionData });
      } else if (_.isEmpty(response.session)) {
        this.setState({ loading: false, data: response, sessionData });
      } else {
        const sessionEditData = _.cloneDeep(response.session);
  ...
  isOfflineSession(activities) {
    let isOffline = true;
    if (!this.state.offlineActivites || !(activities && activities.length)) { return false; }
  ...
  async fetchActivities() {
    ...
    const taskId = this.props.taskId;
    if (taskId) {
      const response = await fetchActivitiesForTaskActionHandler(taskId);
      // let response = await fetchActivitiesActionHandler(profileId);
      const offlineActivites = [];
      _.each(response, (a) => {
        if (a.mode !== 'online') { offlineActivites.push(a._id); }
      });
      if (response) {
        this.setState({
          activities: response,
  ...
  handelBlur(id, e) {
    ...
    const specificData = cloneBackUp[id]
  ...
  handelBlurCopy(id, name, value) {
    const dataList = this.state.sessionData // anotherlist for listing
    const cloneBackUpList = _.cloneDeep(dataList);
    const specificDataList = cloneBackUpList[id]
  ...
  sendSessionDataToParent() {
    const data = _.cloneDeep(this.state.sessionData);
    // data.forEach(d=> {
    //   delete d.isOffline;
    // })
  ...
  addActivity(activity, index) {
    ...
    const totalMinutes = sessionData[index].activities.reduce((sum, value) => {
      const activity = that.state.activities.find(activity => activity._id == value);
      if (activity) {
  ...
  render() {
    const that = this;
    const options = that.state.activities ? that.state.activities.map(activity => ({ value: activity._id, label: activity.displayName })) : [];
      ...
      <div className="step_form_wrap step1">
        ...
        <ScrollArea speed={0.8} className="step_form_wrap" smoothScrolling={true} default={true}>
          <div className="form_bg">
            {this.state.sessionData.map((session, id) => (
                  ...
                  <div className="col-md-3">
                    <div style={{ marginTop: '-12px' }}>
                      <Select
                        ...
                        options={options.filter((activity) => {
                ...
                <div className="panel-body" >
                  <div className="swiper-container manage_tasks">
                    <div className="swiper-wrapper">
                      {session.activities.map((ss, index) => {
                        const activity = that.state.activities.find(activity => ss == activity._id);
                        if (activity) {
                          return (
