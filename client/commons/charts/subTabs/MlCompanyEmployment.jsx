...
export default class MlCompanyEmployment extends React.Component {
  ...
  sendDataToParent(index) {
    if (index == null) {
      ...
      const arr = [];
      _.each(startupCompanyEmployment, (item) => {
      ...
      this.setState({ startupCompanyEmployment })
  ...
  componentWillMount() {
    const empty = _.isEmpty(this.context.startupPortfolio && this.context.startupPortfolio.employmentOfCompanyChart)
    if (!empty) {
