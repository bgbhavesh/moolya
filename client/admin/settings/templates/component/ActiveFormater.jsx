...
class ActiveFormater extends React.Component {
  ...
  async onChange(data) {
    ...
    const isActive = this.refs.status.checked;
    const templateCode = this.props.data.templateCode;
    const templateId = this.props.templateId
    const stepCode = this.props.stepCode
    const subprocessId = this.props.subProcessConfig
    const response = await updateTemplateActionHandler(templateId, templateCode, isActive);
    if (response) {
      if (response.success) { FlowRouter.go(`/admin/settings/stepDetails/${subprocessId}/${templateId}/${stepCode}`); } else { toastr.error(response.result); }
    }
  }
