MlProviderPromocodes = new Mongo.Collection('MlProviderPromocodes');
 
var MlProviderPromocodeSchema=new SimpleSchema([CommonAuditSchema,{
	type:{
		type:String,
	...
	code:{
		...
		custom: function () {
			if (Meteor.isClient && this.isSet&&this.isInsert) {
				var code=ZyProviderPromocodes.findOne({code:{$regex:'^'+this.value+'$',$options: "i" }});
				if(code){
					return "recordExists";
				...
				if(Meteor.isClient){
					...
					if(fromDate && this.isSet){
						...
						Meteor.call("checkDateOverlap",docId,fromDate,"from", function (err,res){
							if(res){
								//ZyProviderPromocodes.simpleSchema().namedContext(context).addInvalidKeys([{name:"validity.from",type:"dateOverlap"}])
					...
					var promoCodeFromDate =null;
					if(fromDate){
						promoCodeFromDate=ZyProviderPromocodes.findOne({"_id":{$ne:docId},"validity.from":{$lte: fromDate},"validity.to":{$gte:fromDate},isActive:true,type:"referral"});
				...
				if (Meteor.isClient) {
					...
					if (toDate && this.isSet) {
						...
						Meteor.call("checkDateOverlap", docId, toDate, "to", function (err, res) {
							if (res) {
								//ZyProviderPromocodes.simpleSchema().namedContext(context).addInvalidKeys([{name:"validity.to",type:"dateOverlap"}])
								FlashMessages.sendError("Date overlap with another promocode");
								return "dateOverlap"
							}
						})
					...
					var promoCodeToDate = null;
					if (toDate) {
						promoCodeToDate = ZyProviderPromocodes.findOne({
							"_id": {$ne: docId},
...
SimpleSchema.messages({
	'atleastOneDefaultVariantShouldBeThere': "At least one Variant should be default",
	'cannotHaveMultipleDefaultVariants': "Cannot specify multiple default variants",
	'defaultVariantMustBeActive':"A default variant must be active"
});


SimpleSchema.messages({'quantityMaximumCustomValidation': "Quantity maximum should be greater than Quantity minimum",
	'minimumPurchaseDiscountAmountValidation':"Minimun Purchase Amount should be greater than Discount Amount",
	'ageValidation':"To Age should be greater than From Age"});
SimpleSchema.messages({'quantityMinimumCustomValidation': "Quantity minimum should be less than Quantity maximum"});
SimpleSchema.messages({'validFromValidation': "Valid From Date must be less than Valid To Date"});
SimpleSchema.messages({'validToValidation': "Valid To Date must be greater than Valid From Date"});
SimpleSchema.messages({'validStartDate': "Valid From Date should be less than Valid To Date"});
SimpleSchema.messages({'pastDate':"Past Date Selected",'invalidDate':"Invalid date",'validEndDate': "Valid To Date should be greater than or equal to Valid From Date",'dateOfAvailability':"Date of availability is required"});
SimpleSchema.messages({'invalidPercentage':"Percentage value should be in range of 0 to 100"});
SimpleSchema.messages({'dateOverlap':"Date overlap with another promocode","noNegativeValues":"Please enter a valid val","maximumPurchaseAmountGreterThenMinumumAmount":"Maximum amount should greter then minimum amount"});

MlProviderPromocodes.attachSchema(MlProviderPromocodeSchema);
