...
export default class InteractionsCounter extends React.Component {
  ...
  async componentWillMount() {
    ...
    const actionNames = ['like', 'connect', 'collaborate', 'favourite', 'view', 'partner', 'enquire', 'follow'];
    const response = await fetchInteractionsCountActionHandler({ resourceType: this.props.resourceType, resourceId: this.props.resourceId, actionNames });
  ...
  render() {
    const props = this.props.portfolioDetails
    const config = [
    ...
    const actionView = config.map((option) => {
      ...
      const actionCount = (_.find(data, { actionName: option.name }) || {}).count;
      return (
        <li key={option.name}>
      ...
      <div>
        <div className="paperfold panel">
          ...
          <a className="startup-logo" href="" onClick={this.onBackHandler.bind(this)}>
            <img
              src={props && props.portfolioImage ? generateAbsolutePath(props.portfolioImage) : '/images/ideator_01.png'}
