...
function dateFormatter(data) {
  const createdDateTime = data && data.data && data.data.registrationDate ? data.data.registrationDate : null;
  ...
  actionConfiguration: [
    {
      ...
      handler: async (data) => {
        ...
        if (!list || list.length == 0) {
          ...
          const response = await validateTransaction(data.registrationId, 'MlRegistration', data[0].assignedUserId);
