...
jQuery.fn.SharpCalendar = jQuery.fn.SC = function (options) {
  ...
  let calendar = { currentDate: options.date },
  ...
  (function () {
    ...
    } else if (options.combineMonthYear) {
      $('.SCMonth').addClass('Comb');
    ...
    let fillYears = function (date) {
      ...
      fillDays = function (date) {
        ...
        for (i = -options.days; i <= options.days; i += 1) {
          ...
          div.html(`<div><div class="SCDayNum">${options.monthNames[d.getMonth()]}${options.combineMonthYear ? `'${d.getFullYear().toString().substring(2, 4)}` : ''}<br/>${options.doubleDigitsDays ? (`0${d.getDate()}`).slice(-2) : d.getDate()}</div></div>`);
      ...
      finish = function () {
        let auxH = sharpCContainer.height() === 0 ? sharpCContainer.parent().height() : sharpCContainer.height(),
  ...
  function animate(old_d, new_d, time, increment) {
    const aux_d = new Date(calendar.currentDate.getTime());
  ...
  sharpCContainer.click((ev) => {
    let el = $(ev.target).closest('.SCElement'),
