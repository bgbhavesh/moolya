...
export default class Review extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      reviewRating: 0, reviews: [], viewType: 'view', selectedReview: {}
    };
  ...
  async reviewHandler() {
    ...
    const response = await reviewActionHandler({
    ...
    if (response && response.success) {
      ...
      this.fetchReviews();
    } else if (response && !response.success && response.code === 401) {
  ...
  async fetchReviews() {
    const resourceDetails = this.props.data || {};
    const reviews = await fetchReviewsActionHandler({ resourceId: resourceDetails.resourceId, resourceType: resourceDetails.resourceType, cursor: '' });
      ...
      <div className="footer_popover pop_review">
        ...
        {this.state.viewType === 'view' && <div>
          <ul className="review_pop main_review text-left">
            {this.state.reviews.map((review, id) => (<li className="click_full" key={id}>
              <div className="media">
