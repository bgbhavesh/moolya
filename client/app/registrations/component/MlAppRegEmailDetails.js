...
export default class MlAppRegEmailDetails extends React.Component {
  ...
  updateEmailOptions(index, did, selectedValue, selObject, callback) {
    ...
    if (index !== -1) {
      // do your stuff here
      const updatedComment = update(this.state.emailDetails[index], {
          ...
          {that.state.emailDetails && (that.state.emailDetails.map((options, key) => (
            <li key={key} onClick={that.emailTabSelected.bind(that, key)}>
              <a data-toggle="pill" href={`#emailIdType${key}`} className="add-contact">
                <FontAwesome
                  name='minus-square'
                  onClick={that.onDeleteEmail.bind(that, key)}/>{options.emailIdTypeName}</a>
            </li>)))}
        </ul>
        ...
        <div className="tab-content clearfix">
          <div className={`tab-pane${this.state.activeTab}`} id="emailA">
            ...
            <div className="form-group mandatory">
              <input
                type="text" placeholder="Enter Email Id" ref={'emailId'} className="form-control float-label"
