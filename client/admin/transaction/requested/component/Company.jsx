...
export default class Company extends React.Component {
  ...
  render() {
    ...
    const companytypesquery = gql`query($type:String,$hierarchyRefId:String){
     ...
     `;
    const companytypesOption = { options: { variables: { type: 'COMPANYTYPE', hierarchyRefId: this.props.registrationInfo.clusterId } } };

    const businesstypesquery = gql` query{
    ...
    `;
    const lookinforquery = gql`query($communityCode:String){
  data:fetchLookingFor(communityCode:$communityCode) {
    ...
    `;
    const lookingOption = { options: { variables: { communityCode: this.props.registrationInfo.registrationType } } };
    const stageofcompquery = gql` query{
    data:fetchStageOfCompany{label:stageOfCompanyName,value:_id}
    }
    `;
    const citiesquery = gql`query($searchQuery:String,$countryId: String){
      data:fetchHeadQuarterOfRegisteredCommunity(searchQuery:$searchQuery,countryId:$countryId){label:name,value:_id}
    }
    `;

    const branchesQuery = gql`query($searchQuery:String,$countryId: [String]){
      data:fetchBrachesOfRegisteredCommunity(searchQuery:$searchQuery,countryId:$countryId){label:name,value:_id}
    }
    `;

    const countryOption = { options: { variables: { countryId: this.state && this.state.selectedHeadquarter ? this.state.selectedHeadquarter : '' } } };
    const branchesOption = { options: { variables: { countryId: this.state && this.state.selectedBranches ? this.state.selectedBranches : null } } };
    const userTypeOption = { options: { variables: { communityCode: this.props.registrationInfo.registrationType } } };
