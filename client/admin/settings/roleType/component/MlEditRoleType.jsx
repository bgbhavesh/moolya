...
class MlEditRoleType extends React.Component {
  ...
  async handleSuccess(response) {
    if (response) {
      if (response.success) { FlowRouter.go('/admin/settings/roleTypeList'); } else { toastr.error(response.result); }
    }
  }

  async findRoleType() {
    const roleTypeId = this.props.config;
    const response = await findRoleTypeActionHandler(roleTypeId);
    this.setState({ loading: false, data: response });
  }
  async updateRoleType() {
    const RoleTypeDetails = {
