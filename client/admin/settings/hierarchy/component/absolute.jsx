...
export default class MlHierarchyDetails extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      data: {},
      hierarchyInfo: [{
        departmentId: '', departmentName: '', subDepartmentId: '', subDepartmentName: '', isMoolya: ''
      }],
      unassignedRoles: [],
      assignedRoles: [],
      finalApproval: null,
  ...
  async findDeptAndsubDept() {
    ...
    const isDefault = this.props.defaultSubChapter;
  ...
  async updateunassignedRoles() {
    const roles = []
    const hierarchyInfo = this.state.unassignedRoles.teamStructureAssignment;
    if (hierarchyInfo) {
  ...
  async updateassignRoles() {
    const roles = []
    const hierarchyInfo = this.state.assignedRoles.teamStructureAssignment;
    if (hierarchyInfo) {
      for (let i = 0; i < hierarchyInfo.length; i++) {
        const json = {
          id: hierarchyInfo[i].roleId,
  ...
  expandComponent(row) {
    let subdepartment,
      selctedRowDetails = false;
    const selectedRow = this.refs.table
    if (selectedRow) {
      const selectedRowValue = selectedRow.state.selectedRowKeys;
  ...
  async updateHierarchy() {
    const response = await this.updatehierarchyAssignments();
    if (response && response.result) {
      ...
      <div className="admin_main_wrap">
        <div className="admin_padding_wrap">
          ...
          <div className="main_wrap_scroll">
            ...
            >
              <BootstrapTable
                ref='table' data={ this.state.hierarchyInfo }
                options={ options }
