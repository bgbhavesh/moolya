...
export default class MlStartupManagement extends Component {
  ...
  onSelectUser(index, uiIndex, e) {
    ...
    let managmentDetails = _.find(this.state.startupManagement, { index });
  ...
  async fetchPortfolioDetails() {
    ...
    const empty = _.isEmpty(that.context.startupPortfolio && that.context.startupPortfolio.management)
    if (empty) {
  ...
  onSaveAction() {
    ...
    let setObject = this.state.startupManagementList
  ...
  sendDataToParent(isSaveClicked) {
    ...
    const startupManagement1 = this.state.startupManagement;
    ...
    this.setState({ startupManagement })
  ...
  async fetchOnlyImages() {
    ...
    if (response) {
      ...
      const dataDetails = this.state.startupManagement
      ...
      if (specificData) {
        ...
        this.setState({ loading: false, startupManagement: cloneBackUp, data: specificData }, () => {
          $('#management-form').slideDown();
        });
      } else {
        this.setState({ loading: false })
  ...
  async imagesDisplay() {
    const response = await fetchStartupDetailsHandler(this.props.portfolioDetailsId, KEY);
    if (response) {
      const detailsArray = response.management ? response.management : []
      const dataDetails = this.state.startupManagement
      const cloneBackUp = _.cloneDeep(dataDetails);
      _.each(detailsArray, (obj, key) => {
        cloneBackUp[key].logo = obj.logo;
      })
      // listDetails = this.state.startupManagementList || [];
      const listDetails = cloneBackUp || [];
      const cloneBackUpList = _.cloneDeep(listDetails);
  ...
  render() {
    ...
    const managementArr = that.state.startupManagementList || [];
              ...
              <div className="col-lg-12">
                <div className="row">
                  ...
                  {managementArr.map((user, index) => {
                    const genderImage = user.gender === 'female' ? '/images/female.jpg' : '/images/def_profile.png';
                    return (
