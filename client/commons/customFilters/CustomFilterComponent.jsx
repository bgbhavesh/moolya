...
export default class MlCustomFilterComponent extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    const filterDef = this.props.filterDefinition;
    const newFilterDef = nextProps.filterDefinition;
  ...
  fetchFilters(filterDef) {
    ...
    const responseFields = filter && filter.filterFields || [];
  ...
  onFromDateSelection(fieldName, subType, displayName, event) {
    if (event._d) {
      const value = moment(event._d).format('MM/DD/YYYY HH:mm:ss');
  ...
  optionsSelected(index, selectedFieldName, displayName, clearFields, selectedValue, callback, selObject) {
    if (displayName == 'Cluster') {
      const that = this
      const result = _.map(clearFields, (currentObject) => {
        const selectedOption = `selectedOption_${currentObject}`;
        that.setState({ [selectedOption]: null })
      });
    }

    const selectedOption = `selectedOption_${selectedFieldName}`;
    const selectedValues = this.state.selectedDropdownValues;
  ...
  setFilterData(selectedFieldName, selectedValue, displayName, clearFields, selectedType, selectedSubType) {
    ...
    const selector = this.create_selector(selectedFieldName, selectedValue, selectedType, selectedSubType);
    // only for between operator


    queries = $.grep(queries, e => e.fieldName != selectedFieldName);

    if (selectedType == 'Date') {
      const dateSelect = {
        fieldName: selectedFieldName, value: JSON.stringify(this.state.dateQuery), fieldType: selectedType, operator: '$and'
      }
      ...
      if (selectedType == 'List') {
        if (displayName == 'Cluster') {
          ...
          _.map(clearFields, (currentObject) => {
  ...
  create_selector(selectedFieldName, selectedValue, selectedType, selectedSubType) {
    ...
    switch (selectedType) {
      case 'String':
        ...
        select = {
          fieldName: selectedFieldName, value: selectedValue, fieldType: selectedType, operator: '$and'
        }
        return select;
        break;
      case 'Number':
        ...
        select = {
          fieldName: selectedFieldName, value: Number(selectedValue), fieldType: selectedType, operator: '$and'
        }
        return select;
      case 'Boolean':
      ...
      case 'Date':
        ...
        const value = moment(selectedValue).startOf('day').toDate();
  ...
  async onCancelFilter() {
    const selectedValues = this.state.selectedDropdownValues
    const inputValues = this.state.filterQueries
    const that = this;
    _.map(selectedValues, (currentObject) => {
      const selectedOption = currentObject.selectedOption;
      that.setState({ [selectedOption]: '' })
    });
    _.map(inputValues, (currentObject) => {
      if (currentObject.fieldType == 'String') {
        const eneteredInput = currentObject.fieldName;
  ...
  render() {
    const that = this;
    const fieldsData = that.props.filterDefinition || {};
    ...
    return (
      <div className="filter_table filter_hide">
        <div className="panel panel-default">
          ...
          {fieldsData && fieldsData.filterFields && fieldsData.filterFields.map((options, id) => {
            let filterListQuery = null
            let select = '';
            let selectedValue = '';
            const zz = options.fieldList || [];
            const moduleType = that.props.module ? that.props.module : ''
            const fieldListDataArray = _.map(zz, (row) => {
              const val = _.omit(row, ['__typename']);
              return val;
            });

            const fieldActive = that.state.isActiveField || ''
            ...
            if (options.fieldType == 'List') {
              ...
              listOptions = {
                options: {
                  variables: {
                    moduleName: options.fieldResolverName, list: fieldListDataArray, filteredListId: that.state.filterQueries, fieldActive, moduleType
                  }
                }
              }
            } else {
            ...
            const fieldListData = options.fieldList && options.fieldList ? options.fieldList[0].listValueId : []
