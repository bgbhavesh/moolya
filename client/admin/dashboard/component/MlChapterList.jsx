...
export default class MlChapterList extends Component {
  ...
  render() {
    ...
    const list = data.map((prop) => {
      const image = prop.countryFlag && prop.countryFlag.trim() !== '' ? <img src={`${Meteor.settings.public.countriesFlagBaseUrl + prop.countryFlag}`}/> : <span className="ml my-ml-chapter"></span>;
      return (
