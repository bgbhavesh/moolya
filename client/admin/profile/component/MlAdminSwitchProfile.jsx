...
export default class MlAdminSwitchProfile extends React.Component {
  constructor(props, context) {
    super(props);
    this.state = {
      loading: true, swiper: null, userProfiles: [], currentSlideIndex: 0, clusterData: {}
    };
  ...
  async fetchExternalUserProfiles() {
    ...
    if (response) {
      const index = _.findIndex(response, { isDefault: true }) || 0;
      const initialSlideIndex = index >= 0 ? index : 0;
  ...
  async setDefaultUserProfile() {
    const profileDetails = this.state.userProfiles[this.state.currentSlideIndex] || {};
    let response = null;
  ...
  async componentWillMount() {
    const resp = await this.fetchExternalUserProfiles();
    this.initializeSwiper();
  }
  ...
  render() {
    ...
    const that = this
    const profileDetails = that.state.userProfiles[that.state.currentSlideIndex] || {};
    const profileExists = !!(that.state.userProfiles && that.state.userProfiles.length > 0);
    const clusterData = that.state.clusterData || {}
              ...
              <div className="col-md-8">
                <div className="swiper-container profile_container">
                  <div className="swiper-wrapper">
                    ...
                    {that.state.userProfiles.map((prf, idx) => (
