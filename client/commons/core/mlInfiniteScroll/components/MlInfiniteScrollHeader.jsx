...
export default class MlInfiniteScrollHeader extends Component {
  ...
  updateSearch(evt) {
    ...
    const searchFields = this.props.config && this.props.config.searchFields ? this.props.config.searchFields : [];
  ...
  render() {
    ...
    const filterData = config.filterData ? config.filterData : [];
    let filterComponent;
    if (config.filter && config.filterComponent) {
      filterComponent = React.cloneElement(config.filterComponent, { submit: updateFilterQuery, filterData });
    }
    console.log(props);
    ...
    return (
      <div>
        ...
        { config.search ?
          <form
            onSubmit={that.updateSearch} style={{
              margin: '0', position: 'absolute', top: '19px', right: '11px', zIndex: '1'
            }}>
