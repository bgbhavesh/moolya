...
MlResolver.MlQueryResolver.AppGenericSearch = (obj, args, context, info) => {
  ...
  /** *******************************************end of all portfolio queries*********************************** */
      ...
      if (cluster.isActive && chapter.isActive && subChapter.isActive) {
        ...
        if (userType == 'Ideators' || userType == 'Investors' || userType == 'Startups' || userType == 'Service Providers' || userType == 'Companies' || userType == 'Institutions') {
          ...
          var pipeline = [
            ...
            {
              $match: {
                'profile.externalUserProfiles.clusterId': clusterId, 'profile.externalUserProfiles.chapterId': chapterId, 'profile.externalUserProfiles.subChapterId': subChapterId, 'profile.externalUserProfiles.communityDefName': userType, 'profile.externalUserProfiles.isApprove': true, 'profile.externalUserProfiles.isActive': true
              }
            }, // Filter out specific community
            {
              $project: {
                _id: 1,
        ...
        // FOR All
          ...
          var pipeline = [
            ...
            {
              $match: {
                'profile.externalUserProfiles.clusterId': clusterId, 'profile.externalUserProfiles.chapterId': chapterId, 'profile.externalUserProfiles.subChapterId': subChapterId, 'profile.externalUserProfiles.isApprove': true, 'profile.externalUserProfiles.isActive': true
              }
            },
    ...
    const pipeline = [
      ...
      {
        $match: {
          'members.userId': userId, 'members.profileId': profileId, 'members.status': 'Pending', isCancelled: { $ne: true }
        }
      },
      {
        $addFields: {
      ...
      {
        $lookup: {
          from: 'users', localField: 'appointmentWith.userId', foreignField: '_id', as: 'userDetails'
        }
      },
    ...
    const pipeline = [
      ...
      {
        $match: {
          'members.userId': userId, 'members.profileId': profileId, 'members.status': 'Accepted', isCancelled: { $ne: true }
        }
      },
      {
        $addFields: {
    ...
    const pipeline = [
      ...
      {
        $match: {
          'members.userId': userId, 'members.profileId': profileId, 'members.status': 'Completed', isCancelled: { $ne: true }
        }
      },
      {
        $addFields: {
    ...
    const pipeline = [
      ...
      {
        $lookup: {
          from: 'mlPortfolioDetails', localField: 'resourceId', foreignField: '_id', as: 'portfolio'
        }
      },
      { $unwind: { path: '$portfolio', preserveNullAndEmptyArrays: true } },
      {
        $lookup: {
          from: 'users', localField: 'portfolio.userId', foreignField: '_id', as: 'portfolioUser'
        }
      },
      ...
      {
        $lookup: {
          from: 'mlIdeas', localField: 'resourceId', foreignField: 'portfolioId', as: 'idea'
        }
      },
      { $unwind: { path: '$idea', preserveNullAndEmptyArrays: true } },
      {
        $lookup: {
          from: 'mlStartupPortfolio', localField: 'resourceId', foreignField: 'portfolioDetailsId', as: 'startup'
        }
      },
      { $unwind: { path: '$startup', preserveNullAndEmptyArrays: true } },
      {
        $lookup: {
          from: 'mlFunderPortfolio', localField: 'resourceId', foreignField: 'portfolioDetailsId', as: 'funder'
        }
      },
    ...
    if (isDefaultSubChapter || userSubChapter.moolyaSubChapterAccess.externalUser.canView || userSubChapter.moolyaSubChapterAccess.externalUser.canSearch) {
      if (isListView && query.bounds) {
        ...
        if (leftLng < 0 && rightLng < 0) {
          if (leftLng > rightLng) {
            clusters = mlDBController.find('MlClusters', {
              $and:
                  [
                  	{ isActive: true },
                  	{
