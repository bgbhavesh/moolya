...
function departmentsFormatter(data) {
  const departments = data && data.data && data.data.departmentsList ? data.data.departmentsList : [];
...
function subdepartmentsFormatter(data) {
  const departments = data && data.data && data.data.subdepartmentsList ? data.data.subdepartmentsList : [];
  if (departments.length > 0) {
    return <div>{departments.join()}</div>;
  }
  return <div>All</div>;
...
function clustersFormatter(data) {
  const departments = data && data.data && data.data.clustersList ? data.data.clustersList : [];
  if (departments.length > 0) {
    return <div>{departments.join()}</div>;
  }
  return <div>All</div>;
...
function subChapterFormatter(data) {
  const departments = data && data.data && data.data.subChapterList ? data.data.subChapterList : [];
  if (departments.length > 0) {
    return <div>{departments.join()}</div>;
  }
  return <div>All</div>;
...
function dateFormatter(data) {
  const createdDateTime = data && data.data && data.data.createdDateTime;
  if (createdDateTime) {
...
function createdByFormatter(data) {
  const createdBy = data && data.data && data.data.createdBy;
  if (createdBy) {
...
function updateddateFormatter(data) {
  const updatedDateTime = data && data.data && data.data.updatedDateTime;
  if (updatedDateTime) {
...
function updatedByFormatter(data) {
  const updatedBy = data && data.data && data.data.updatedBy;
  if (updatedBy) {
...
function roleType(data) {
  const roleType = data && data.data && data.data.roleType;
  if (roleType == 'moolya') {
  ...
  actionConfiguration: [
    ...
    },
    {
      ...
      handler: (data) => {
        ...
        { toastr.error('Please uncheck the record') } else {
