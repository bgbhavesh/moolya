...
class MlAppServiceBasicInfo extends Component {
   ...
   */
  validTillToggle() {
    if (!this.props.viewMode) { $('#date-time').toggleClass('rdtOpen'); }
  }
  ...
  render() {
    ...
    const citiesQuery = gql`query($stateIds:[String]){data:fetchCitiesPerStates(stateIds:$stateIds) {
     value:_id
     label:name
     }
   }`;
    ...
    const states = clusterData.state ? clusterData.state : [];
    console.log(states);
    const statesOption = { options: { variables: { countryId: clusterCode } } };
    const citiesOption = { options: { variables: { stateIds: states } } };
