...
class MlEditSpecificationType extends React.Component {
  ...
  async handleSuccess(response) {
    if (response) {
      if (response.success) { FlowRouter.go('/admin/settings/documentProcess/specificationList'); } else { toastr.error(response.result); }
    }
  }
  async findSpecificationType() {
    const SpecificationTypeId = this.props.config
