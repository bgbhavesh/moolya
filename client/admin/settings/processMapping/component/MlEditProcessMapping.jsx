...
class MlEditProcessMapping extends React.Component {
  ...
  async handleSuccess(response) {
    if (response) {
      if (response.success) { FlowRouter.go('/admin/settings/documentProcess/processList'); } else { toastr.error(response.result); }
    }
  }
  ...
  async updateProcess() {
    ...
    if (ret) {
      ...
      const type = this.state.assignDocument[0].type;
      const category = this.state.assignDocument[0].category;
      if (!type) {
...
`; */
    const fetchUsers = gql` query($communityId:[String]){  
    ...
    `;
    const professionquery = gql` query($industry:[String]!){
...
`; const clusterquery = gql`  query{
  data:fetchActiveClusters{label:countryName,value:_id}
    ...
    const statesQuery = gql`query($clusters:[String]){  
        data:FetchActiveStatesForCluster(clusters:$clusters) {
          value:_id
          label:name
        }  
    }`;
    const chapterquery = gql`query($states:[String],$clusters:[String]){  
    ...
    const stateOption = { options: { variables: { clusters: this.state.clusters } } };
    const chapterOption = { options: { variables: { states: this.state.states, clusters: this.state.clusters } } };
    const subChapterOption = { options: { variables: { chapters: this.state.chapters, clusters: this.state.clusters, displayAllOption: true } } };
    const professionOption = { options: { variables: { industry: this.state.industries } } };
