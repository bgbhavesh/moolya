...
MlResolver.MlQueryResolver.FetchMapData = (obj, args, context, info) => {
  ...
  if (userProfile.hierarchyLevel != 1) {
    ...
    const related = new MlAdminUserContext().getRelatedSubChaptersForNonMoolya(userId);
    const relatedSubChapterIds = related.relatedSubChapterIds;
    const relatedChapterIds = related.relatedChapterIds;
    const isDefaultSubChapter = related.isDefaultSubChapter;
    const userSubChapter = related.userSubChapter;
    ...
    switch (args.moduleName) {
      case 'cluster':
        ...
        if (isDefaultSubChapter) {
          ...
          query = {
            clusterId: args.id, chapterId: { $in: chapIds }, subChapterId: { $in: subIds }, isActive: true
          };
        } else {
          chapterCount = mlDBController.find('MlChapters', { clusterId: args.id, isActive: true, _id: { $in: relatedChapterIds } }, context).count();
          query = {
            clusterId: args.id, isActive: true, chapterId: { $in: relatedChapterIds }, subChapterId: { $in: relatedSubChapterIds }
          };
        }
        break;
      case 'chapter':
        moduleContext = mlDBController.findOne('MlChapters', { _id: args.id }, context).chapterName;
        const chapter = mlDBController.findOne('MlChapters', { _id: args.id }, context);
        if (isDefaultSubChapter) {
          const sc = mlDBController.findOne('MlSubChapters', { chapterId: args.id, isActive: true, isDefaultSubChapter: true }, context);
          chapterCount = 1;
          query = {
            clusterId: chapter.clusterId, chapterId: args.id, subChapterId: sc._id, isActive: true
          };
        } else {
          chapterCount = mlDBController.find('MlSubChapters', { chapterId: args.id, isActive: true, _id: { $in: relatedSubChapterIds } }, context).count();
          query = {
            clusterId: chapter.clusterId, chapterId: args.id, isActive: true, subChapterId: { $in: relatedSubChapterIds }
          };
        }
        break;
      case 'subChapter':
        moduleContext = mlDBController.findOne('MlSubChapters', { _id: args.id }, context).subChapterName;
        const subChapter = mlDBController.findOne('MlSubChapters', { _id: args.id })
  ...
  const communityData = mlDBController.find('MlCommunityDefinition', { isActive: true }, context).fetch();
  const response = [];
  ...
  _.each(communityData, (item, value) => {
    ...
    if (item.communityImageLink != 'ml my-ml-browser_5' && item.communityImageLink != 'ml ml-moolya-symbol') {
      ...
      const pipeline = [
        {
          $match: {
            'profile.isSystemDefined': { $exists: false }, 'profile.isExternaluser': true, 'profile.isActive': true, 'profile.externalUserProfiles': { $elemMatch: queryObj }
          }
        },
        { $unwind: '$profile.externalUserProfiles' },
        {
          $lookup: {
            from: 'mlPortfolioDetails', localField: 'profile.externalUserProfiles.profileId', foreignField: 'profileId', as: 'portfolio'
          }
        },
      ...
      if (query) {
        if (query.clusterId && query.chapterId && query.subChapterId) {
          pipeline.push({
            $match: {
              'profile.externalUserProfiles.clusterId': query.clusterId, 'profile.externalUserProfiles.chapterId': query.chapterId, 'profile.externalUserProfiles.subChapterId': query.subChapterId, 'profile.externalUserProfiles.communityDefName': query.communityDefName, 'profile.externalUserProfiles.isApprove': true, 'profile.externalUserProfiles.isActive': true
            }
          })
        } else if (query.clusterId && query.chapterId) {
          pipeline.push({
            $match: {
              'profile.externalUserProfiles.clusterId': query.clusterId, 'profile.externalUserProfiles.chapterId': query.chapterId, 'profile.externalUserProfiles.communityDefName': query.communityDefName, 'profile.externalUserProfiles.isApprove': true, 'profile.externalUserProfiles.isActive': true
            }
          })
        } else if (query.clusterId) {
          pipeline.push({
            $match: {
              'profile.externalUserProfiles.clusterId': query.clusterId, 'profile.externalUserProfiles.communityDefName': query.communityDefName, 'profile.externalUserProfiles.isApprove': true, 'profile.externalUserProfiles.isActive': true
            }
          })
        }
      }

      const users = mlDBController.aggregate('users', pipeline, context);
  ...
  const TU = _.map(response, 'count');
  const totalUsers = _.sum(TU);
  response.push({
    key: 'totalUsers',
    count: totalUsers,
...
MlResolver.MlQueryResolver.fetchCommunities = (obj, args, context, info) => {
  ...
  const userhierarchy = mlDBController.findOne('MlHierarchy', { level: Number(userProfile.hierarchyLevel) }, context)
  if (!userhierarchy) {
    return { data: null }
  }

  const clusterId = (!args.clusterId && !userhierarchy.isParent ? args.clusterId = userProfile.defaultProfileHierarchyRefId : '') || (args.clusterId && ((args.clusterId == userProfile.defaultProfileHierarchyRefId) || userhierarchy.isParent) ? args.clusterId : '');
  const chapterId = (!args.chapterId && !userhierarchy.isParent ? args.chapterId = ((userProfile.defaultChapters.indexOf('all')) >= 0 == '') : '') || (args.chapterId && ((userProfile.defaultChapters.indexOf('all') >= 0 || userProfile.defaultChapters.indexOf(args.chapterId) > -1) || userhierarchy.isParent) ? args.chapterId : '');
  const subChapterId = (!args.subChapterId && !userhierarchy.isParent ? args.subChapterId = ((userProfile.defaultSubChapters.indexOf('all')) >= 0 == '') : '') || (args.subChapterId && ((userProfile.defaultSubChapters.indexOf('all') >= 0 || userProfile.defaultSubChapters.indexOf(args.subChapterId) > -1) || userhierarchy.isParent) ? args.subChapterId : '');

  if (clusterId != '' && chapterId != '' && subChapterId != '') {
    query = {
      $and: [{
        clusterId, chapterId, subChapterId, hierarchyCode: 'SUBCHAPTER'
      }]
    };
  } else if (clusterId != '' && chapterId != '') {
    query = { $and: [{ clusterId, chapterId, hierarchyCode: 'CHAPTER' }] };
  } else if (clusterId != '') {
    query = { $and: [{ clusterId, hierarchyCode: 'CLUSTER' }] };
  } else if (userhierarchy.isParent) {
    query = { hierarchyCode: 'PLATFORM' };
  }

  const communitiesAccess = MlCommunityAccess.find(query).fetch();
  // let communitiesAccess = mlDBController.find('MlCommunityAccess', query, context).fetch();
  communitiesAccess.map((communityAccess) => {
    // let platformCommunity = MlCommunityAccess.findOne({"hierarchyCode":"PLATFORM", "communityDefCode":communityAccess.communityDefCode});
    const platformCommunity = mlDBController.findOne('MlCommunityAccess', { hierarchyCode: 'PLATFORM', communityDefCode: communityAccess.communityDefCode }, context)
    const community = {};
    ...
    if (iscommunityActive < 0) { iscommunityActive = _.findIndex(userProfile.defaultCommunities, { communityCode: communityAccess.communityDefCode }) }

    if (!platformCommunity.isActive || iscommunityActive < 0) { communityAccess.isActive = false }
...
MlResolver.MlQueryResolver.fetchCommunityDef = (obj, args, context, info) => {
  ...
  if (clusterId != '' && chapterId != '' && subChapterId != '') {
    subChapterQuery = {
      $and: [{
        subChapterId, hierarchyCode: 'SUBCHAPTER', communityDefCode: args.communityId, isActive: true
      }]
    };
    ...
    chapterQuery = {
      $and: [{
        chapterId, hierarchyCode: 'CHAPTER', communityDefCode: args.communityId, isActive: true
      }]
    };
    subChapterQuery = {
      $and: [{
        chapterId, hierarchyCode: 'SUBCHAPTER', communityDefCode: args.communityId, isActive: true
      }]
    };
    ...
    clusterQuery = {
      $and: [{
        clusterId, hierarchyCode: 'CLUSTER', communityDefCode: args.communityId, isActive: true
      }]
    };
    chapterQuery = {
      $and: [{
        clusterId, hierarchyCode: 'CHAPTER', communityDefCode: args.communityId, isActive: true
      }]
    };
    subChapterQuery = {
      $and: [{
        clusterId, hierarchyCode: 'SUBCHAPTER', communityDefCode: args.communityId, isActive: true
      }]
    };
    ...
    _.each(clusters, (clusterid) => {
      let Chapters = [];
      chapterQuery = {
        $and: [{
          hierarchyCode: 'CHAPTER', communityDefCode: args.communityId, clusterId: clusterid, isActive: true
        }]
      };
      communitiesAccess = mlDBController.find('MlCommunityAccess', chapterQuery, context).fetch();
      Chapters = communitiesAccess && _.map(communitiesAccess, 'chapterId');
    ...
    _.each(chapters, (chapterid) => {
      let SubChapters = [];
      subChapterQuery = {
        $and: [{
          hierarchyCode: 'SUBCHAPTER', communityDefCode: args.communityId, chapterId: chapterid, isActive: true
        }]
      };
      communitiesAccess = mlDBController.find('MlCommunityAccess', subChapterQuery, context).fetch();
      SubChapters = communitiesAccess && _.map(communitiesAccess, 'subChapterId');
...
MlResolver.MlMutationResolver.createCommunityAccess = (obj, args, context, info) => {
  ...
  const communitiesDef = mlDBController.find('MlCommunityAccess', { $and: [{ hierarchyCode: 'PLATFORM' }, { isActive: true }] }, context).fetch()
  _.each(communitiesDef, (community) => {
...
MlResolver.MlMutationResolver.createCommunity = (obj, args, context, info) => {
  ...
  const communitiesDef = mlDBController.find('MlCommunityAccess', { subChapterId: args.subChapterId }, context).fetch()
  for (let i = 0; i < communitiesDef.length; i++) {
    if (communitiesDef[i]) {
      const community = {
...
MlResolver.MlMutationResolver.updateCommunityDef = (obj, args, context, info) => {
  ...
  if (!args.communityId) { return new MlRespPayload().errorPayload('Failed to update community 1', 400); }
  if (!context.userId) {
  ...
  // let chapterId = args.chapterId && ((_.find(userProfile.defaultChapters, args.chapterId)) || userHierarchy.isParent) ? args.chapterId: "";
  // let subChapterId = args.subChapterId && ((_.find(userProfile.defaultSubChapters, args.subChapterId)) || userHierarchy.isParent) ? args.subChapterId: ""
  const chapterId = args.chapterId && ((_.findIndex(userProfile.defaultChapters, args.chapterId)) || userHierarchy.isParent) ? args.chapterId : '';
  const subChapterId = args.subChapterId && ((_.findIndex(userProfile.defaultSubChapters, args.subChapterId)) || userHierarchy.isParent) ? args.subChapterId : ''
  ...
  const hierarchy = (levelCode && mlDBController.findOne('MlHierarchy', { code: levelCode }, context)) || '';
  ...
  if (doEdit) {
    ...
    communityAccess = mlDBController.findOne('MlCommunityAccess', {
      $and: [{
        hierarchyCode: levelCode, clusterId: args.clusterId ? args.clusterId : null, chapterId: args.chapterId ? args.chapterId : null, subChapterId: args.subChapterId ? args.subChapterId : null, communityDefCode: args.communityId
      }]
    }, context);
    if (communityAccess) {
      let isUpdate = false;
      ...
      if (isUpdate == true) { resp = mlDBController.update('MlCommunityAccess', communityAccess._id, communityAccess, { $set: true }, context) }
    }
    ...
    _.each(clusters, (item) => {
      _.each(item.difference, (clusterid) => {
    ...
    _.each(chapters, (item) => {
      _.each(item.difference, (chapterid) => {
    ...
    _.each(subchapters, (item) => {
      _.each(item.difference, (subchapterid) => {
    ...
    if (resp) { return new MlRespPayload().successPayload('Successfully updated', 200); }
  }
...
MlResolver.MlQueryResolver.fetchCommunitiesSelect = (obj, args, context, info) => {
  ...
  if (args.clusterId != '' && args.chapterId != '' && args.subChapterId != '') {
    query = {
      $and: [{
        clusterId: args.clusterId, chapterId: args.chapterId, subChapterId: args.subChapterId, isActive: true
      }]
    };
  }
  const communitiesAccess = MlCommunityAccess.find(query).fetch();
...
updateDB = (collectionName, query, payload, options, context) => mlDBController.update(collectionName, query, payload, options, context)
