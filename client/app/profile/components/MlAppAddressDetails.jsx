...
export default class AppAddressDetails extends React.Component {
  ...
  async onDeleteAddress(index, value) {
    const listArray = update(this.state.details, {
      $splice: [[index, 1]]
    });
    const detailsType = 'ADDRESSTYPE';
    const registerid = this.props.registerId;
  ...
  updateOptions(index, did, selectedValue, selObject, callback) {
    ...
    if (index !== -1) {
      ...
      const updatedComment = update(this.state.details[index], {
      ...
      const newData = update(this.state.details, {
  ...
  countryUpdateOptions(index, did, selectedValue, selObject, callback) {
    if (index !== -1) {
      ...
      this.setState({
        details: newData, countrySelectedValue: selObject.label, countryId: did, countryCode: selObject.code
      });
    }
  }
  ...
  async onEditAddress(index, value) {
    ...
    if (index !== -1) {
      // do your stuff here
      const registrationDetails = this.props.addressInfoDetails
...
*/
      ...
      if (contactExist) {
        ...
        /* else if(clusterCountry && addressSelectedCountry && isDeafaultChecked && clusterCountry != addressSelectedCountry) {
          ...
          const labelValue = this.state.selectedAddressLabel ? this.state.selectedAddressLabel : this.state.details[index].addressTypeName;
          const valueSelected = this.state.selectedValue ? this.state.selectedValue : this.state.details[index].addressType;
          const countryLabelValue = this.state.countrySelectedValue ? this.state.countrySelectedValue : this.state.details[index].addressCountry;
          const countryIDValue = this.state.countryId ? this.state.countryId : this.state.details[index].addressCountryId;
          const stateLabelValue = this.state.selectedStateValue ? this.state.selectedStateValue : this.state.details[index].addressState;
          const stateIDValue = this.state.stateId ? this.state.stateId : this.state.details[index].addressStateId;
          const updatedComment = update(this.state.details[index], {
  ...
  async addressTabSelected(index, value) {
    ...
    const countryValue = this.state.countryId ? this.state.countryId : this.state.details[index].addressCountryId;
  ...
  onMakeDefaultChange(index) {
    const addressInfo = this.props && this.props.addressInfoDetails ? this.props.addressInfoDetails : [];
    const presentObject = addressInfo && addressInfo[index] ? addressInfo[index] : {}
    const presentObjIsDefault = presentObject && presentObject.isDefaultAddress ? presentObject.isDefaultAddress : null
    if (!presentObjIsDefault) {
      const dbData = _underscore.pluck(addressInfo, 'isDefaultAddress') || [];
  ...
  render() {
    ...
    const addressTypeOption = { options: { variables: { type: 'ADDRESSTYPE', hierarchyRefId: clusterId } } };
