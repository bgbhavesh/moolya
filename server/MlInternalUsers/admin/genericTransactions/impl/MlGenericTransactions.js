...
class MlGenericTransactions{
  ...
  selfAssignTransaction(transactionType,transactionId, userId){
       ...
       */
      var hierarchyAssignment = mlDBController.find('MlHierarchyAssignments',{'teamStructureAssignment':{$elemMatch: {roleId:defaultRole.roleId}}}).fetch();

      /*
          If hierarchy, allow users to self assign otherwise block them and throw error.
       */
      if(hierarchyAssignment && hierarchyAssignment.length>0){
