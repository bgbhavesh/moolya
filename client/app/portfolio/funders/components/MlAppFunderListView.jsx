...
export default class MlAppFunderListView extends Component {
  ...
  async viewFunderDetails(portfolioId, e) {
    ...
    if (this.props.config.isExplore && response && response.canAccess) { FlowRouter.go(`/app/explore/investor/${portfolioId}`) } else if (response && response.canAccess) { FlowRouter.go(`/app/funder/${portfolioId}`) } else if (response && !response.canAccess) { toastr.error('Portfolio not available for view') }
  }
