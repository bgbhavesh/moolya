...
class MlAppServiceSelectTask extends Component {
  ...
  componentDidMount() {
    ...
    if (this.props && this.props.taskDetails && this.props.taskDetails[0]) {
      const tab = this.props.taskDetails[0];
   ...
   */
  async getOffices() {
    const response = await fetchOfficeActionHandler();
    if (response) {
      this.setState({
        offices: response
      });
  ...
  addUser(activityIdx, teamIdx, userIdx) {
    const { activities, extraUsers } = this.state;
    activities[activityIdx].teams[teamIdx].users[userIdx].isAdded = true;
    const userId = activities[activityIdx].teams[teamIdx].users[userIdx].userId;
    const profileId = activities[activityIdx].teams[teamIdx].users[userIdx].profileId;
  ...
  async getUsers(resp, index, duration) {
    ...
    const activityTeams = activities.map((activity) => {
      activity.teams = activity.teams ? activity.teams : [];
      const teams = activity.teams.map(async (team) => {
        let response;
    ...
    Promise.all(activityTeams.map(Promise.all, Promise)).then((values) => {
      console.log(values);
      values.forEach((teams, index) => {
        const teamsInfo = teams.map((users, userIndex) => {
          const team = activities[index].teams[userIndex];
          const usersInfo = (users && users.length > 0) ? users.map((user) => {
            const userInfo = {
              name: user.name,
                          ...
                          <div className="list_icon mart0">
                            ...
                            <span className="price"><div className="form-group">
                              <label>
                                ...
                                <span
                                  key={activity.duration ? 'notLoadedYetMin' : 'loadedMin'} disabled="true"
  ...
  async chooseTeamType(evt, activityIdx, teamIdx) {
    ...
    if (evt.target.value == 'connections') {
      ...
      if (resp) {
        activities[activityIdx].teams[teamIdx].users = resp.map(user => ({
          name: user.name,
          profileId: user.profileId,
          profileImage: user.profileImage,
          userId: user.userId
  ...
  render() {
    ...
    const serviceTask = this.props.serviceTask ? this.props.serviceTask : [];
    const currentTask = serviceTask.find(data => data.id == that.props.selectedTab);
    const {
