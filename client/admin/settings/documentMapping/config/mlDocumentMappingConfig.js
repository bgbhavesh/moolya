...
function categoriesformatter(data) {
  const categories = data && data.data && data.data.kycCategory ? data.data.kycCategory : [];
  //   let ids =[];
...
function allowableFormatformatter(data) {
  const allowableFormat = data && data.data && data.data.allowableFormat ? data.data.allowableFormat : [];
  // let ids =[];
...
function clustersformatter(data) {
  const clusters = data && data.data && data.data.clusters ? data.data.clusters : [];
...
function chaptersformatter(data) {
  const chapters = data && data.data && data.data.chapters ? data.data.chapters : [];
...
function subChaptersformatter(data) {
  const subChapters = data && data.data && data.data.subChapters ? data.data.subChapters : [];
...
const mlDocumentMappingTableConfig = new MlViewer.View({
  ...
  actionConfiguration: [
    ...
    },
    {
      ...
      actionName: 'add',
      handler: (data) => {
        if (data && data.documentId) { toastr.error('Please uncheck the record') }
