...
const mlEmailTypeTableConfig = new MlViewer.View({
  ...
  columns: [
    ...
    {
      dataField: 'emailTypeInfo.emailName', title: 'Name', dataSort: true, customComponent(data) { return <div>{data.data.emailTypeInfo.emailName}</div> }
    },
    {
      dataField: 'emailTypeInfo.emailDisplayName', title: 'Display Name', dataSort: true, customComponent(data) { return <div>{data.data.emailTypeInfo.emailDisplayName}</div> }
    },
    {
      dataField: 'emailTypeInfo.aboutEmail', title: 'About', dataSort: true, customComponent(data) { return <div>{data.data.emailTypeInfo.aboutEmail}</div> }
    },
  ...
  sizePerPage: 5,
  queryOptions: true,
  buildQueryOptions: config => ({ context: { settingsType: 'EMAILTYPE' } }),
  graphQlQuery: gql`
