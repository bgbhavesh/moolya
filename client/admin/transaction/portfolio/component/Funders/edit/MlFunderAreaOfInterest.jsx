...
export default class MlFunderAreaOfInterest extends Component {
  ...
  async fetchPortfolioDetails() {
    ...
    const empty = _.isEmpty(that.context.funderPortfolio && that.context.funderPortfolio.areaOfInterest)
    const response = await fetchfunderPortfolioAreaInterest(portfolioDetailsId);
    if (empty) {
      if (response && response.length) { this.setState({ loading: false, funderAreaOfInterest: response, funderAreaOfInterestList: response }); } else { this.setState({ loading: false }) }
    } else {
      this.setState({
        loading: false,
  ...
  onTileClick(index, uiIndex, e) {
    const cloneArray = _.cloneDeep(this.state.funderAreaOfInterest);
  ...
  lockPrivateKeys(selIndex) {
    const privateValues = this.funderAreaOfInterestServer && this.funderAreaOfInterestServer[selIndex] ? this.funderAreaOfInterestServer[selIndex].privateFields : []
  ...
  onSaveAction(e) {
    ...
    let setObject = this.state.funderAreaOfInterest
  ...
  onOptionSelectedIndustry(selectedFunding, callback, selObject) {
    ...
    details = _.extend(details, {
      industryTypeId: selectedFunding || null,
  ...
  onLogoFileUpload(image, fileInfo) {
    ...
    const data = {
      ...
      portfolio: { areaOfInterest: [{ logo: { fileUrl: '', fileName }, index: this.state.selectedIndex }] }
    };
  ...
  async fetchOnlyImages() {
    ...
    if (response) {
      ...
      const dataDetails = this.state.funderAreaOfInterest
  ...
  sendDataToParent(isSaveClicked) {
    ...
    const investment = this.state.funderAreaOfInterest;
    ...
    const arr = [];
    _.each(funderAreaOfInterest, (item) => {
    ...
    this.setState({ funderAreaOfInterest })
  ...
  `;
    ...
    const funderAreaOfInterestList = that.state.funderAreaOfInterestList || [];
    return (
      <div>
              ...
              >
                <div className="col-lg-12">
                  <div className="row">
                    ...
                    {/* list of interest */}
                    {funderAreaOfInterestList.map((details, idx) => (
