  ...
  >
    {
      clusters && clusters.length > 0 ? clusters
        .map(({ ...markerProps, id, numPoints }) => (
          numPoints === 1
            ? <MapMarkers
              key={id} lat={markerProps.lat} {...mapContext} module={module} hover={mapContext.hoverKey === markerProps.id}
...
const compareQueryOptions = function (a, b) {
...
getZoomFromMarkers = (zoom, mapContext, clickedOn) => {
  try {
    ...
    _.forEach(mapContext.data.data, (cluster, i) => {
      if (cluster.lat && cluster.lng && clickedOn.indexOf(cluster._id) > -1) {
        const pointLatLong = { lat: cluster.lat, lng: cluster.lng };
    ...
    const proposedZoom = fitBounds(bounds, size).zoom;
...
getBoundsFromPoints = (points) => {
  const boundObj = new google.maps.LatLngBounds();
...
const mapClusterHOC = compose(
  ...
  withState(
    ...
    configurations => ({ center: configurations.center, zoom: configurations.zoom })
  ),
  // describe events
  withHandlers({
    ...
    onChildClick: ({ setMapProps, mapContext }) => (evt, loc, zoom, centerPoint) => {
      ...
      const myzoom = { zoom: getZoomFromMarkers(zoom, mapContext, evt) };
  ...
  withPropsOnChange(
    ...
    ({ mapProps, getCluster }) => {
      ...
      if (mapProps.bounds) {
        ...
        mapBounds = clusterMap.map(({
          wx, wy, numPoints, points
        }) => {
          const allIds = points.map(point => (point._id)).join('_'); // to get zoom on click, this id is used in getZoomFromMarkers
          return {
            lat: parseFloat(wy),
            lng: parseFloat(wx),
            text: numPoints,
