...
class MlEditRole extends React.Component {
  ...
  async handleSuccess(response) {
    if (response) {
      if (response.success) { FlowRouter.go('/admin/settings/rolesList'); } else { toastr.error(response.result); }
    }
  }
  ...
  async updateRole() {
    const modules = _.cloneDeep(this.state.assignModulesToRoles);
    ...
    const uniqModule = _.uniqBy(modules, 'moduleId');
    ...
    const emptyCluster = _.filter(roleObject.assignRoles, ['cluster', ''])
    const emptyChapter = _.filter(roleObject.assignRoles, ['chapter', ''])
    const emptySubChapter = _.filter(roleObject.assignRoles, ['subChapter', ''])
    const emptyCommunity = _.filter(roleObject.assignRoles, ['community', ''])
    const emptyDepartment = _.filter(roleObject.assignRoles, ['department', ''])
    const emptySubDepartment = _.filter(roleObject.assignRoles, ['subDepartment', ''])
  ...
  onUserTypeSelect(val) {
    if (val) { this.setState({ selectedUserType: val.value }) } else {
  ...
  async findRole() {
    const roleid = this.props.config;
                    ...
                    <div className="form_bg">
                      <form>
                        <div className="form-group">
                          <input
                            type="text" ref="roleName" placeholder="Role Name"
                        ...
                        <div className="form-group">
                          <input
                            type="text" ref="diplayName" placeholder="Display Name"
                        ...
                        <div className="form-group">
                          <Select
                            name="form-field-name" ref="userType" options={UserTypeOptions}
                        ...
                        {this.state.data && this.state.data.assignRoles ? (
                          <MlAssignClustersToRoles
                            getassignRoleToClusters={this.getassignRoleToClusters.bind(this)}
                        ...
                        <div className="form-group switch_wrap inline_switch">
                          ...
                          <label className="switch">
                            <input
                              type="checkbox" ref="isActive" checked={this.state.isActive}
                        ...
                        <div className="form-group">
                          <div className="input_types">
                            <div className="input_types"><input
                              id="checkbox1" type="checkbox" ref="isNonMoolyaAvailable"
                    ...
                    >
                      <form style={{ marginTop: '0px' }}>
                        ...
                        {this.state.data && this.state.data.modules ? (
                          <MlAssignModulesToRoles
                            getassignModulesToRoles={this.getassignModulesToRoles.bind(this)}
