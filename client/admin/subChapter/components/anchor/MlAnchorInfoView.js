...
export default class MlAnchorInfoView extends React.Component {
  ...
  componentDidUpdate() {
    ...
    const dHeight = this.props.isAdmin ? 200 : 200
  ...
  async getAnchorUserDetails(id) {
    const response = await findBackendUserActionHandler(id);
  ...
  async getAnchorUsers() {
    ...
    const response = await findAnchorUserActionHandler({ clusterId, chapterId, subChapterId })
  ...
  changePath() {
    let queryParams = this.props.queryParams && this.props.queryParams.viewMode
    queryParams = JSON.parse(queryParams)
    if (this.props.isAdmin) { FlowRouter.go(`/admin/dashboard/${this.props.clusterId}/${this.props.chapterId}/${this.props.subChapterId}/` + `communities?viewMode=${queryParams}`) } else { FlowRouter.go(`/app/dashboard/${this.props.clusterId}/${this.props.chapterId}/${this.props.subChapterId}/` + `communities?viewMode=${queryParams}`) }
  }
  ...
  async registerAsClick() {
    ...
    if (this.isAllowRegisterAs) { this.toggle() } else { toastr.error('Please complete your pending registration') }
  }
  ...
  async submitRegisterAs() {
    const pickStates = pick(this.state, ['userName', 'firstName', 'lastName', 'contactNumber', 'email', 'registrationType', 'identityType', 'countryId'])
    const propsId = pick(this.props, ['clusterId', 'chapterId', 'subChapterId'])
    const finalRegData = _.extend(pickStates, propsId);
    const registrationId = this.state.registerId
    const response = await registerAsInfo(finalRegData, registrationId);
    if (response && response.success) {
      const reg = JSON.parse(response.result)
      toastr.success('User registered successfully');
      FlowRouter.go(`/app/register/${reg.registrationId}`);
    } else if (response && !response.success) { toastr.error(response.result); }
  }

  renderCommunityCount() {
    return this.state.data.portfolioCounter.map((value, say) => (<li key={say}>
      <a href="">
  ...
  render() {
    ...
    const chapterQuery = gql`query($id:String){data:fetchChapters(id:$id) {
        value:_id
        label:chapterName
      }  
    }`;
    const subChapterQuery = gql`query($id:String,$displayAllOption:Boolean){  
      data:fetchSubChaptersSelect(id:$id,displayAllOption:$displayAllOption) {
        value:_id
        label:subChapterName
      }  
    }`;
    const fetchCommunities = gql` query($isRegisterAs: Boolean){
        data:fetchCommunitiesFromDef(isRegisterAs:$isRegisterAs){label:name,value:code}
      } 
    `;
    const fetchIdentity = gql`query($communityId:String){
    ...
    const chapterOption = { options: { variables: { id: this.props.clusterId } } };
    const subChapterOption = { options: { variables: { id: this.props.chapterId, displayAllOption: false } } };
    const identityOptions = { options: { variables: { communityId: this.state.registrationType } } };
    const communityOptions = { options: { variables: { isRegisterAs: true } } }
    return (
          ...
          <div className="panel panel-default">
            ...
            <div className="panel-body nopadding">
              <div className="col-md-2">
                <img
                  src={this.state.subChapterImageLink} className="margintop"
                ...
                <ul className={'list-info'}>
                  {
                    this.state.contactDetails.length !== 0 && this.state.contactDetails.map((cd, index) => {
                      const {
                        emailId, buildingNumber, street, town, area, landmark, countryId, stateId, pincode, contactNumber
                      } = cd;
                      return (
                        <li key={index}>
        ...
        <Popover placement="top" isOpen={this.state.popoverOpen} target="default_target" toggle={this.toggle}>
          ...
          <PopoverContent>
            <div className="form-group">
              <Moolyaselect
                multiSelect={false} placeholder="Registration Type" className="form-control float-label"
            ...
            <div className="col-md-6 nopadding-left">
              <div className="form-group ">
                ...
                <input
                  type="text" ref="firstName" value={this.state.firstName} placeholder="First Name"
                  className="form-control float-label" disabled="true"/>
              </div>
              <div className="form-group ">
                <span className={`placeHolder ${lastNameActive}`}>Last Name</span>
                <input
                  type="text" ref="contactNumber" value={this.state.lastName} placeholder="Contact number"
            ...
            <div className="col-md-6 nopadding-right">
              <div className="form-group ">
                ...
                <input
                  type="text" ref="lastName" value={this.state.contactNumber} placeholder="Last Name"
                  className="form-control float-label" disabled="true"/>
              </div>
              <div className="form-group ">
                <span className={`placeHolder ${emailActive}`}>Email Id</span>
                <input
                  type="text" ref="email" value={this.state.email} placeholder="Email Id"
                  className="form-control float-label" disabled="true"/>
              </div>
              <div className="form-group">
                <Moolyaselect
            ...
            <div className="assign-popup">
              <a
                data-toggle="tooltip" title="Save" data-placement="top" className="hex_btn hex_btn_in"
                onClick={this.submitRegisterAs}>
                <span className="ml ml-save"></span>
              </a>
              <a
                data-toggle="tooltip" title="Cancel" data-placement="top" href="" className="hex_btn hex_btn_in"
