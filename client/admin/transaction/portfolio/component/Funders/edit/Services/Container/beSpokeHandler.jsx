...
export default class BeSpokeHandler extends Component {
  ...
  componentWillMount() {
    if (this.props.beSpokeDetails) {
      ...
      console.log(details)
      const mode = this.props.beSpokeDetails[this.props.beSpokeIndex].mode;
      if (mode === 'offline') { this.setState({ mode: true }) } else { this.setState({ mode: false }) }
    }
  }
  ...
  handleDuration() {
    ...
    details = _.extend(details, { duration });
    this.setState({ details })
  }
  ...
  async updateBeSpokeData(data) {
    if (data && this.isValidBeSpoke()) {
      const detailsData = _.cloneDeep(this.state.details);
      console.log('detailsData', detailsData)
      const temp = _.omit(detailsData.duration, '__typename');
      let attachTemp = [];
      if (detailsData.attachments && detailsData.attachments.length) {
        attachTemp = detailsData.attachments.map((info, index) => {
          info.fileUrl.map((images) => {
      ...
      const service = _.omit(detailsData, '__typename');
  ...
  async onFileUpload(e, index) {
    ...
    const file = e.target.files[0];// document.getElementById("fileinput").files[0];
    if (file) {
      const data = { moduleName: 'PROFILE', actionName: 'UPDATE', user }
      const response = await multipartASyncFormHandler(data, file, 'registration', this.onFileUploadCallBack.bind(this, index));
      return response;
    }
  }
  ...
  documentName(index, e) {
    const attach = this.state.attachmentDocs;
