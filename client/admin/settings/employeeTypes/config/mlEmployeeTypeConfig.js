...
const mlEmployeeTypeTableConfig = new MlViewer.View({
  ...
  columns: [
    ...
    {
      dataField: 'employmentTypeInfo.employmentName', title: 'Employee Type', dataSort: true, customComponent(data) { return <div>{data.data.employmentTypeInfo.employmentName}</div> }
    },
    {
      dataField: 'employmentTypeInfo.employmentDisplayName', title: 'Display Name', dataSort: true, customComponent(data) { return <div>{data.data.employmentTypeInfo.employmentDisplayName}</div> }
    },
  ...
  /*  graphQlQuery:gql`
              ...
              ` */
  queryOptions: true,
  buildQueryOptions: config => ({ context: { settingsType: 'EMPLOYMENTTYPE' } }),
  graphQlQuery: gql`
