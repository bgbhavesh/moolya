...
export default class MlFunderPrincipalTeamView extends React.Component {
  ...
  onSelectPrincipal(index, e) {
    _.each(this.state.funderPrincipalList[index], (value, field) => {
      if (typeof (field) === 'string') { $(`#${field}`).removeClass('fa-lock').addClass('un_lock fa-unlock') }
    })
    this.setState({ PIndex: index })

    _.each(this.state.funderPrincipalList[index].privateFields, (pf) => {
      $(`#${pf.booleanKey}`).removeClass('un_lock fa-unlock').addClass('fa-lock')
    })
  }
  onSelectTeam(index, e) {
    _.each(this.state.funderTeamList[index], (value, field) => {
      if (typeof (field) === 'string') { $(`#team${field}`).removeClass('fa-lock').addClass('un_lock fa-unlock') }
    })
    this.setState({ TIndex: index })

    _.each(this.state.funderTeamList[index].privateFields, (pf) => {
  ...
  render() {
    ...
    const PIndex = that.state.PIndex
    const selectedPrincipal = that.state.funderPrincipalList[PIndex] || {};
    const TIndex = that.state.TIndex
    const selectedTeam = that.state.funderTeamList[TIndex] || {};
                ...
                <div className="panel panel-default">
                  ...
                  <div className="panel-body">
                    <div className="col-lg-12 top-content ideators_list">
                      <div className="row">
                        {that.state.funderPrincipalList.map((principal, idx) => (
                            ...
                            {that.state.funderPrincipalList.map((principal, idx) => (
                              <li key={idx} onClick={that.onSelectPrincipal.bind(that, idx)}>
                                <div className="team-block">
                                  <img
                                    src={principal.logo ? generateAbsolutePath(principal.logo.fileUrl) : '/images/def_profile.png'}
                                    className="team_img"/>
                                  <h3>
                          ...
                          <ul>
                            {that.state.funderTeamList.map((team, idx) => (
                              <li onClick={that.onSelectTeam.bind(that, idx)} key={idx}>
                                <div className="team-block">
                                  <img
                                    src={team.logo ? generateAbsolutePath(team.logo.fileUrl) : '/images/def_profile.png'}
                                    className="team_img"/>
                                  <h3>
