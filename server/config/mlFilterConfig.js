if (Meteor.isServer) {
  // for registration
  const registrationFilterExists = MlFilters.findOne({ moduleName: 'registration' });
  if (!registrationFilterExists) {
    MlFilters.upsert({ moduleName: 'registration' }, {
      $set: {
  ...
  // for portfolio
  const portfolioFilterExists = MlFilters.findOne({ moduleName: 'portfolio' });
  if (!portfolioFilterExists) {
    MlFilters.upsert({ moduleName: 'portfolio' }, {
      $set: {
  ...
  // for transaction log
  const transactionLogFilterExists = MlFilters.findOne({ moduleName: 'transactionLog' });
  if (!transactionLogFilterExists) {
    MlFilters.upsert({ moduleName: 'transactionLog' }, {
      $set: {
  ...
  const internalRequestFilterExists = MlFilters.findOne({ moduleName: 'internalRequests' });
  if (!internalRequestFilterExists) {
    MlFilters.upsert({ moduleName: 'internalRequests' }, {
      $set: {
  ...
  const approvedRequestFilterExists = MlFilters.findOne({ moduleName: 'internalApprovedRequests' });
  if (!approvedRequestFilterExists) {
    MlFilters.upsert({ moduleName: 'internalApprovedRequests' }, {
      $set: {
  ...
  const processDocumentsFilterExists = MlFilters.findOne({ moduleName: 'documents' });
  if (!processDocumentsFilterExists) {
    MlFilters.upsert({ moduleName: 'documents' }, {
      $set: {
  ...
  const templateAssignmentFilterExists = MlFilters.findOne({ moduleName: 'templateAssignment' });
  if (!templateAssignmentFilterExists) {
    MlFilters.upsert({ moduleName: 'templateAssignment' }, {
      $set: {
  ...
  // for office
  const officeFilterExists = MlFilters.findOne({ moduleName: 'office' });
  if (!officeFilterExists) {
    MlFilters.upsert({ moduleName: 'office' }, {
      $set: {
        filterName: 'Office',
...
*/
  ...
  const processSetupFilterExists = MlFilters.findOne({ moduleName: 'processSetup' });
  if (!processSetupFilterExists) {
    MlFilters.upsert({ moduleName: 'processSetup' }, {
      $set: {
  ...
  // for share
  const shareFilterExists = MlFilters.findOne({ moduleName: 'share' });
  if (!shareFilterExists) {
    MlFilters.upsert({ moduleName: 'share' }, {
      $set: {
  ...
  // for external users
  const usersFilterExists = MlFilters.findOne({ moduleName: 'users' });
  if (!usersFilterExists) {
    MlFilters.upsert({ moduleName: 'users' }, {
      $set: {
        filterName: 'Users',
  ...
  // for history
  const auditFilterExists = MlFilters.findOne({ moduleName: 'audit' });
  if (!auditFilterExists) {
    MlFilters.upsert({ moduleName: 'audit' }, {
      $set: {
        filterName: 'History',
  ...
  // for service cards
  const serviceCardsFilterExists = MlFilters.findOne({ moduleName: 'serviceCards' });
  if (!serviceCardsFilterExists) {
    MlFilters.upsert({ moduleName: 'serviceCards' }, {
      $set: {
  ...
  // for service cards
  const appointmentsFilterExists = MlFilters.findOne({ moduleName: 'appointments' });
  if (!appointmentsFilterExists) {
    MlFilters.upsert({ moduleName: 'appointments' }, {
      $set: {
