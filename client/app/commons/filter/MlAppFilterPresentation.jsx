        ...
        {filters.map((filter) => {
          ...
          switch (filter.type) {
            case 'INPUT':
              return (
                <div className="col-lg-3">
                  <input type="text" ref="input" placeholder={filter.displayName} onChange={evt => props.updateFilterQuery(evt.target.value, filter.field) } className="form-control float-label" id="" disabled={filter.isRestrictedFilter}/>
                </div>
              );
              break;
            case 'RADIO':
              ...
              break;
            case 'SELECT':
              const selected = selectedData[filter.field] ? selectedData[filter.field] : '';
              return (
                <div className="col-lg-3">
                  <Moolyaselect
                    ...
                    onSelect={value => props.updateFilterQuery(value, filter.field)}
                    isDynamic={true}
            ...
            case 'CHECKBOX':
              ...
              break;
            case 'MULTICHECKBOX':
              return filter.options.map(option => <input type="checkbox"/>);
              break;
            default:
