...
export default class MlFunderAbout extends React.Component {
  ...
  onBudgetClick(fieldName, field, e) {
    ...
    const privateKey = {
      keyName: fieldName, booleanKey: field, isPrivate, objectName: 'investmentBudget', tabName: 'funderAbout'
    }
  ...
  async fetchPortfolioDetails() {
    ...
    const empty = _.isEmpty(that.context.funderPortfolio && that.context.funderPortfolio.funderAbout)
    if (empty) {
      if (response) {
  ...
  onLogoFileUpload(image) {
    const file = image;
    const data = {
      moduleName: 'PORTFOLIO', actionName: 'UPLOAD', portfolioDetailsId: this.props.portfolioDetailsId, portfolio: { funderAbout: { profilePic: '' } }
    };
  ...
  render() {
    ...
    const genderImage = this.state.data && this.state.data.gender === 'female' ? '/images/female.jpg' : this.state.defaultProfilePic;
    const investmentFrom = this.state.data && this.state.data.investmentFrom ? this.state.data.investmentFrom : '';
    let personal = null,
      familyFund = null;
