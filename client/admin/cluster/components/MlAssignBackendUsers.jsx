...
class MlAssignBackendUsers extends React.Component {
  ...
  async find_Cluster_Roles(userId) {
    ...
    if (userRoles) { roles = userRoles || []; }
    this.setState({
      loading: false, user_Roles: roles, selectedBackendUser: userId, mlroleDetails: roles
    });
    return roles
  }
  ...
  render() {
    ...
    const query = gql`query($clusterId:String, $chapterId:String, $subChapterId:String, $communityId:String){data:fetchUsersByClusterDepSubDep(clusterId:$clusterId, chapterId:$chapterId, subChapterId:$subChapterId, communityId:$communityId){label:username,value:_id}}`;
    const userid = this.state.selectedBackendUser || '';
    const userDisplayName = this.state.userDisplayName || '';
    const username = this.state.username || '';
    const deActive = that.state.deActive;
    let contextHeader = '';
    ...
    let imageUrl = '';
    if (this.state.profilePic) { imageUrl = urlCreator.createObjectURL(this.state.profilePic); }
    return (
                ...
                <div className="left_wrap left_user_blocks">
                  <ScrollArea speed={0.8} className="left_wrap">
                    ...
                    <MlAssignBackendUserList
                      clusterId={that.props.params.clusterId} chapterId={that.props.params.chapterId}
              ...
              <div className="left_wrap">
                <ScrollArea speed={0.8} className="left_wrap">
                  ...
                  <form>
                    ...
                    <div className="form-group">
                      <input
                        type="text" placeholder="Display Name" ref="displayName" readOnly="true" defaultValue={userDisplayName}
                        className="form-control float-label" id="dName"/>
                    </div>
                    <div className="form-group">
                      <input
                        type="text" readOnly="true" placeholder="User Name" className="form-control float-label"
                        id="userName" ref="userName" defaultValue={username}/>
                    </div>

                    {userid ? (<MlAssignBackednUserRoles
                      userId={userid} clusterId={that.props.params.clusterId} chapterId={that.props.params.chapterId} subChapterId={that.props.params.subChapterId} communityId={that.props.params.communityId}
