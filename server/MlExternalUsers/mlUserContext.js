...
class MlUserContext {
  ...
  userProfileDetailsByProfileId(profileId) {
    ...
    const user = Meteor.users.findOne({ $and: [{ 'profile.isExternaluser': true }, { 'profile.externalUserProfiles': { $elemMatch: { profileId } } }] }) || {}
    if (user && user.profile && user.profile.externalUserProfiles) {
      const user_profiles = user.profile.externalUserProfiles;
      user_Profile = _.find(user_profiles, { profileId });
      if (user_Profile) {
  ...
  getDefaultMenu(userId) {
    ...
    if (userProfile && userProfile.communityDefCode) {
      ...
      if (registration) { menu = MlAppMenuConfig.findOne({ $and: [{ isProfileMenu: false }, { communityCode: userProfile.communityDefCode }, { isActive: true }] }); } else { menu = MlAppMenuConfig.findOne({ $and: [{ isProfileMenu: false }, { communityCode: 'BRW' }, { isActive: true }] }); }
    } else {
  ...
  getDefaultProfileMenu(userId) {
    ...
    if (userProfile && userProfile.communityDefCode) {
      const registration = this.isUserRegistrationApproved(userProfile);
      if (registration) { menu = MlAppMenuConfig.findOne({ $and: [{ isProfileMenu: true }, { communityCode: userProfile.communityDefCode }, { isActive: true }] }); } else { menu = MlAppMenuConfig.findOne({ $and: [{ isProfileMenu: true }, { communityCode: 'BRW' }, { isActive: true }] }); }
    } else {
      // commmunity type browser will not have any profile
  ...
  currencyTypeForEndUsers(userDetails) {
    ...
    if (profileId !== 'all') {
      const userProfiles = userInfo.profile.externalUserProfiles;
      userProfiles.forEach((defaultProfile) => {
        if (defaultProfile.profileId === profileId) { profileClusterId = defaultProfile.clusterId; }
      })
      return profileClusterId;
    } return this.userProfileDetails(userId).clusterId;
  }
}
