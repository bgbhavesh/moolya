...
export default class AddressDetails extends React.Component {
  ...
  updateOptions(index, did, selectedValue, selObject, callback) {
    ...
    if (index !== -1) {
      ...
      const updatedComment = update(this.state.addressDetails[index], {
      ...
      const newData = update(this.state.addressDetails, {
  ...
  countryUpdateOptions(index, did, selectedValue, selObject, callback) {
    if (index !== -1) {
      ...
      this.setState({
        addressDetails: newData, countrySelectedValue: selObject.label, countryId: did, countryCode: selObject.code
      });
    }
  }
  ...
  async addressTabSelected(index, value) {
    ...
    const countryValue = this.state.countryId ? this.state.countryId : this.state.addressDetails[index].addressCountryId;
  ...
  async onDeleteAddress(index, value) {
    const listArray = update(this.state.addressDetails, {
      $splice: [[index, 1]]
    });
  ...
  isValidate() {
    ...
    const newArray = this.state.addressDetails || []
  ...
  async onSavingAddress(index, value) {
    ...
    const ret = mlFieldValidations(refs)


    const clusterDetails = await findClusterTypeActionHandler(this.props.clusterId)
    const clusterCountry = clusterDetails && clusterDetails.countryName ? clusterDetails.countryName : null;
    const addressSelectedCountry = this.state.countrySelectedValue ? this.state.countrySelectedValue : null;
    const isDeafaultChecked = this.refs.defaultAddress.checked;
    const countrycode = this.state.countryCode ? this.state.countryCode : null;
    const contactNumber = this.refs.phoneNumber && this.refs.phoneNumber.value;
    const isValidPhoneNumber = validatedPhoneNumber(countrycode, contactNumber);
    if (ret) {
      toastr.error(ret);
      ...
      const addressDetailsObject = this.state.addressInformation;
      ...
      if (response) {
        ...
        if (!response.success) {
          ...
          this.setState({
            selectedValue: '', selectedAddressLabel: '', countrySelectedValue: '', selectedStateValue: '', countryId: '', stateId: ''
          });
  ...
  async onEditAddress(index, value) {
    ...
    if (index !== -1) {
      // do your stuff here
      const registrationDetails = this.props.registrationInfo.addressInfo
      const dbData = _underscore.pluck(registrationDetails, 'addressType') || [];
      ...
      const clusterDetails = await findClusterTypeActionHandler(this.props.clusterId)
      const clusterCountry = clusterDetails && clusterDetails.countryName ? clusterDetails.countryName : null;
      const addressSelectedCountry = this.state.countrySelectedValue ? this.state.countrySelectedValue : this.state.addressDetails[index].addressCountry;

      const isDeafaultChecked = this.refs[`defaultAddress${index}`].checked;
      ...
      if (contactExist) {
        ...
        const ret = mlFieldValidations(refs)
        const countrycode = this.state.countryCode ? this.state.countryCode : null;
        const contactNumber = this.refs[`phoneNumber${index}`] && this.refs[`phoneNumber${index}`].value;
        const isValidPhoneNumber = validatedPhoneNumber(countrycode, contactNumber);
        if (ret) {
          toastr.error(ret);
        } else if (clusterCountry && addressSelectedCountry && isDeafaultChecked && clusterCountry != addressSelectedCountry) {
          ...
          const labelValue = this.state.selectedAddressLabel ? this.state.selectedAddressLabel : this.state.addressDetails[index].addressTypeName;
          const valueSelected = this.state.selectedValue ? this.state.selectedValue : this.state.addressDetails[index].addressType;
          const countryLabelValue = this.state.countrySelectedValue ? this.state.countrySelectedValue : this.state.addressDetails[index].addressCountry;
          const countryIDValue = this.state.countryId ? this.state.countryId : this.state.addressDetails[index].addressCountryId;
          const stateLabelValue = this.state.selectedStateValue ? this.state.selectedStateValue : this.state.addressDetails[index].addressState;
          const stateIDValue = this.state.stateId ? this.state.stateId : this.state.addressDetails[index].addressStateId;
          const updatedComment = update(this.state.addressDetails[index], {
  ...
  render() {
    const that = this;
    const addressTypeQuery = gql`query($type:String,$hierarchyRefId:String){
    ...
    const statesQuery = gql`query ($countryId: String) {
    ...
    const addressTypeOption = { options: { variables: { type: 'ADDRESSTYPE', hierarchyRefId: this.props.clusterId } } };
    const statesOption = { options: { variables: { countryId: this.state.countryId } } };
      ...
      <div className="panel-body">
        ...
        <div className="ml_tabs">
          <ul className="nav nav-pills">
            ...
            {that.state.addressDetails && (that.state.addressDetails.map((options, key) => (
          ...
          <div className="tab-content clearfix">
            ...
            <div className={`tab-pane${this.state.activeTab}`} id="1a">
              ...
              <div className="form-group">
                <Moolyaselect
                  multiSelect={false} ref={'addressCountry'}
              ...
              <div className="form-group">
                <Moolyaselect
                  multiSelect={false} ref={'addressState'}
              ...
              <div className="form-group mandatory">
                <input
                  type="text" ref={'addressPinCode'} placeholder="Pincode" name ={'addressPinCode'}
            ...
            {that.state.addressDetails && (that.state.addressDetails.map((options, key) => (
              <div className="tab-pane" id={`adressType${key}`} key={key}>
                <div className="form-group">
                  <Moolyaselect
                    multiSelect={false} ref={`address${key}`}
                ...
                <div className="form-group mandatory">
                  <input
                    type="text" name ={'name'} ref={`name${key}`} placeholder="Name"
                    className="form-control float-label" id="" defaultValue={options.name} data-required={true} data-errMsg="Name is required"/>
                </div>
                <div className="form-group mandatory">
                  <input
                    type="text" ref={`addressFlat${key}`} name ={'addressFlat'} className="form-control float-label"
                ...
                <div className="form-group mandatory">
                  <input
                    type="text" ref={`addressLocality${key}`} name ={'addressLocality'}
                ...
                <div className="form-group">
                  <input
                    type="text" ref={`addressLandmark${key}`} placeholder="Landmark" name ={'addressLandmark'}
                    className="form-control float-label" id="" defaultValue={options.addressLandmark}/>
                </div>
                <div className="form-group">
                  <input
                    type="text" ref={`addressArea${key}`} placeholder="Area" name ={'addressArea'}
                ...
                <div className="form-group">
                  <Moolyaselect
                    multiSelect={false} ref={`addressCountry${key}`} className="form-control float-label" valueKey={'value'}
                ...
                <Moolyaselect
                  multiSelect={false} ref={`addressState${key}`}
                  placeholder="Select State" mandatory={true}
                ...
                <div className="form-group mandatory">
                  <input
                    type="text" ref={`addressCity${key}`} placeholder="Town/City" name ={'addressCity'}
                ...
                <div className="form-group mandatory">
                  <input
                    type="text" name ={'phoneNumber'} ref={`phoneNumber${key}`} placeholder="Phone Number"
                ...
                <div className="form-group mandatory">
                  <input
                    type="text" ref={`addressPinCode${key}`} placeholder="Pincode" name ={'addressPinCode'}
                ...
                <div className="form-group switch_wrap inline_switch">
                  ...
                  <label className="switch">
                    <input
                      type="checkbox" ref={`defaultAddress${key}`}
                ...
                <div className="ml_icon_btn">
                  ...
                  <a
                    href="#" onClick={that.onEditAddress.bind(that, key)}
