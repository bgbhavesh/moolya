...
MlResolver.MlQueryResolver.fetchInternalTaskById = (obj, args, context, info) => {
  ...
  if (args.internalTaskId) {
    ...
    const attendees = internalTask.attendees.map(attendee => attendee.userId);
    internalTask.userInfo = mlDBController.find('users', { _id: { $in: attendees } }).fetch().map(user => ({
      id: user._id,
  ...
  const code = 400;
  const response = new MlRespPayload().errorPayload('Internal task id is required', code);
  return response;
};
...
MlResolver.MlMutationResolver.createInternalTask = (obj, args, context, info) => {
  if (args.internalTask) {
    if (args.internalTask.attendees && args.internalTask.attendees.length) {
      ...
      args.internalTask.attendees.forEach((attendee) => {
        const dataToInsert = args.internalTask;
      ...
      const code = 200;
      const response = new MlRespPayload().successPayload('Internal task inserted', code);
      return response;
    }
    const code = 400;
    const response = new MlRespPayload().errorPayload('Internal task attendees is required', code);
    return response;
  }
  const code = 400;
  const response = new MlRespPayload().errorPayload('Internal task data is required', code);
  return response;
};
...
MlResolver.MlMutationResolver.createSelfInternalTask = (obj, args, context, info) => {
  ...
  if (args.selfInternalTask) {
    const users = args.selfInternalTask.users;
    const dataToInsert = args.selfInternalTask;
    dataToInsert.userId = context.userId;
    ...
    if (users && users.length) {
      ...
      users.forEach((user) => {
        ...
        dataToInsert.isSelfAssigned = false;
        const result = mlDBController.insert('MlInternalTask', dataToInsert, context);
      });
      const code = 200;
      const response = new MlRespPayload().successPayload('Internal task created', code);
      return response;
    }
...
MlResolver.MlMutationResolver.updateInternalTask = (obj, args, context, info) => {
  if (args.internalTaskId) {
    if (args.internalTask) {
      const resp = mlDBController.update('MlInternalTask', { _id: args.internalTaskId }, args.internalTask, { $set: 1 }, context);
      if (resp) {
        const code = 200;
        const response = new MlRespPayload().successPayload('Internal task updated', code);
        return response;
      }
    } else {
      const code = 400;
