...
export default class MlAppEditOffice extends React.Component {
  ...
  async getMyOfficeRole() {
    ...
    const response = await getMyOfficeRoleActionHandler(officeId);
    if (response.success) {
      this.setState({
  ...
  async officeDetails() {
    ...
    if (response && response.success) {
      ...
      const obj = data[0];
      const availableCommunities = obj && obj.office && obj.office.availableCommunities && obj.office.availableCommunities.length ? obj.office.availableCommunities : [];
      ...
      this.setState({ loading: false, office: obj.office, availableCommunities })
    } else {
  ...
  render() {
    const that = this;
    const noAccessMessage = <p className="text-danger">No access for team member. Only principle can have access to this tab.</p>
