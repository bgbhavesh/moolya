...
*/
  ...
  componentWillMount() {
    ...
    const isOFB = _.isMatch(details, { registrationType: 'OFB' });
    this.setState({
      loading: false,
  ...
  componentDidMount() {
    ...
    const isVerifiedUser = _.find(this.props.emailDetails, { verified: true })
  ...
  optionBySelectRegistrationType(value, calback, selObject) {
    ...
    this.setState({
      registrationType: value, identityType: null, coummunityName: selObject.label, cluster: null, chapter: null, subchapter: null, userType: null
    });
  }
  ...
  isUpdated() {
    const existingObject = this.props.registrationInfo || {}
    const oldObject = {
    ...
    const differences = diff(existingObject, newObject);
  ...
  render() {
    let MlActionConfig
    const registrationDate = this.state.registrationDate
    const hours = moment().diff(registrationDate, 'hours')

    const showReject = !!((this.props.emailDetails && this.props.emailDetails.length > 0 && !this.props.emailDetails[0].verified))
...
`;
    ...
    const subChapterOption = { options: { variables: { id: this.state.chapter } } }
