...
MlResolver.MlQueryResolver.SearchQuery = (obj, args, context, info) => {
  ...
  if (args.sortData) {
    const sortObj = getQuery.sortFunction(args);
  ...
  if (args.module == 'department') {
    const userProfileDep = new MlAdminUserContext().userProfileDetails(context.userId);
    var queryChange;
    ...
    data.map((doc, index) => {
      let clusterIds = [];
      const chapterIdsArray = [];
      const subchapterIdsArray = [];

      const departments = doc && doc.depatmentAvailable ? doc.depatmentAvailable : [];
      departments.map((department) => {
        const currentDepClusterIds = department && department.cluster && department.cluster ? department.cluster : [];
        clusterIds = clusterIds.concat(currentDepClusterIds);

        const chapterId = department && department.chapter && department.chapter ? department.chapter : '';
        if (chapterId != 'all') {
          chapterIdsArray.push(chapterId);
        }
        const subchapterId = department && department.subChapter && department.subChapter ? department.subChapter : '';
        if (subchapterId != 'all') {
      ...
      const clusterNames = _.pluck(clusterData, 'clusterName') || [];
      const chapterNamesArray = _.pluck(chapterData, 'chapterName') || [];
      const subchapterNamesArray = _.pluck(subchapterData, 'subChapterName') || [];
  ...
  if (args.module == 'subDepartment') {
    const userProfileSub = new MlAdminUserContext().userProfileDetails(context.userId);
    var queryChange;
    ...
    data.map((doc, index) => {
      ...
      const chapterIdsArray = [];
      const subchapterIdsArray = [];
      const departments = doc && doc.subDepatmentAvailable ? doc.subDepatmentAvailable : [];
  ...
  if (args.module == 'states') {
    const pipeline = [
      ...
      {
        $lookup: {
          from: 'mlStates', localField: 'countryCode', foreignField: 'countryCode', as: 'states'
        }
      },
      { $unwind: '$states' },
      {
    ...
    pipeline.push({
      $project: {
        count: 1, _id: '$data._id', countryName: '$data.countryName', name: '$data.name', displayName: '$data.displayName', countryId: '$data.countryId', countryCode: '$data.countryCode', isActive: '$data.isActive'
      }
    });
    if (findOptions.sort) {
  ...
  if (args.module == 'cities') {
    const countries = MlCountries.find({ isActive: true }).fetch();
    const states = MlStates.find({ isActive: true }).fetch();
    const allIds = _.pluck(states, '_id');
    const cities = MlCities.find({ $and: [{ stateId: { $in: allIds } }, query] }, findOptions).fetch();
    _.each(cities, (item, key) => {
      _.each(states, (s, v) => {
        if (item.stateId == s._id) { item.stateId = s.name; }
      })
      _.each(countries, (s, v) => {
        if (item.countryId == s._id) { item.countryCode = s.country; }
      })
    })
    data = cities;
  ...
  if (args.module == 'documentMapping') {
    ...
    data.map((doc, index) => {
      ...
      const chapterIds = [];
      const subChapterIds = [];
      const kycCategoryIds = [];
      const allowableFormatIds = [];
  ...
  if (args.module == 'Users') {
    ...
    var queryChange = {}
    const dataContext = MlSubChapterAccessControl.getAccessControl('SEARCH', context, null)
    ...
    data.map((doc, index) => {
      let hirarichyLevel = []
      const roleNames = []
      const userProfiles = doc && doc.profile.InternalUprofile.moolyaProfile.userProfiles ? doc.profile.InternalUprofile.moolyaProfile.userProfiles : [];
  ...
  if (args.module == 'roles') {
    const userProfileMenu = new MlAdminUserContext().userProfileDetails(context.userId);
    var queryChange;
    ...
    data.map((doc, index) => {
      const departmentsIdsArray = [];
      const subdepartmentsIdsArray = [];
      const clusterIdsArray = [];
      const chapterIdsArray = [];
      const subchapterIdsArray = [];

      const assignRolesData = doc && doc.assignRoles ? doc.assignRoles : [];
      assignRolesData.map((doc) => {
  ...
  if (args.module == 'processmapping') {
    ...
    data.map((doc, index) => {
      ...
      const communityIds = doc.communities || []
      const communityData = MlCommunityDefinition.find({ code: { $in: communityIds } }).fetch() || [];
      const communityNames = _lodash.map(communityData, 'name') || []

      const industryIds = doc.industries || []
      const industryData = MlIndustries.find({ _id: { $in: industryIds } }).fetch() || [];
      const industryNames = _lodash.map(industryData, 'industryName') || []

      const professionIds = doc.professions || []
      const professionData = MlProfessions.find({ _id: { $in: professionIds } }).fetch() || [];
      const professionNames = _lodash.map(professionData, 'professionName') || []

      const clusterIds = doc.clusters || []
      const clusterData = MlClusters.find({ _id: { $in: clusterIds } }).fetch() || [];
      const clusterNames = _lodash.map(clusterData, 'clusterName') || []

      const stateIds = doc.states
      const stateData = MlStates.find({ _id: { $in: stateIds } }).fetch() || [];
      const stateNames = _lodash.map(stateData, 'name') || []

      const chapterIds = doc.chapters
      const chapterData = MlChapters.find({ _id: { $in: chapterIds } }).fetch() || [];
      const chapterNames = _lodash.map(chapterData, 'chapterName') || []

      const userTypeIds = doc.userTypes
      let userTypeNamesString = ''
      ...
      const subChaptersIds = doc.subChapters
      let subChapterNamesString = ''
  ...
  if (args.module == 'documents') {
    ...
    if (_.isEmpty(query)) {
      ...
      let seen = false;
      for (let j = 0; j != filterQuery.length; ++j) {
  ...
  if (args.module == 'hierarchy') {
    ...
    data.map((doc, index) => {
      const entry = {
        _id: doc._id, level: doc.level, module: doc.module, role: doc.role
      }
      list.push(entry)
    })
    data = list
  ...
  if (args.module == 'FunderPortfolio') {
    const value = mlDBController.find('MlPortfolioDetails', { status: 'PORT_LIVE_NOW', communityCode: 'FUN' }, context).fetch() // making dependency of funders on portfolio status
    const portId = _lodash.map(value, '_id')
    const finalQuery = mergeQueries(query, { portfolioDetailsId: { $in: portId } });
  ...
  if (args.module == 'SubDomain') {
    const domain = MlSubDomain.find(query, findOptions).fetch();
    const modArray = []
    _.each(domain, (item) => {
      const industryName = MlIndustries.findOne({ _id: item.industryId }).industryName;
      const obj = item;
  ...
  if (args.module == 'officeTransaction') {
    const pipeline = [
      ...
      {
        $lookup: {
          from: 'mlTransactionsLog', localField: 'officeId', foreignField: 'docId', as: 'transactionLog'
        }
      },
  ...
  if (args.module == 'userTransaction') {
    const pipeline = [
      ...
      {
        $lookup: {
          from: 'mlOfficeTransaction', localField: '_id', foreignField: 'userId', as: 'office'
        }
      },
      {
        $lookup: {
    ...
    data = data.map((doc) => {
      ...
      if (transactionsTypeCheck.indexOf(transactionType) >= 0) {
        ...
        const activityDocId = doc.activityDocId;
        switch (activity) {
          case 'connection':
            const status = ['Pending', 'Accepted', 'Declined', 'Blocked'];
            const data = mlDBController.findOne('MlConnections', activityDocId);
            doc.transactionId = data && data.transactionId ? data.transactionId : doc.transactionId;
            doc.status = status[data.status];
            break;
          case 'onBoard':
            const stage = mlDBController.findOne('MlStage', activityDocId);
