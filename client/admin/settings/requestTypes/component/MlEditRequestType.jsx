...
class MlEditRequestType extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      loading: true, data: {}, transactionType: ' ', transactionId: ' '
    };
  ...
  async handleSuccess(response) {
    if (response) {
      if (response.success) { FlowRouter.go('/admin/settings/requestTypeList'); } else { toastr.error(response.result); }
    }
  }
  async findRequestType() {
    const requestTypeId = this.props.config;
  ...
  render() {
    ...
    const transactionType = gql`query  {
