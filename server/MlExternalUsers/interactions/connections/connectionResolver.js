...
MlResolver.MlQueryResolver.fetchConnectionByTransaction = (obj, args, context, info) => {
  try {
    ...
    const connectionId = mlInteractionService.fetchConnectionByTransaction(args.transactionId, 'connectionRequest') || {};
    const connection = mlDBController.findOne('MlConnections', { _id: connectionId }, context);
    const requestedConnection = mlDBController.findOne('MlConnections', {
      _id: connection._id, isAccepted: false, isBlocked: false, isDenied: false, requestedFrom: { $ne: context.userId }
    }, context);
...
MlResolver.MlMutationResolver.connectionRequest = (obj, args, context, info) => {
  try {
    ...
    const isAlreadyExist = mlDBController.findOne('MlConnections', { connectionCode }, context);
    ...
    if (!isAlreadyExist) {
      ...
      const connectRequest = MlAlertNotification.onConnectionRequestSent(toUser._id);
    ...
    resp = mlDBController.update(
      'MlConnections', {
        connectionCode, isBlocked: false, isAccepted: false, $or: [{ isDenied: false, requestedFrom: { $type: 10 } }, { isDenied: true, requestedFrom: { $ne: fromuser._id } }]
      },
      {
    ...
    if (resp === 1) {
      // //create the transaction record and log
      const fromUserType = 'user';
      const connectionRec = mlDBController.findOne('MlConnections', { connectionCode }, context) || {};
  ...
  const code = 200;
  const response = new MlRespPayload().errorPayload('connection request failed', code);
  return response;
}
...
MlResolver.MlMutationResolver.acceptConnection = (obj, args, context, info) => {
  try {
    ...
    const connection = mlDBController.findOne('MlConnections', { _id: connectionId, 'users.userId': context.userId }, context) || {};
    const contextUser = mlInteractionService.getUserDetails(context.userId);
    ...
    const result = mlDBController.update(
      ...
      {
        isAccepted: true, isBlocked: false, isDenied: false, updatedBy: contextUser.username, updatedAt: new Date(), actionUserId: contextUser._id, status: 1
      }, // update doc
      { $set: true }, context
    );
    ...
    if (result) {
      const fromUser = connection && connection.users && connection.users.length > 0 && connection.users[0] && connection.users[0].userId ? connection.users[0].userId : ''
...
MlResolver.MlMutationResolver.rejectConnection = (obj, args, context, info) => {
  try {
    ...
    const result = mlDBController.update(
      ...
      {
        isAccepted: false, isBlocked: false, isDenied: true, updatedBy: contextUser.username, updatedAt: new Date(), actionUserId: contextUser._id, status: 2
      }, // update doc
      { $set: true }, context
    );
    ...
    if (result) {
      const fromUser = connection && connection.users && connection.users.length > 0 && connection.users[0] && connection.users[0].userId ? connection.users[0].userId : ''
      const toUser = connection && connection.users && connection.users.length > 0 && connection.users[1] && connection.users[1].userId ? connection.users[1].userId : ''
...
MlResolver.MlQueryResolver.fetchConnectionsByReg = (obj, args, context, info) => {
  ...
  if (args && args.registrationId && args.communityCode) {
    const query = [
      ...
      {
        $lookup: {
          from: 'mlConnections', localField: 'registrationInfo.userId', foreignField: 'users.userId', as: 'connections'
        }
      },
      { $unwind: '$connections' },
      ...
      {
        $project: {
          isEqual: { $cmp: ['$connections.users.userId', '$registrationInfo.userId'] },
          connections: 1,
          userId: 1
        }
      },
...
MlResolver.MlQueryResolver.fetchConnectionByUser = (obj, args, context, info) => {
  ...
  const query = [
    ...
    {
      $lookup: {
        from: 'users', localField: 'users.userId', foreignField: '_id', as: 'users'
      }
    },
    { $unwind: '$users' },
