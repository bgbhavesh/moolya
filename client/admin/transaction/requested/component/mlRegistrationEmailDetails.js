...
export default class EmailDetails extends React.Component {
  ...
  isValidate() {
    ...
    const newArray = this.state.emailDetails || []
  ...
  async onSavingEmailDetails(index, value) {
    ...
    if (ret) {
      ...
      const emailList = this.state.emailDetailsObject;
  ...
  async onUpdatingEmailDetails(index, value) {
    ...
    if (index !== -1) {
      // do your stuff here
      const registrationDetails = this.props.registrationInfo.emailInfo
      const dbData = _underscore.pluck(registrationDetails, 'emailIdType') || [];
      ...
      if (contactExist) {
        ...
        const updatedEmailId = this.refs[`emailId${index}`].value
        const isValidEmail = validatedEmailId(updatedEmailId);
        if (ret) {
          toastr.error(ret);
        } else if (updatedEmailId && !isValidEmail) {
          toastr.error('Please enter a valid email-Id');
        } else {
          const labelValue = this.state.selectedEmailTypeLabel ? this.state.selectedEmailTypeLabel : this.state.emailDetails[index].emailIdTypeName;
          const valueSelected = this.state.selectedEmailTypeValue ? this.state.selectedEmailTypeValue : this.state.emailDetails[index].emailIdType;
          const updatedComment = update(
            this.state.emailDetails[index],
            {
          ...
          const newData = update(this.state.emailDetails, {
  ...
  updateEmailOptions(index, did, selectedValue, selObject, callback) {
    ...
    if (index !== -1) {
      // do your stuff here
      const updatedComment = update(this.state.emailDetails[index], { emailIdTypeName: { $set: selObject.label }, emailIdType: { $set: did }, emailId: { $set: this.refs[`emailId${index}`].value } });
  ...
  async onDeleteEmail(index, value) {
    const listArray = update(this.state.emailDetails, {
      $splice: [[index, 1]]
    });
    const detailsType = 'EMAILTYPE';
    const registerid = this.props.registerId;
    ...
    return (<div className="panel-body">
      ...
      <div className="ml_tabs">
        <ul className="nav nav-pills">
          ...
          {that.state.emailDetails && (that.state.emailDetails.map((options, key) => (
        ...
        <div className="tab-content clearfix">
          <div className={`tab-pane${this.state.activeTab}`} id="emailA">
            <div className="form-group">
              <Moolyaselect
                multiSelect={false} ref={'emailType'}
          ...
          {that.state.emailDetails && (that.state.emailDetails.map((options, key) => (<div className="tab-pane" id={`emailIdType${key}`} key={key}>
            <div className="form-group">
              <Moolyaselect
                multiSelect={false} ref={`emailIdType${key}`}
                placeholder="Select Email Type" mandatory={true}
