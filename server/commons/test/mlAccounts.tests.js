...
describe('mlAccounts Module', () => {
  ...
  it('should fail to send verification email if user email address is not provided', () => {
    const address = false;
    const user = {
      _id: 'abcdefgh1234', email: 'rakesh.naik@raksan.in', emails: ['rakesh.naik@raksan.in'], password: 'Welcome123$%', profile: { name: 'rakesh.naik' }
    };
  ...
  it('should fail to send verification email if matching user is not found', () => {
    const address = 'mohammed.mohasin@raksan.in';
    const user = {
      _id: 'abcdefgh1234', email: 'rakesh.naik@raksan.in', emails: [{ address: 'rakesh.naik@raksan.in' }], password: 'Welcome123$%', profile: { name: 'rakesh.naik' }
    };
  ...
  it('should send customized verification email', () => {
    const user = {
      _id: 'abcdefgh1234', email: 'rakesh.naik@raksan.in', emails: [{ address: 'rakesh.naik@raksan.in' }], password: 'Welcome123$%', profile: { name: 'rakesh.naik' }
    };
    mldbfoStub.returns(user);
    ...
    const address = 'rakesh.naik@raksan.in';
    const tokenRecord = {
      token: Random.secret(),
      address,
      when: new Date()
    ...
    const customEmailComponent = function (user, verificationLink) {
      return 'This is a tests email message';
    }

    const mlRegStub = stubs.create('mlRegUpdate', MlRegistration, 'update');

    //
    // For contentType as HTML
    //
    const emailOptionsContentTypeHtml = { context: {}, emailContentType: 'html' };
  ...
  it('should send packaged verification email', () => {
  });
  it('should send verification email of type html', () => {
  });
  it('should send verification email of type json', () => {
  });
  it('should throw verification email link expired error for expired tokens', () => {
  });
  it('should throw verification email link error for unknown address', () => {
  });
  it('should verify email address', () => {
  });
  it('should fail to send verification SMS OTP for unregistered mobile number', () => {
  });
  it('should send customized verification SMS OTP', () => {
  });
  it('should send packaged verification SMS OTP', () => {
  });
  it('should fail to resend verification SMS OTP for unregistered mobile number', () => {
  });
  it('should resend customized verification SMS OTP', () => {
  });
  it('should resend packaged verification SMS OTP', () => {
  });
  it('should fail to verify OTP for unregistered mobile number', () => {
  });
  it('should fail to verify for already verified mobile number', () => {
  });
  it('should fail to verify for expired OTP', () => {
  });
  it('should fail to verify for incorrect OTP', () => {
  });
  it('should fail to verify for invalid OTP', () => {
  });
  it('should verify Mobile Number OTP', () => {
  });
});
