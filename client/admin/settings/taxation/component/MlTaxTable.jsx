...
export default class MlTaxTable extends Component {
  constructor(props) {
    ...
    this.state = {
      taxTypeInfo: [{
        taxId: '', taxName: '', taxPercentage: '', applicableStates: '', statesInfo: [], aboutTax: ''
      }]
    }
  ...
  componentWillMount() {
    const taxTableDetails = this.props.taxTableDetails
    if (taxTableDetails) {
      const taxInfo = []
  ...
  onGetTaxDetails(taxDetails) {
    ...
    const taxTypeInfoDetails = this.state.taxTypeInfo
  ...
  async findTaxTypDetails() {
    const taxDetails = await findTaxTypeDetailsActionHandler();
    const taxInfo = []
    for (let i = 0; i < taxDetails.length; i++) {
      const json = {
