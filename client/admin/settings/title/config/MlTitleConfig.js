...
const mlTitleTableConfig = new MlViewer.View({
  ...
  columns: [
    ...
    {
      dataField: 'titleInfo.titleName', title: 'Title', dataSort: true, customComponent(data) { return <div>{data.data.titleInfo.titleName}</div> }
    },
    {
      dataField: 'titleInfo.titleDisplayName', title: 'Display Name', dataSort: true, customComponent(data) { return <div>{data.data.titleInfo.titleDisplayName}</div> }
    },
  ...
  actionConfiguration: [
    ...
    },
    {
      ...
      actionName: 'add',
      handler: (data) => {
        if (data && data.id) { FlowRouter.go('/admin/settings/titleList') } else {
  ...
  sizePerPage: 5,
  queryOptions: true,
  buildQueryOptions: config => ({ context: { settingsType: 'TITLE' } }),
  graphQlQuery: gql`
