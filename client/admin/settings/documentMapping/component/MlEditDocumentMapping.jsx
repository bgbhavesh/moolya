...
class MlEditDocumentMapping extends React.Component {
  ...
  async handleSuccess(response) {
    if (response) {
      if (response.success) { FlowRouter.go('/admin/settings/documentProcess/documentMappingList'); } else { toastr.error(response.result); }
    }
  }
  ...
  async findDocument() {
    ...
    if (response) {
      ...
      this.setState({ allowableUnit, data: cloneBackUp, loading: false })
      ...
      if (this.state.data.documentType) {
        const documentTypeId = this.state.data.documentType
        this.setState({ documentType: documentTypeId });
      }
      if (this.state.data.kycCategory) {
        const kycCategoryId = this.state.data.kycCategory;
        this.setState({ kycCategory: kycCategoryId });
      }
      if (this.state.data.allowableFormat) {
        const allowableFormatId = this.state.data.allowableFormat
      ...
      if (this.state.data && this.state.data.validity) {
        const existData = this.state.data;
  ...
  async updateDocument() {
    ...
    if (ret) {
      ...
      const id = this.refs.documentId.value;
  ...
  onemploymentDateSelection(event) {
    if (event._d) {
      const value = moment(event._d).format('MM-DD-YYYY 00:00:00');
      const existData = this.state.data;
