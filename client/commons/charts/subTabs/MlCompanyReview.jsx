...
export default class MlCompanyReview extends React.Component {
  ...
  sendDataToParent(index) {
    if (index == null) {
      ...
      const arr = [];
      _.each(startupCompanyReview, (item) => {
      ...
      this.setState({ startupCompanyReview })
  ...
  componentWillMount() {
    const empty = _.isEmpty(this.context.startupPortfolio && this.context.startupPortfolio.reviewOfCompanyChart)
    if (!empty) {
