...
import {createLibrary, fetchLibrary, updatePrivacyDetails} from '../actions/IdeaActionHandler'
...
export default class PortfolioLibrary extends React.Component{
  ...
  constructor(props) {
    ...
    this.updatePrivacyDetails.bind(this);
  }
  ...
  // }
  //
  async updatePrivacyDetails(index, element , type){
    switch(type) {
        case"image": {
          const response = await updatePrivacyDetails(index, element, type)
          let userId = Meteor.userId()
          this.getLibraryDetails(userId);
          return response
          break;
  ...
  randomVideo(link,index){
    ...
    let videoPreviewUrl,videoPreviewType;
    videoPreviewUrl = data[index].fileUrl;
    videoPreviewType = data[index].fileType
    this.setState({previewVideo:videoPreviewUrl,videoUrl:videoPreviewUrl,videoFileType:videoPreviewType});
  ...
  toggleImageLock(id){
    ...
    console.log(id);
    this.updatePrivacyDetails(id,this.state.imagesLock,"image")
  ...
  toggleTemplateLock(id){
    ...
    console.log(id);
    this.updatePrivacyDetails(id,this.state.templatesLock)
  ...
  render(){
    ...
    var videoJsOptions = [{
      ...
      sources: [{
        ...
        type: this.state.videoPreviewType?this.state.videoPreviewType:'video/mp4'
      }]
    }]
    ...
    const Images = imageData.map(function(show,id) {
      return(
        <div className="thumbnail"key={id}>
          {show.isPrivate || that.state.imagesLock[id] ? <FontAwesome onClick={()=>that.toggleImageLock(id)} name='lock' /> : <FontAwesome onClick={()=>that.toggleImageLock(id)} name='unlock'/> }
