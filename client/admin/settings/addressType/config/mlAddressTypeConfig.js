...
const mlAddressTypeTableConfig = new MlViewer.View({
  ...
  columns: [
    ...
    {
      dataField: 'addressTypeInfo.addressName', title: 'Name', dataSort: true, customComponent(data) { return <div>{data.data.addressTypeInfo.addressName}</div> }
    },
    {
      dataField: 'addressTypeInfo.addressDisplayName', title: 'Display Name', dataSort: true, customComponent(data) { return <div>{data.data.addressTypeInfo.addressDisplayName}</div> }
    },
  ...
  sizePerPage: 5,
  queryOptions: true,
  buildQueryOptions: config => ({ context: { settingsType: 'ADDRESSTYPE' } }),
  graphQlQuery: gql`
