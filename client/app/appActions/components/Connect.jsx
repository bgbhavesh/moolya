...
export default class Connect extends React.Component {
  ...
  async connectHandler() {
    ...
    const response = await connectActionHandler({ resourceId: resourceDetails.resourceId, resourceType: resourceDetails.resourceType });
  ...
  async acceptConnectionHandler() {
    const response = await acceptConnectionActionHandler({ connectionId: 'yQqsrA5TaXptNJGEr' });
    if (response) {
  ...
  async rejectConnectionHandler() {
    const response = await rejectConnectionActionHandler({ connectionId: 'yQqsrA5TaXptNJGEr' });
    if (response) {
