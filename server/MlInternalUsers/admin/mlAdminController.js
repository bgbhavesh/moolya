...
export const createApolloServer = (customOptions = {}, customConfig = {}) => {
  ...
  const parseBody = bodyParser.json({ limit: '10mb' });
  ...
  graphQLServer.get(config.view, async (req, res, next) => {
    const pathJadeFiles = Assets.absoluteFilePath('microSite/views/about.pug')
   ...
   */
  graphQLServer.get('/sitemap.xml', async (req, res) => {
    // Creating SiteMap.
  ...
  graphQLServer.use(config.path, parseBody, graphqlExpress(async (req, res) => {
    try {
      ...
      const isAut = mlAuthorization.authChecker({ req, context })
      if (!isAut) {
        res.json({ unAuthorized: true, message: 'Not Authorized' })
        return;
      }
  ...
  if (config.assignUsersPath) {
    graphQLServer.post(config.assignUsersPath, multipartMiddleware, Meteor.bindEnvironment((req, res) => {
      let context = {};
  ...
  if (config.registrationPath) {
    graphQLServer.use(config.registrationPath, multipartMiddleware, Meteor.bindEnvironment((req, res) => {
      ...
      if (req && req.body && req.body.data) {
        ...
        const docTypeId = data && data.docTypeId
        let response;
        const file = req.files.file;
        ...
        if (file) {
          ...
          switch (moduleName) {
            case 'REGISTRATION': {
              ...
              imageUploadCallback = Meteor.bindEnvironment((resp, serverRes, serverResOb) => {
                const registrationDocumentUploadReq = {
            ...
            case 'PORTFOLIO': {
              if (data.portfolioDetailsId) {
                ...
                imageUploadCallback = Meteor.bindEnvironment((resp, serverRes, serverResOb) => {
                  const details = MlPortfolioDetails.findOne({ _id: data.portfolioDetailsId });
                  if (details) {
                    const clientPortfolio = data.portfolio;
                    for (key in clientPortfolio) {
                      const inner = clientPortfolio[key]
                      if (typeof inner === 'object') {
                        for (key1 in inner) {
                          const file = inner[key1]
            ...
            case 'PORTFOLIO_PROFILE_IMG': {
              imageUploaderPromise = new ImageUploader().uploadFile(file, bucketName, 'registrationDocuments/');
              imageUploadCallback = Meteor.bindEnvironment((resp, serverRes, serverResOb) => {
                const portfolioDocumentUploadReq = {
            ...
            case 'PORTFOLIO_IDEA_IMG': {
              imageUploaderPromise = new ImageUploader().uploadFile(file, bucketName, 'registrationDocuments/');
              imageUploadCallback = Meteor.bindEnvironment((resp, serverRes, serverResOb) => {
                const ideaImage = { fileUrl: resp, fileName: file.name };
                if (data.isCreate) {
          ...
          if (imageUploaderPromise) {
            imageUploaderPromise.then((uploadResp) => { // sucess
              let response = null;
              if (uploadResp) {
              ...
              const response = new MlRespPayload().errorPayload('Failed to Upload the resource', 404);
              res.send(response);
            });
          }
        }
  ...
  if (config.registrationAPIPath) {
    ...
    graphQLServer.post(config.registrationAPIPath, bodyParser.json(), Meteor.bindEnvironment((req, res) => {
      ...
      if (req && req.body && req.body.data) {
        ...
        if (apiKey && apiKey === '741432fd-8c10-404b-b65c-a4c4e9928d32') {
          if (data.email) {
            ...
            const code = 400;
            const result = { message: 'email,countyId,registrationType are mandatory fields' }
          ...
          const code = 401;
          const result = { message: 'The request did not have valid authorization credentials' }
          const response = new MlRespPayload().errorPayload(result, code);
          console.log(response);
          res.send(response);
        }
  ...
  if (config.countries) {
    ...
    graphQLServer.get(config.countries, bodyParser.json(), Meteor.bindEnvironment((req, res) => {
      res.header('Access-Control-Allow-Origin', '*');
      res.header('Access-Control-Allow-Headers', 'Origin, X-Requested-With, Content-Type, Accept');
      let context = {};
      context = getContext({ req });
  ...
  if (config.cities) {
    ...
    graphQLServer.post(config.cities, bodyParser.json(), Meteor.bindEnvironment((req, res) => {
      res.header('Access-Control-Allow-Origin', '*');
      res.header('Access-Control-Allow-Headers', 'Origin, X-Requested-With, Content-Type, Accept');
      let context = {};
      context = getContext({ req });
  ...
  if (config.communities) {
    ...
    graphQLServer.post(config.communities, bodyParser.json(), Meteor.bindEnvironment((req, res) => {
      res.header('Access-Control-Allow-Origin', '*');
      res.header('Access-Control-Allow-Headers', 'Origin, X-Requested-With, Content-Type, Accept');
      let context = {};
      context = getContext({ req });
  ...
  if (config.couponValidate) {
    ...
    graphQLServer.post(config.couponValidate, bodyParser.json(), Meteor.bindEnvironment((req, res) => {
      res.header('Access-Control-Allow-Origin', '*');
      res.header('Access-Control-Allow-Headers', 'Origin, X-Requested-With, Content-Type, Accept');
      let context = {};
      context = getContext({ req });
          ...
          if (data) {
            ...
            if (resp) {
              ...
              if (resp.validityFrom <= currentDate && currentDate <= resp.validityTo) {
                const code = 200;
                const result = { message: 'valid promo applied' }
                const response = new MlRespPayload().successPayload(result, code);
                console.log(response);
                res.send(response);
              } else {
                const code = 401;
                const result = { message: 'Invalid promo code applied' }
                const response = new MlRespPayload().errorPayload(result, code);
                console.log(response);
                res.send(response);
              }
  ...
  if (config.resetPassword) {
    ...
    graphQLServer.post(config.resetPassword, bodyParser.json(), Meteor.bindEnvironment((req, res) => {
      console.log(req, res);
      res.header('Access-Control-Allow-Origin', '*');
      res.header('Access-Control-Allow-Headers', 'Origin, X-Requested-With, Content-Type, Accept');
      let context = {};
  ...
  if (config.forgotPassword) {
    ...
    graphQLServer.post(config.forgotPassword, bodyParser.json(), Meteor.bindEnvironment((req, res) => {
      res.header('Access-Control-Allow-Origin', '*');
      res.header('Access-Control-Allow-Headers', 'Origin, X-Requested-With, Content-Type, Accept');
      let context = {};
      context = getContext({ req });
  ...
  if (config.verifyEmail) {
    ...
    graphQLServer.post(config.verifyEmail, bodyParser.json(), Meteor.bindEnvironment((req, res) => {
      console.log(req, res);
      res.header('Access-Control-Allow-Origin', '*');
      res.header('Access-Control-Allow-Headers', 'Origin, X-Requested-With, Content-Type, Accept');
      let context = {};
  ...
  if (config.verifyMobileNumber) {
    ...
    graphQLServer.post(config.verifyMobileNumber, bodyParser.json(), Meteor.bindEnvironment((req, res) => {
      res.header('Access-Control-Allow-Origin', '*');
      res.header('Access-Control-Allow-Headers', 'Origin, X-Requested-With, Content-Type, Accept');
      let context = {};
      context = getContext({ req });
  ...
  if (config.resendOTP) {
    ...
    graphQLServer.post(config.resendOTP, bodyParser.json(), Meteor.bindEnvironment((req, res) => {
      res.header('Access-Control-Allow-Origin', '*');
      res.header('Access-Control-Allow-Headers', 'Origin, X-Requested-With, Content-Type, Accept');
      let context = {};
      context = getContext({ req });
  ...
  if (config.verifyLaterUserMobileNumber) {
    ...
    graphQLServer.post(config.verifyLaterUserMobileNumber, bodyParser.json(), Meteor.bindEnvironment((req, res) => {
      res.header('Access-Control-Allow-Origin', '*');
      res.header('Access-Control-Allow-Headers', 'Origin, X-Requested-With, Content-Type, Accept');
      let context = {};
      context = getContext({ req });
  ...
  if (config.buildVersion) {
    ...
    graphQLServer.post(config.buildVersion, bodyParser.json(), Meteor.bindEnvironment((req, res) => {
      res.header('Access-Control-Allow-Origin', '*');
      res.header('Access-Control-Allow-Headers', 'Origin, X-Requested-With, Content-Type, Accept');
...
updateBuildVersion = (request) => {
  ...
  const returnResponse = { success: false, dbUpdate: false };
