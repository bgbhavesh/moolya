...
const mlGenderTableConfig = new MlViewer.View({
  ...
  columns: [
    ...
    {
      dataField: 'genderInfo.genderName', title: 'Name', dataSort: true, customComponent(data) { return <div>{data.data.genderInfo.genderName}</div> }
    },
    {
      dataField: 'genderInfo.genderDisplayName', title: 'Display Name', dataSort: true, customComponent(data) { return <div>{data.data.genderInfo.genderDisplayName}</div> }
    },
    {
      dataField: 'genderInfo.aboutGender', title: 'About', dataSort: true, customComponent(data) { return <div>{data.data.genderInfo.aboutGender}</div> }
    },
  ...
  sizePerPage: 5,
  queryOptions: true,
  buildQueryOptions: config => ({ context: { settingsType: 'GENDER' } }),
  graphQlQuery: gql`
