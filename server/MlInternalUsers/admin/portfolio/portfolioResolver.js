...
MlResolver.MlQueryResolver.fetchPortfolioDetailsByUserId = (obj, args, context, info) => {
  if (context.userId) {
    ...
    if (defaultProfile) {
      let defaultCommunity = defaultProfile.communityDefCode || {};
      let portfolio = MlPortfolioDetails.findOne({ $and: [{ userId: context.userId }, { communityCode: defaultCommunity }, { profileId: defaultProfile.profileId }] });
      if (portfolio) {
