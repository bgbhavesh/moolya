...
const mlTaxTypeTableConfig = new MlViewer.View({
  ...
  columns: [
    ...
    {
      dataField: 'taxTypeInfo.taxName', title: 'Tax Type', dataSort: true, customComponent(data) { return <div>{data.data.taxTypeInfo.taxName}</div> }
    },
    {
      dataField: 'taxTypeInfo.taxDisplayName', title: 'Display Name', dataSort: true, customComponent(data) { return <div>{data.data.taxTypeInfo.taxDisplayName}</div> }
    },
  ...
  actionConfiguration: [
    ...
    },
    {
      ...
      actionName: 'add',
      handler: (data) => {
        if (data && data.id) { FlowRouter.go('/admin/settings/taxTypeList') } else {
  ...
  sizePerPage: 5,
  queryOptions: true,
  buildQueryOptions: config => ({ context: { settingsType: 'TAXTYPE' } }),
  sizePerPage: 5,
  graphQlQuery: gql`
