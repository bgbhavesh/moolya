...
export default class MlAssignDocument extends React.Component {
  ...
  componentWillMount() {
    const assignDocument = this.props.documents
    if (assignDocument) {
      const assignDocumentDetails = []
  ...
  RemoveModuleToRoles(id, event) {
    let assignDocuments;
    assignDocuments = this.state.assignDocuments.filter((object, index) => id !== index);
    this.setState({
      assignDocuments
    })
  ...
  optionsBySelectDocument(index, selectedIndex) {
    const assignDocuments = this.state.assignDocuments
    const assignedData = _.findWhere(assignDocuments, { type: selectedIndex, category: assignDocuments[index].category });
    if (assignedData && assignedData != undefined) {
      toastr.error('Document combination exists');
    } else {
      assignDocuments[index].type = selectedIndex
      this.setState({ assignDocuments })
      this.props.getAssignedDocuments(this.state.assignDocuments)
    }
  }
  optionsBySelectKyc(index, selectedIndex) {
    const assignDocuments = this.state.assignDocuments
    const assignedData = _.findWhere(assignDocuments, { type: assignDocuments[index].type, category: selectedIndex });
    if (assignedData && assignedData != undefined) {
      toastr.error('Document combination exists');
    } else {
  ...
  onStatusChange(index, event) {
    const assignDocumentsDetails = this.state.assignDocuments
  ...
  render() {
    ...
    const docTypequery = gql` query{
    data:fetchDocumentsType{label:docTypeName,value:_id}
    }
`;
    const docKycquery = gql`query($documentTypeId:String,$clusterId:[String],$chapterId:[String],$subChapterId:[String]){
...
`;
    ...
    return (
      ...
      <div>
        ...
        {that.state.assignDocuments.map((options, id) => {
          const kycOption = {
            options: {
              variables: {
                documentTypeId: options.type, clusterId: that.props.clusterId, chapterId: that.props.chapterId, subChapterId: that.props.subChapterId
              }
            }
          };
          return (
