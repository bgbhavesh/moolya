...
export default class FunderAboutView extends React.Component {
  ...
  imageUpload(id, e) {
    ...
    const response = multipartASyncFormHandler(data, file, 'registration', this.onFileUploadCallBack.bind(this, id, file));
    return response;
  }
  ...
  imagesDisplay(index, fileUrl, imageObject) {
    const tempTaskImages = this.state.tasks || []
    const taskImages = _.cloneDeep(tempTaskImages);
    taskImages.map((data, id) => {
      if (id === index) {
        if (data.images) {
  ...
  render() {
    ...
    const that = this;
    const totalTasks = tasks.map((task, id) => (
      <div>
          ...
          <div className="panel-body uploaded_files_swiper" key={id}>
            <ul className="swiper-wrapper">
              <li className="doc_card" data-toggle="tooltip" data-placement="bottom" title="File name">
                {task.images ? task.images.map((image, id) => (<img src={image} style={{ maxWidth: 150 }}/>)) : ''}
              </li>
            </ul>
          </div>
        </div>
