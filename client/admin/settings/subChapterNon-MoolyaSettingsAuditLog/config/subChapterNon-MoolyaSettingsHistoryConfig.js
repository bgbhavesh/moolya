...
function ipAddressFormatter(data) {
  const ipaddr = data && data.data && data.data.userAgent.ipAddress ? data.data.userAgent.ipAddress : '';
  return <div>{ipaddr}</div>
}

function dateFormatter(data) {
  const createdDateTime = data && data.data && data.data.timeStamp ? data.data.timeStamp : null;
  return <div>{createdDateTime && moment(createdDateTime).format('DD-MM-YYYY hh:mm:ss')}</div>;
}


const mlsubChapterNonMoolyaSettingsTabHistoryTableConfig = new MlViewer.View({
  name: 'auditLogTable',
  ...
  buildQueryOptions: config =>
    ({ context: { moduleName: 'GLOBALSETTINGS,MASTERSETTINGS,TAXATION,USERS,DEPARTMENT,SUBDEPARTMENT,ROLES' } }), // return {context:{moduleName:"CLUSTER, SUBCHAPTER",clusterId:config.params&&config.params.clusterId?config.params.clusterId:null}}
...
export { mlsubChapterNonMoolyaSettingsTabHistoryTableConfig };
