...
MlResolver.MlQueryResolver.fetchIdeatorUsers = (obj, args, context, info) => {
  ...
  const code = 200;
  const response = new MlRespPayload().successPayload(sample, code);
  return response;
}
...
MlResolver.MlQueryResolver.findAddressBook = (obj, args, context, info) => {
  ...
  if (user) {
    ...
    const mobileNumbs = [];
    _.each(user.mobileNumbers, (mob) => {
      if (mob.verified) {
        const country = mlDBController.findOne('MlCountries', { _id: mob.countryId }) || {};
...
MlResolver.MlMutationResolver.updateUserGeneralInfo = (obj, args, context, info) => {
  ...
  const registrationDetails = mlDBController.findOne('users', { _id: context.userId }, context) || {};
  ...
  if (id) {
    const code = 200;
    const response = new MlRespPayload().successPayload(id, code);
    return response
  }
}
...
MlResolver.MlMutationResolver.createUserGeneralInfo = (obj, args, context, info) => {
  ...
  if (args.registration && args.registrationId && args.profileId) {
    ...
    const infoDetails = _underscore.find(addInfo, { profileId: args.profileId }) || {};


    if (args && args.registration) {
      if (args.type == 'CONTACTTYPE') {
        const dbData = _underscore.pluck(infoDetails.contactInfo, 'numberType') || [];
        ...
        if (contactExist) {
          const code = 409;
          const response = new MlRespPayload().errorPayload("'Contact type' already exist!", code);
          return response;
        }
        ...
        const dbData = _underscore.pluck(infoDetails.addressInfo, 'addressType') || [];
        ...
        const dbData = _underscore.pluck(infoDetails.emailInfo, 'emailIdType') || [];
...
MlResolver.MlQueryResolver.fetchUserProfiles = (obj, args, context, info) => {
  ...
  if (user) {
    let userProfiles = user.profile.isExternaluser ? user.profile.externalUserProfiles : [];

    // todo: add isApproved Attribute to the profile
    userProfiles = _.filter(userProfiles, { isActive: true }) || [];
    userProfiles = _.map(userProfiles, (profile) => {
      const communityDetails = mlDBController.findOne('MlCommunityDefinition', { code: profile.communityDefCode }, context) || {};
...
MlResolver.MlMutationResolver.deActivateUserProfile = (obj, args, context, info) => {
  ...
  if (user && args && args.profileId) {
    const result = mlDBController.update(
      'users', { 'profile.externalUserProfiles': { $elemMatch: { profileId: args.profileId } } },
      { 'profile.externalUserProfiles.$.isDefault': false }, { $set: true }, context
    );
    response = new MlRespPayload().successPayload({}, 200);
    if (result) {
      const emailSent = MlEmailNotification.onDeactivateUser(context);
      const verficationEmail = MlEmailNotification.requestForProfileDeactivation(context);
    }
  } else {
    const code = 409;
...
MlResolver.MlMutationResolver.setDefaultProfile = (obj, args, context, info) => {
  ...
  if (user && args && args.profileId) {
    ...
    if (hasSwitchedProfile) {
      result = mlDBController.update(
        'users', { _id: userId, 'profile.hasSwitchedProfile': true, 'profile.externalUserProfiles': { $elemMatch: { profileId: args.profileId } } },
        { 'profile.switchedProfileDefaultId': args.profileId }, { $set: true }, context
      );
    } else {
      result = mlDBController.update(
        'users', { _id: userId, 'profile.externalUserProfiles': { $elemMatch: { isDefault: true } } },
      ...
      result = mlDBController.update(
        'users', { _id: userId, $or: [{ 'profile.hasSwitchedProfile': false }, { 'profile.hasSwitchedProfile': { $type: 10 } }, { 'profile.hasSwitchedProfile': { $exists: false } }], 'profile.externalUserProfiles': { $elemMatch: { profileId: args.profileId } } },
...
MlResolver.MlMutationResolver.switchExternalProfile = (obj, args, context, info) => {
  ...
  const user = mlDBController.findOne('users', { _id: userId }) || {}
  if (user && args && args.profileId) {
    const defaultUserProfile = _.find(user.profile.externalUserProfiles, { isDefault: true }) || user.profile.externalUserProfiles[0];
    const defaultUserProfileId = defaultUserProfile ? defaultUserProfile.profileId : null;
    ...
    result = mlDBController.update(
      'users', { _id: userId, 'profile.externalUserProfiles': { $elemMatch: { profileId: args.profileId } } },
      {
...
MlResolver.MlMutationResolver.deActivateUserProfileByContext = (obj, args, context, info) => {
  ...
  if (args.userProfiles && args.userProfiles.profileId) {
    const profileId = args.userProfiles.profileId
    const result = mlDBController.update(
      'users', { 'profile.externalUserProfiles': { $elemMatch: { profileId } } },
      {
        'profile.externalUserProfiles.$.isDefault': false,
        'profile.externalUserProfiles.$.isActive': args.userProfiles.isActive
      }, { $set: true }, context
    );
    if (result) { response = new MlRespPayload().successPayload('User profile updated successfully ', 200); } else { response = new MlRespPayload().errorPayload('Unable to update profile', 409); }
  } else {
...
MlResolver.MlQueryResolver.findDefaultUserProfile = (obj, args, context, info) => {
  ...
  if (user && user.profile && user.profile.isExternaluser === true) {
    const user_profiles = user.profile.externalUserProfiles || [];
...
MlResolver.MlQueryResolver.fetchMoolyaAdmins = (obj, args, context, info) => {
  ...
  let profileDetails = {}
  if (args.profileId) { profileDetails = new MlUserContext().userProfileDetailsByProfileId(args.profileId) } else { profileDetails = new MlUserContext(userId).userProfileDetails(userId) }
  var query = [
    {
      $match: {
        'profile.isInternaluser': true,
...
MlResolver.MlQueryResolver.fetchAppMapData = (obj, args, context, info) => {
  ...
  const userSubChapterId = userProfile.subChapterId;
  ...
  if (!isDefaultSubChapter) {
    ...
    if (relatedSubChapters && relatedSubChapters.length > 0) {
      ...
      const relatedSC = mlDBController.find('MlSubChapters', { _id: { $in: relatedSubChapterIds } }).fetch()
  ...
  switch (args.moduleName) {
    case 'cluster':
      ...
      if (isDefaultSubChapter) {
        const sub = mlDBController.find('MlSubChapters', { $or: [{ clusterId: args.id, isActive: true, 'moolyaSubChapterAccess.externalUser.canSearch': true }, { clusterId: args.id, isActive: true, isDefaultSubChapter: true }] }, context).fetch()
        ...
        query = {
          clusterId: args.id, chapterId: { $in: ids }, subChapterId: { $in: subIds }, isActive: true
        };
      } else {
        chapterCount = mlDBController.find('MlChapters', { clusterId: args.id, isActive: true, _id: { $in: relatedChapterId } }, context).count();
        query = {
          clusterId: args.id, isActive: true, chapterId: { $in: relatedChapterId }, subChapterId: { $in: relatedSubChapterIds }
        };
      }
      break;
    case 'chapter':
      ...
      if (isDefaultSubChapter) {
        const sub = mlDBController.find('MlSubChapters', { $or: [{ chapterId: args.id, isActive: true, 'moolyaSubChapterAccess.externalUser.canSearch': true }, { chapterId: args.id, isActive: true, isDefaultSubChapter: true }] }, context).fetch()
        const ids = _.map(sub, '_id');

        chapterCount = sub.length;
        query = {
          chapterId: args.id, clusterId: chapter.clusterId, subChapterId: { $in: ids }, isActive: true
        };
      } else {
        ...
        query = {
          $or: [{
            ...
            clusterId: chapter.clusterId, chapterId: args.id, isActive: true, isDefaultSubChapter: true
          }]
        };
      }
      break;
    case 'subChapter':
      ...
      query = {
        clusterId: subChapter.clusterId, chapterId: subChapter.chapterId, subChapterId: args.id, isActive: true
      };
      break;
    case 'community':
      moduleContext = 'Users'
