...
class MlDBController {
  ...
  insert(collectionName, payload, context) {
    ...
    if (resp) {
      const response = mlAuditLog.insertAudit({ collectionName, docId: resp, queryPayload: payload }, context)
      return resp;
    }
  }
  ...
  find(collectionName, query, context, fieldsProj) {
    let collection;
    if (collectionName == 'users') { collection = Meteor.users } else { collection = MlCollections[collectionName] }
  ...
  findOne(collectionName, query, context) {
    ...
    const response = collection.findOne(query);
    return response;
  }
