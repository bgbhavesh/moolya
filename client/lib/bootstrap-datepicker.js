 ...
 */(function (factory) {
  ...
  Datepicker.prototype = {
    ...
    _resolveViewName(view, default_value) {
    ...
    _process_options(opts) {
      ...
      o.daysOfWeekHighlighted = $.map(o.daysOfWeekHighlighted, d => parseInt(d, 10));
    ...
    fillMonths() {
      ...
      let html = '',
        i = 0;
      while (i < 12) {
        const focused = localDate && localDate.getMonth() === i ? ' focused' : '';
    ...
    setRange(range) {
      if (!range || !range.length) { delete this.range; } else { this.range = $.map(range, d => d.valueOf()); }
      this.fill();
    },
    ...
    _fill_yearsView(selector, cssClass, factor, step, currentYear, startYear, endYear, callback) {
      let html,
        view,
        year,
        steps,
      ...
      steps = $.map(this.dates, d => parseInt(d.getUTCFullYear() / step, 10) * step);
    ...
    click(e) {
      ...
      const navArrow = target.closest('.prev, .next');
