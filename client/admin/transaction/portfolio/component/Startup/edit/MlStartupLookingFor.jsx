...
export default class MlStartupLookingFor extends Component {
  ...
  async fetchPortfolioDetails() {
    ...
    const empty = _.isEmpty(that.context.startupPortfolio && that.context.startupPortfolio.lookingFor)
  ...
  onTileClick(index, e) {
    const cloneArray = _.cloneDeep(this.state.startupLookingFor);
  ...
  lockPrivateKeys(selIndex) {
    const privateValues = this.startupLookingForServer && this.startupLookingForServer[selIndex] ? this.startupLookingForServer[selIndex].privateFields : []
  ...
  onSaveAction(e) {
    ...
    let setObject = this.state.startupLookingFor
    if (this.context && this.context.startupPortfolio && this.context.startupPortfolio.lookingFor) { setObject = this.context.startupPortfolio.lookingFor }
  ...
  sendDataToParent(isSaveClicked) {
    ...
    const startupLookingFor1 = this.state.startupLookingFor;
    let startupLookingFor = _.cloneDeep(startupLookingFor1);
    data.index = this.state.selectedIndex;
    if (isSaveClicked) { startupLookingFor[this.state.selectedIndex] = data; }
    const arr = [];
    _.each(startupLookingFor, (item) => {
    ...
    this.setState({ startupLookingFor })
  ...
  render() {
    ...
    const lookingOption = { options: { variables: { communityCode: 'STU' } } };
    const that = this;
    const startupLookingForList = that.state.startupLookingForList || [];
    return (
      <div>
              ...
              >
                <div className="col-lg-12">
                  <div className="row">
                    ...
                    {startupLookingForList.map((details, idx) => (<div className="col-lg-2 col-md-3 col-sm-3" key={idx}>
