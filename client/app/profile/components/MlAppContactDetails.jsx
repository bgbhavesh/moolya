...
export default class AppContactDetails extends React.Component {
  ...
  async onSavingContact(index, value) {
    ...
    const countrycode = this.state.countryDetails.countryCode;
  ...
  async onEditingContact(index, value) {
    ...
    const registrationDetails = this.props.contactInfoDetails
    ...
    if (contactExist) {
      ...
      if (ret) {
        ...
        const labelValue = this.state.selectedNumberTypeLabel ? this.state.selectedNumberTypeLabel : this.state.details[index].numberTypeName;
        const valueSelected = this.state.selectedNumberTypeValue ? this.state.selectedNumberTypeValue : this.state.details[index].numberType;
        const updatedComment = update(this.state.details[index], {
  ...
  render() {
    const that = this;
    const details = that.state.details;
    const verifiedMobileNumbers = this.props.verifiedMobileNumbers ? this.props.verifiedMobileNumbers : [];
    ...
    const numberTypeOption = { options: { variables: { type: 'CONTACTTYPE', hierarchyRefId: clusterId } } };
        ...
        {showLoader === true ? (<MlLoader/>) : (
          <div className="ml_tabs">
            <ul className="nav nav-pills">
              ...
              {verifiedMobileNumbers && (verifiedMobileNumbers.map((options, key) => (
            ...
            <div className="tab-content clearfix">
              ...
              <div className={`tab-pane${this.state.activeTab}`} id="contactA">
                <div className="form-group">
                  <Moolyaselect
                    multiSelect={false} ref="numberType" placeholder="Select Number Type" mandatory={true}
              ...
              {/* FOR VIERIFIED MOBILE NUMBERS */}
              {verifiedMobileNumbers && (verifiedMobileNumbers.map((options, key) => (<div className="tab-pane" id={`Official${key}`} key={key}>
                <div className="form-group mandatory">
                  <input
                    type="text" placeholder="Select Number Type" value={options.numberType}
                    className="form-control float-label" readOnly={true}/>
                </div>
                <div className="form-group">
                  <input
                    type="text" placeholder="Enter Country Code" value={options.phoneNumberCode}
                    className="form-control float-label" readOnly={true}/>
                </div>
                {/* defaultValue={countryPhoneCode} valueKey={countryPhoneCode} */}
                <div className="form-group mandatory">
                  <input
                    type="text" placeholder="Enter Number" id="phoneNumber" value={options.mobileNumber}
                    className="form-control float-label" readOnly={true}/>
                </div>
              </div>)))}


              {details && (details.map((options, key) => (<div className="tab-pane" id={`numberType${key}`} key={key}>
                <div className="form-group">
                  <Moolyaselect
                    multiSelect={false} ref={`numberType${key}`} placeholder="Select NumberType"
                    mandatory={true}
                ...
                <div className="form-group">
                  <input
                    type="text" placeholder="Enter Country Code" ref={`countryCode${key}`}
                    className="form-control float-label"/>
                </div>
                {/* defaultValue={countryPhoneCode} valueKey={countryPhoneCode} */}
                <div className="form-group mandatory">
                  <input
                    type="text" ref={`contactNumber${key}`} placeholder="Enter Number" id="phoneNumber"
