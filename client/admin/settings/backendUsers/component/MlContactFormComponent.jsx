...
export default class MlContactFormComponent extends React.Component {
  constructor(props) {
    ...
    this.state = {
      contactForm: [{
        contactNumberType: null, countryCode: '', number: '', isOTPValidated: ''
      }],
      isWorkEnble: true,
  ...
  componentWillMount() {
    const assignContactForm = this.props.contacts
    if (assignContactForm) {
      const assignContactFormDetails = []
  ...
  AssignDepartment(idx) {
    this.setState({
      contactForm: this.state.contactForm.concat([{
        contactNumberType: null, countryCode: '', number: '', isOTPValidated: ''
      }])
    });
  }
  RemoveAssignContactForm(idx, event) {
    ...
    assignContactForm = this.state.contactForm.filter((object, index) => idx !== index);
    this.setState({
  ...
  optionsBySelectNumberType(index, selectedValue) {
    const assignContactDetails = this.state.contactForm
...
`;
    ...
    return (
      <div>
        ...
        {that.state.contactForm.map((contactForm, idx) => (
