...
class MlAppPortfolio extends Component {
  ...
  activateActionPanelAccordion() {
    const actionPanelActivationHandler = this.state.actionPanelActivationHandler;
  ...
  getSelectedAnnotation(selAnnotation) {
    ...
    setTimeout(() => {
      ...
      if (selAnnotation) {
        that.setState({ annotationData: selAnnotation }, () => {
  ...
  async fetchPortfolioImage() {
    const response = await fetchPortfolioImageHandler(this.props.config)
    if (response) {
  ...
  async fetchEditPortfolioTemplate(pId) {
    ...
    const reg = await fetchTemplateHandler({
      process: 'Registration', subProcess: 'Registration', stepCode: 'PORTFOLIO', recordId: pId, mode: 'edit', userType, connection: appClient
    });
  ...
  async fetchViewPortfolioTemplate(id) {
    const userType = this.context.userType;
    const reg = await fetchTemplateHandler({
      process: 'Registration', subProcess: 'Registration', stepCode: 'PORTFOLIO', recordId: id, mode: 'view', userType, connection: appClient
    });
    this.setState({ editComponent: reg && reg.component ? reg.component : null });
  }
  ...
  async fetchIdeaId() {
    ...
    if (response) { this.setState({ loading: false, ideaId: response._id }); } else {
  ...
  async interactionActionHandler(actionConfig, handlerCallback) {
    const resourceDetails = { resourceId: this.props.config, resourceType: 'portfolio' };
    if (handlerCallback) { // to handle the popover
      ...
      const resp = await actionConfig.handleCallBack(resourceDetails);
      return resp;
    }
  }
  async onInteractionSuccess() {
    const interactionAutoId = new Date().getTime();
    this.setState({ interactionAutoId });
  }
  ...
  assignActionHandlerProxy(actionConfig) {
    ...
    const actionMap = {
      like: 'interaction', connect: 'interaction', favourite: 'interaction', follow: 'interaction', enquire: 'interaction', review: 'interaction', partner: 'interaction', collaborate: 'interaction'
    };
        ...
        {showLoader === true ? (<MlLoader/>) : (
          <div className="app_padding_wrap">
            <div className="col-md-12">
              <InteractionsCounter
                resourceType={'portfolio'} resourceId={this.props.config}
                ...
                <ul id="comments-list" className="comments-list">
                  <li>
                    ...
                    <div className="comment_wrap">
                      <textarea
                        ref="comment" id="commentData" className="form-control comment-input-box"
