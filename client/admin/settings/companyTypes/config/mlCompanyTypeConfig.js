...
const mlCompanyTypeTableConfig = new MlViewer.View({
  ...
  columns: [
    ...
    {
      dataField: 'companyTypeInfo.companyName', title: 'Company Type', dataSort: true, customComponent(data) { return <div>{data.data.companyTypeInfo.companyName}</div> }
    },
    {
      dataField: 'companyTypeInfo.companyDisplayName', title: 'Display Name', dataSort: true, customComponent(data) { return <div>{data.data.companyTypeInfo.companyDisplayName}</div> }
    },
  ...
  sizePerPage: 5,
  queryOptions: true,
  buildQueryOptions: config => ({ context: { settingsType: 'COMPANYTYPE' } }),
  graphQlQuery: gql`
