...
class MlEditNotificationTemplate extends React.Component {
  ...
  async handleSuccess(response) {
    if (response) {
      if (response.success) { FlowRouter.go('/admin/settings/notificationTemplateList'); } else { toastr.error(response.result); }
    }
  }

  async findNotificationTemplate() {
    const tempId = this.props.config
  ...
  async updateNotificationTemplate() {
    ...
    if (ret) {
      ...
      const dynamicKeys = this.refs.dynamicKeys.value || '';
      const tempDetails = {
  ...
  templateContent(actionConfig, handlerCallback) {
    const tempContent = this.refs.content.value || '';
      ...
      },
      {
        ...
        actionComponent(props) {
          return <div className={props.activeClass} id={props.actionName}>
            <div
              onClick={props.onClickHandler} className={props.activesubclass} data-toggle="tooltip"
