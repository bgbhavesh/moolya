...
export default class MlAppActivityList extends React.Component {
  ...
  async fetchActivities() {
    ...
    const response = await fetchActivitiesActionHandler(profileId);
    if (response) {
      this.setState({
        activities: response,
        loading: false
  ...
  editMode(id, selectedProfileId) {
    const profileId = FlowRouter.getParam('profileId') === 'all' ? selectedProfileId : FlowRouter.getParam('profileId');
