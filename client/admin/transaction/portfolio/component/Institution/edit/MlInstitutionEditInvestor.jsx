...
export default class MlInstitutionEditInvestor extends React.Component {
  ...
  async fetchPortfolioDetails() {
    ...
    const empty = _.isEmpty(that.context.institutionPortfolio && that.context.institutionPortfolio.investor);
  ...
  onTileClick(index, uiIndex, e) {
    const cloneArray = _.cloneDeep(this.state.institutionInvestor);
  ...
  lockPrivateKeys(selIndex) {
    const privateValues = this.institutionInvestorServer && this.institutionInvestorServer[selIndex] ? this.institutionInvestorServer[selIndex].privateFields : []
  ...
  onSaveAction(e) {
    ...
    let setObject = this.state.institutionInvestor;
  ...
  sendDataToParent(isSaveClicked) {
    ...
    const institutionInvestor1 = this.state.institutionInvestor;
    ...
    const arr = [];
    _.each(institutionInvestor, (item) => {
    ...
    this.setState({ institutionInvestor })
  ...
  async fetchOnlyImages() {
    ...
    if (response && response.investor) {
      const dataDetails = this.state.institutionInvestor
      const cloneBackUp = _.cloneDeep(dataDetails);
      const specificData = cloneBackUp[this.state.selectedIndex];
      if (specificData) {
        const curUpload = response.investor[this.state.selectedIndex]
  ...
  render() {
    ...
    const investorsArray = that.state.institutionInvestorList || [];
