...
export default class MoolyaSelect extends Component {
  ...
  fetchSelectOptions(props, queryCallbackHandler, searchTerm) {
    const queryOptions = props.queryOptions && props.queryOptions.options && props.queryOptions.options.variables ? props.queryOptions : { options: { variables: {} } };
  ...
  onChangeCallBackHandler(items) {
    if (this._isMounted) {
      ...
      if (this.props.getUpdatedCallback) {
        ...
        const self = this
        _.each(items, (item) => {
          const isAvailiable = _.indexOf(self.props.selectedValue, item.value)
          if (isAvailiable >= 0) { updatedValues.push(self.props.selectedValue[isAvailiable]) }
        })
