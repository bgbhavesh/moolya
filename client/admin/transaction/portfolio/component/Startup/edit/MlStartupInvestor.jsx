...
export default class MlStartupInvestor extends Component {
  ...
  async fetchPortfolioDetails() {
    ...
    const empty = _.isEmpty(that.context.startupPortfolio && that.context.startupPortfolio.investor)
  ...
  onTileClick(index, uiIndex, e) {
    const cloneArray = _.cloneDeep(this.state.startupInvestor);
  ...
  lockPrivateKeys(selIndex) {
    const privateValues = this.startupInvestorServer && this.startupInvestorServer[selIndex] ? this.startupInvestorServer[selIndex].privateFields : []
  ...
  onSaveAction(e) {
    ...
    let setObject = this.state.startupInvestor
  ...
  sendDataToParent(isSaveClicked) {
    ...
    const startupInvestor1 = this.state.startupInvestor;
  ...
  onLogoFileUpload(image, fileInfo) {
    ...
    if (file) {
      const data = {
        moduleName: 'PORTFOLIO', actionName: 'UPLOAD', portfolioDetailsId: this.props.portfolioDetailsId, portfolio: { investor: [{ logo: { fileUrl: '', fileName }, index: this.state.selectedIndex }] }
      };
  ...
  async fetchOnlyImages() {
    ...
    if (response && response.investor) {
      ...
      const dataDetails = this.state.startupInvestor
      const cloneBackUp = _.cloneDeep(dataDetails);
      const specificData = cloneBackUp[thisState];
      if (specificData) {
        const curUpload = response.investor[thisState]
  ...
  render() {
    ...
    const investorsArray = that.state.startupInvestorList || [];
              ...
              >
                <div className="col-lg-12">
                  <div className="row">
                    ...
                    {investorsArray.map((details, idx) => (<div className="col-lg-2 col-md-3 col-sm-3" key={idx}>
                    ...
                    <div className="medium-popover"><div className="row">
                      <div className="col-md-12">
                        <div className="form-group mandatory">
                          <input
                            type="text" name="investorName" placeholder="Name" className="form-control float-label"
