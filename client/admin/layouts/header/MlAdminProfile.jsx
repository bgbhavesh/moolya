...
class MlAdminProfileApp extends Component {
  ...
  handleClick() {
    ...
    if (userDefaultObj.hierarchyCode == 'PLATFORM') { FlowRouter.go('/admin/dashboard/clusters'); } else { FlowRouter.go('/admin/dashboard/chapters'); }
  }
  ...
  async getValue() {
    ...
    if (user) {
      const genderImage = user && user.profile && user.profile.genderType === 'female' ? '/images/female.jpg' : '/images/ideator_01.png';
      const fullName = user && (`${user.profile.firstName}${user.profile.lastName}`);
      const profileImage = user && (!user.profile.profileImage || user.profile.profileImage === ' ') ? genderImage : user.profile.profileImage;
      this.setState({
        firstName: fullName,
  ...
  componentWillUpdate() {
    // console.log(this.props.user);
    const temp = this.state.profilePic;
    console.log(temp);
  }
    ...
    return (
      <div className="header_top">
        ...
        <div className="ml_profile" role="navigation">
          <h1
            id="NavLbl" className="" style={{ background: `url(${generateAbsolutePath(this.state.profilePic)}) center center` }}
          ></h1>
          <ol>
