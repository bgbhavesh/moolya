...
export default class MlInstitutionEditAwards extends React.Component {
  ...
  async fetchPortfolioDetails() {
    ...
    const empty = _.isEmpty(that.context.institutionPortfolio && that.context.institutionPortfolio.awardsRecognition)
  ...
  onSaveAction(e) {
    ...
    let setObject = this.state.institutionAwards
  ...
  onTileClick(index, uiIndex, e) {
    const cloneArray = _.cloneDeep(this.state.institutionAwards);
  ...
  lockPrivateKeys(selIndex) {
    const privateValues = this.institutionAwardServer && this.institutionAwardServer[selIndex] ? this.institutionAwardServer[selIndex].privateFields : []
  ...
  onOptionSelected(selectedAward, callback, selObject) {
    ...
    if (selectedAward) {
      ...
      this.setState({ data: details, selectedVal: selectedAward, awardName: selObject.label }, () => {
        // this.sendDataToParent()
      })
    } else {
      details = _.extend(details, { awardId: '', awardName: '' });
      this.setState({ data: details, selectedVal: '', awardName: '' }, () => {
        // this.sendDataToParent()
      })
    }
  }
  ...
  sendDataToParent(isSaveClicked) {
    ...
    const awards = this.state.institutionAwards;
    ...
    this.setState({ institutionAwards })
  ...
  onLogoFileUpload(e) {
    ...
    const data = {
      moduleName: 'PORTFOLIO', actionName: 'UPLOAD', portfolioDetailsId: this.props.portfolioDetailsId, portfolio: { awardsRecognition: [{ logo: { fileUrl: '', fileName }, index: this.state.selectedIndex }] }
    };
  ...
  async fetchOnlyImages() {
    ...
    if (response && response.awardsRecognition) {
      const dataDetails = this.state.institutionAwards
      const cloneBackUp = _.cloneDeep(dataDetails);
      const specificData = cloneBackUp[this.state.selectedIndex];
      if (specificData) {
        const curUpload = response.awardsRecognition[this.state.selectedIndex]
  ...
  render() {
    ...
    const institutionAwardsList = that.state.institutionAwardsList || [];
              ...
              <ScrollArea
                ...
                <div className="col-lg-12">
                  <div className="row">
                    ...
                    {institutionAwardsList.map((details, idx) => (<div className="col-lg-2 col-md-3 col-sm-3" key={idx}>
