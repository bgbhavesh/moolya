...
export default class MlFunderInvestment extends Component {
  ...
  async fetchPortfolioDetails() {
    ...
    const empty = _.isEmpty(that.context.funderPortfolio && that.context.funderPortfolio.investments)
    const response = await fetchfunderPortfolioInvestor(portfolioDetailsId);
    if (empty) {
      if (response && response.length) { this.setState({ loading: false, funderInvestment: response, funderInvestmentList: response }); } else { this.setState({ loading: false }) }
    } else {
      this.setState({
        loading: false,
  ...
  dateChange(event) {
    const value = event && event._d ? moment(event._d).format('DD-MM-YYYY') : ''
    let details = this.state.data;
    const name = 'dateOfInvestment';
    details = _.omit(details, [name]);
  ...
  addInvestment() {
    this.setState({
      selectedObject: 'default', popoverOpen: !(this.state.popoverOpen), data: {}, selectedVal: ''
    })
  ...
  onOptionSelected(selectedFunding) {
    ...
    this.setState({ data: details, selectedVal: selectedFunding }, () => {
      // this.sendDataToParent()
    })
  }
  ...
  onTileClick(index, e) {
    const cloneArray = _.cloneDeep(this.state.funderInvestment);
  ...
  lockPrivateKeys(selIndex) {
    const privateValues = this.funderInvestmentServer && this.funderInvestmentServer[selIndex] ? this.funderInvestmentServer[selIndex].privateFields : []
  ...
  onSaveAction(e) {
    ...
    let setObject = this.state.funderInvestment
  ...
  sendDataToParent(isSaveClicked) {
    ...
    const investment = this.state.funderInvestment;
    ...
    this.setState({ funderInvestment })
  ...
  render() {
    ...
    const funderInvestmentList = that.state.funderInvestmentList || [];
    return (
      <div>
              ...
              >
                <div className="col-lg-12">
                  <div className="row">
                    ...
                    {funderInvestmentList.map((details, idx) => (
                      ...
                      <div className="row">
                        <div className="col-md-12">
                          <div className="form-group mandatory">
                            ...
                            <FontAwesome
                              name='unlock' className="input_icon un_lock" id="isDateOfInvestmentPrivate"
                              onClick={this.onLockChange.bind(this, 'dateOfInvestment', 'isDateOfInvestmentPrivate')}/>
                          </div>
                          <div className="form-group mandatory">
                            <input
                              type="text" placeholder="Company Name" name="investmentcompanyName" ref="investmentcompanyName"
                          ...
                          <div className="form-group mandatory">
                            <input
                              type="number" placeholder={`Investment Amount in ${this.state.currencyName} (${this.state.currencySymbol})`} name="investmentAmount" ref="investmentAmount" min={0}
                              defaultValue={this.state.data.investmentAmount}
                              className="form-control float-label" onBlur={this.handleBlur} data-required={true}
                              data-errMsg="Investment Amount is required"/>
                            <FontAwesome
                              name='unlock' className="input_icon un_lock" id="isInvestmentAmountPrivate"
                              onClick={this.onLockChange.bind(this, 'investmentAmount', 'isInvestmentAmountPrivate')}/>
                          </div>
                          <div className="form-group">
                            <Moolyaselect
                              multiSelect={false} className="form-field-name" valueKey={'value'} ref={'typeOfFundingId'}
                          ...
                          <div className="form-group">
                            <input
                              type="text" placeholder="About" name="aboutInvestment"
                              defaultValue={this.state.data.aboutInvestment}
                              className="form-control float-label" onBlur={this.handleBlur.bind(this)}/>
                            <FontAwesome
                              name='unlock' className="input_icon un_lock" id="isAboutInvestmentPrivate"
