...
export default class AppEmailDetails extends React.Component {
  ...
  async onUpdatingEmailDetails(index, value) {
    ...
    if (index !== -1) {
      // do your stuff here
      const registrationDetails = this.props.emailInfoDetails
      ...
      if (contactExist) {
        ...
        const emailId = this.refs[`emailId${index}`].value
        const isValidEmail = validatedEmailId(emailId);
        if (ret) {
          toastr.error(ret);
        } else if (emailId && !isValidEmail) {
          ...
          const labelValue = this.state.selectedEmailTypeLabel ? this.state.selectedEmailTypeLabel : this.state.details[index].emailIdTypeName;
          const valueSelected = this.state.selectedEmailTypeValue ? this.state.selectedEmailTypeValue : this.state.details[index].emailIdType;
  ...
  async onClear(index, selectedTabValue, value) {
    if (index == null) { this.refs.emailId.value = ''; } else { this.refs[`emailId${index}`].value = ''; }
  }
