...
export default class MlAppServiceProviderListView extends Component {
  async viewDetails(portfolioId, e) {
    ...
    if (this.props.config.isExplore && response && response.canAccess) { FlowRouter.go(`/app/explore/serviceProvider/${portfolioId}`) } else if (response && response.canAccess) { FlowRouter.go(`/app/serviceProvider/${portfolioId}`) } else if (response && !response.canAccess) { toastr.error('Portfolio not available for view') }
  }
