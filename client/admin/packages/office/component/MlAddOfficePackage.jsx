...
class MlAddOfficePackage extends Component {
  ...
  createOfficePackage() {
    ...
    let communities = _.uniqBy(this.state.communities, 'communityId');
    communities = communities.filter(data => typeof data.userCount !== undefined && data.userCount !== 0);
    ...
    const isValid = this.validateUserData(officePackage)
    if (isValid && isValid.success) { this.createNewPackage(officePackage) } else if (isValid && !isValid.success) { toastr.error(isValid.result); }
  ...
  async createNewPackage(officePackage) {
    const response = await createOfficePackageHandler(officePackage);
    if (response && response.success) {
  ...
  validateUserData(usersData) {
    if (usersData && usersData.principalUserCount && usersData.teamUserCount && usersData.totalCount) {
      ...
      if ((PUC + TUC) != TC) { return { success: false, result: 'Total user count should be equal to principal and team' } } else if (!_.isEmpty(usersData.availableCommunities)) {
        ...
        if (Number(addArray) != TUC) { return { success: false, result: 'Communities Users count should be equal to Team user count' } }
        return { success: true }
  ...
  optionsBySelectCommunity(Idx, calback, selObject) {
    const communities = [];
    _.each(selObject, (community) => {
      const count = community.userCount ? community.userCount : 0
  ...
  optionsBySelectFrequencyTypes(Idx, calback, selObject) {
    const frequencyType = selObject
    // this.setState({frequency:Idx});
    this.setState({ frequency: Idx, frequencyType });
  }

  onStatusChange(e) {
    const status = this.refs.isActive.checked
  ...
  render() {
    const that = this;
    const serviceTypeQuery = gql`query{data:fetchServicecardTypes {label:displayName, value: _id}}`;
    const frequencyTypeQuery = gql`query{data:fetchFrequencyTypes {label:displayName, value: _id}}`;
    const accountsquery = gql`query{data: FetchAccount {label:accountName,value: _id}}`;
    const communityQuery = gql`query{data:fetchCommunitiesDef {communityId:_id, communityName:name}}`;
    const officeCommunityQuery = gql`query {data: getOfficeUserTypes {communityId:code,communityName:displayName}}`;
    const clusterquery = gql` query{data:fetchClustersForMap{clusterName:displayName,clusterId:_id}}`;
    const chapterOption = this.state.clusters.length > 0 ? { options: { variables: { clusters: this.state.clusterIds } } } : { options: { variables: { clusters: [] } } };
    ...
    const subChapterOption = this.state.chapterIds.length > 0 && this.state.clusterIds.length > 0 ? { options: { variables: { chapters: this.state.chapterIds, clusters: this.state.clusterIds } } } : { options: { variables: { chapters: [], clusters: [] } } };
          ...
          <div className="col-lx-6 col-sm-6 col-md-6 nopadding-left">
            <div className="left_wrap left_user_blocks">
              ...
              >
                ...
                <div className="form-group">
                  <Moolyaselect
                    multiSelect={false} placeholder={'Service Card Type'} className="form-control float-label"
                ...
                <div className="form-group">
                  <Moolyaselect
                    multiSelect={false} placeholder={'Account Type'} className="form-control float-label"
            ...
            <div className="left_wrap">
              ...
              >
                <form>
                  <div className="form-group">
                    <Moolyaselect
                      multiSelect={true} className="form-control float-label" valueKey={'communityId'} labelKey={'communityName'} queryType={'graphql'} query={communityQuery} isDynamic={true}
                  ...
                  <Moolyaselect
                    multiSelect={true} mandatory={true} className="form-control float-label" queryType={'graphql'}
                  ...
                  <div className="swiper-container blocks_in_form">
                    <div className="swiper-wrapper">
                      {this.state.communities.map((options, id) => (
                        <div className="swiper-slide" key={id}>
                          ...
                          <div className="form-group mart20">
                            <input
                              type="Number" defaultValue={options.userCount} placeholder="No of Users"
                  ...
                  <Moolyaselect
                    multiSelect={true} placeholder={'Chapter'} className="form-control float-label" valueKey={'chapterId'} labelKey={'chapterName'}
