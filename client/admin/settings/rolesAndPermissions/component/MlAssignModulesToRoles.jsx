...
export default class MlAssignModulesToRoles extends React.Component {
  ...
  RemoveModuleToRoles(id, event) {
    ...
    assignModulesToRoles = this.state.assignModulesToRoles.filter((object, index) => id !== index);
    this.setState({
      assignModulesToRoles
    }, () => {
  ...
  componentWillMount() {
    const assignModulesToRolesDetails = this.props.assignModulesToRolesData
    if (assignModulesToRolesDetails) {
      const assignModulesToRolesForm = []
      for (let i = 0; i < assignModulesToRolesDetails.length; i++) {
        const actions = assignModulesToRolesDetails[i].actions
        const actionVal = []
        if (actions) {
  ...
  optionsBySelectModule(index, selectedIndex) {
    const assignModulesToRoles = this.state.assignModulesToRoles
    assignModulesToRoles[index].moduleId = selectedIndex
    this.setState({ assignModulesToRoles })
    this.props.getassignModulesToRoles(this.state.assignModulesToRoles)
  }
  ...
  onStatusChange(id, event) {
    if (event.target.checked) {
      const departmentDetails = this.state.assignModulesToRoles
  ...
  render() {
    const that = this;
    const getModulesquery = gql` query{data:fetchModules{label:name,value:_id}}`;

    return (
      <div>

        {that.state.assignModulesToRoles.map((options, id) => {
                ...
                <div className="row">
                  <div className="col-md-6">
                    <div className="form-group">
                      <Moolyaselect
                        multiSelect={false} placeholder="Module Name" className="form-control float-label" valueKey={'value'}
                  ...
                  <div className="col-md-6">
                    <div className="form-group switch_wrap inline_switch" style={{ marginTop: '7px' }}>
                      ...
                      <label className="switch">
                        <input
                          type="checkbox" checked={options.isActive}
                  ...
                  <div className="col-md-12 ">
                    <div className="form-group">
                      <div className="input_types"><input
                        id={`chapter_admin_create${id}`} type="checkbox" name="CREATE"
                      ...
                      <div className="input_types"><input
                        id={`chapter_admin_read${id}`} type="checkbox" name="READ"
                        checked={statusRead}
                        onChange={that.optionsBySelectAction.bind(that, id)}/><label
                        htmlFor={`chapter_admin_read${id}`}><span></span>Read</label></div>
                      <div className="input_types"><input
                        id={`chapter_admin_update${id}`} type="checkbox" name="UPDATE"
                        checked={statusUpdate}
                        onChange={that.optionsBySelectAction.bind(that, id)}/><label
                        htmlFor={`chapter_admin_update${id}`}><span></span>Update</label></div>
                      <div className="input_types"><input
                        id={`chapter_admin_delete${id}`} type="checkbox" name="DELETE"
