...
class MlEditOfficePackage extends Component {
  ...
  async fetchOfficePackageDetails() {
    const officePackageId = this.props.config.officeId;
    const response = await fetchOfficePackageHandler(officePackageId);
    if (response) {
      ...
      // var communities = []
      _.each(response.clusters, (cluster) => {
      ...
      _.each(response.chapters, (chapter) => {
  ...
  handleUpdateOfficePackage() {
    ...
    if (isValid && isValid.success) { this.updateOfficePackage(officePackage) } else if (isValid && !isValid.success) { toastr.error(isValid.result); }
  }

  async updateOfficePackage(officePackage) {
    const response = await updateOfficePackageHandler(this.props.config.officeId, officePackage);
  ...
  validateUserData(usersData) {
    if (usersData && usersData.principalUserCount && usersData.teamUserCount && usersData.totalCount) {
      ...
      if ((PUC + TUC) != TC) { return { success: false, result: "'Total user count' should be equal to 'Principal count and Team count'" } } else if (!_.isEmpty(usersData.availableCommunities)) {
            ...
            <div className="left_wrap">
              ...
              >
                <form>
                  ...
                  <MoolyaSelect
                    multiSelect={true} className="form-control float-label" valueKey={'communityId'}
