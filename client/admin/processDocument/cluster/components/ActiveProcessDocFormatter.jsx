...
class ActiveProcessFormatter extends React.Component {
  ...
  async onChange(data) {
    ...
    const id = data.id
    const response = await updateProcessActionHandler(id, ProcessDetails);
    if (response) {
      if (response.success) { FlowRouter.go('/admin/documents/clusterList'); } else { toastr.error(response.result); }
    }
  }
