...
MlResolver.MlQueryResolver['fetchUsersForDashboard'] = (obj, args, context, info) => {
  ...
  if(clusterId != "" && chapterId != "" && subChapterId != "" && communityCode != ""){
    ...
    if(cluster.isActive && chapter.isActive && subChapter.isActive){
      ...
      //   // FOR Internal Users
        let internalUsers = mlDBController.find('users', {"$and":[{"profile.isSystemDefined":{$exists:false}},{"profile.isInternaluser":true}]}, context, findOptions).fetch();
      ...
      if(cluster.isActive && chapter.isActive && subChapter.isActive){
          if(userType == "All"){
              ...
              let allUsers = mlDBController.find('users', {"$and":[{"profile.isSystemDefined":{$exists:false}},{"profile.isInternaluser":true}]}, context).fetch();
    ...
    if(cluster.isActive && chapter.isActive){
      if(userType == "All"){
        ...
        //   // UserType needed to be introduced
          let allUsers = mlDBController.find('users', {"$and":[{"profile.isSystemDefined":{$exists:false}},{"profile.isInternaluser":true}]}, context, findOptions).fetch();
          _.each(allUsers, function (user){
              let userProfiles = user.profile.InternalUprofile.moolyaProfile.userProfiles;
              let profile = _.find(userProfiles, {clusterId:clusterId});
              if(profile){
    ...
    if(cluster.isActive ){
      ...
      if(userType == "All"){
        ...
        _.each(external, function (user){
          ...
          var defaultExternalProfile = _.find(user.profile.externalUserProfiles, {isDefault:true});
          if(!defaultExternalProfile){
            defaultExternalProfile = user.profile.externalUserProfiles[0]
          }
          if(defaultExternalProfile.communityDefCode != null){
            user.communityCode = defaultExternalProfile.communityDefCode
            // user.clusterName = "";
            user.name = (user.profile.firstName?user.profile.firstName:"")+" "+(user.profile.lastName?user.profile.lastName:"");
            users.push(user);
          }
  ...
  if(user&&args&&args.clusterId){
    ...
    /*switch profile - if user has switched profile,check for profile switch flag and set switchedProfileDefaultId to selected id */
     result= mlDBController.update('users',{'_id':userId,"profile.InternalUprofile.moolyaProfile.hasSwitchedProfile": true,'profile.InternalUprofile.moolyaProfile.userProfiles':{$elemMatch: {'clusterId': args.clusterId}}},
      {"profile.InternalUprofile.moolyaProfile.userProfiles.$.isDefault": true,"profile.InternalUprofile.moolyaProfile.switchedProfileDefaultId": args.clusterId}, {$set: true}, context);
...
MlResolver.MlMutationResolver['switchProfile'] = (obj, args, context, info) => {
  ...
  if(user&&args&&args.clusterId){
    ...
    /*switch profile - if user has switched profile,set switchedProfileDefaultId to selected id */
      result= mlDBController.update('users',{'_id':userId,'profile.InternalUprofile.moolyaProfile.userProfiles':{$elemMatch: {'clusterId': args.clusterId}}},
        {"profile.InternalUprofile.moolyaProfile.hasSwitchedProfile": true,
         "profile.InternalUprofile.moolyaProfile.switchedProfileDefaultId": args.clusterId,
