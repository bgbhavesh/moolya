...
MlResolver.MlMutationResolver.createTransaction = (obj, args, context, info) => {
  ...
  const requestDetails = MlRequestType.findOne({ _id: args.transaction.requestTypeId }) || {};
  ...
  const transactionDetails = args.transaction
  const id = mlDBController.insert('MlTransactions', args.transaction, context)
  if (id) {
    const code = 200;
...
MlResolver.MlMutationResolver.updateTransactionStatus = (obj, args, context, info) => {
  ...
  const id = mlDBController.update(collection, { _id: args.transactionId }, { status: args.status }, { $set: true }, context)
  if (id) {
    const code = 200;
    const result = { transactionId: id }
    const response = new MlRespPayload().successPayload(result, code);
...
MlResolver.MlMutationResolver.assignTransaction = (obj, args, context, info) => {
  ...
  let canUpdateAllTransactions = true
  let hierarchy = null
  let successCount = 0
  const user = mlDBController.findOne('users', { _id: params.user }, context)
  const transactions = args.transactionId

  transactions.map((transaction) => {
    if (canUpdateAllTransactions) {
      const trans = mlDBController.findOne(collection, { transactionId: transaction }, context)

      if (user && params.cluster == null && params.department == null && params.role == null) {
        let defaultRole = {};
        const userProfile = _.find(user.profile.InternalUprofile.moolyaProfile.userProfiles, { clusterId: trans.registrationInfo.clusterId });
        if (userProfile) {
          const userRolesData = userProfile && userProfile.userRoles ? userProfile.userRoles : [];
          ...
          if (defaultRole) {
            ...
            if (canUpdateAllTransactions) {
              try {
                ...
                if (!hierarchy) { canUpdateAllTransactions = false; }
              } catch (e) {
                canUpdateAllTransactions = false;
  ...
  if (hierarchyDesicion === true && successCount == transactions.length && canUpdateAllTransactions) {
    const date = new Date();
    const allocationObj = MlStatusRepo.getStatusDefinition('ADM_ASSIGN_COMP', 'allocation');
    const allocation = {
    ...
    if (canUpdateAllTransactions) {
      transactions.map((trans) => {
        if (hierarchy) {
          const id = mlDBController.update(
            collection, { transactionId: trans },
            {
              allocation,
              // status:"WIP",
              // userId:params.user,
...
MlResolver.MlQueryResolver.fetchTransactions = (obj, args, context, info) => {
  if (args.transactionType) {
    ...
    const transactions = mlTransactionsListRepo.fetchTransactions(transactionType, userId, status);
    return transactions;
  }
  return null;
}
...
MlResolver.MlMutationResolver.createRegistrationTransaction = (obj, args, context, info) => {
  const transaction = {};
  const transact = MlTransactionTypes.findOne({ _id: args.transactionType }) || {};
  ...
  const id = mlDBController.insert('MlTransactions', transaction, context)
  if (id) {
    const code = 200;
    const result = transaction.requestId
    const response = new MlRespPayload().successPayload(result, code);
    return response
  }
}
...
MlResolver.MlMutationResolver.updateRegistrationTransaction = (obj, args, context, info) => {
  const transaction = MlTransactions.findOne({ requestId: args.transactionInfo.requestId }) || {};
  const date = new Date();
  ...
  const id = mlDBController.update('MlTransactions', { requestId: args.transactionInfo.requestId }, { cluster: args.transactionInfo.cluster, chapter: args.transactionInfo.chapter, transactionUpdatedDate: date.date }, { $set: true }, context)
  if (id) {
    const code = 200;
    const result = { transactionId: id }
    const response = new MlRespPayload().successPayload(result, code);
...
MlResolver.MlMutationResolver.selfAssignTransaction = (obj, args, context, info) => {
  ...
  if (hierarchyDesicion === true && successCount == transactions.length) {
    ...
    const userProfile = _.find(userprofiles, item => item.isDefault == true);
    ...
    let updateCount = 0
    transactions.map((trans) => {
      const hierarchy = mlHierarchyAssignment.findHierarchy(roleDetails.clusterId, roleDetails.departmentId, roleDetails.subDepartmentId, roleDetails.roleId, roleDetails.subChapterId)
      if (hierarchy) {
        const id = mlDBController.update(collection, { transactionId: trans }, {
          allocation,
          // status: "WIP",
          // userId: user._id,
          hierarchy: hierarchy._id,
...
MlResolver.MlMutationResolver.unAssignTransaction = (obj, args, context, info) => {
  ...
  if (hierarchyDesicion === true && successCount == transactions.length) {
    ...
    transactions.map((trans) => {
      const date = new Date();
      const allocationObj = MlStatusRepo.getStatusDefinition('ADM_ASSIGN_PEND', 'allocation');
      const id = mlDBController.update(collection, { transactionId: trans }, {
...
MlResolver.MlQueryResolver.fetchTransactionsLog = (obj, args, context, info) => {
  if (args.userId) {
    ...
    const transactions = mlDBController.find('MlTransactionsLog', { userId, transactionTypeName: args.transactionTypeName }).fetch();
    return transactions;
  }
  return null;
}
...
MlResolver.MlMutationResolver.createTransactionLog = (obj, args, context, info) => {
  ...
  const id = mlDBController.insert('MlTransactionsLog', args.transaction, context)
  if (id) {
    const code = 200;
    const result = { transactionId: id }
    const response = new MlRespPayload().successPayload(result, code);
...
MlResolver.MlQueryResolver.validateTransaction = (obj, args, context, info) => {
  const collection = args.collection
  const transaction = mlDBController.findOne(collection, { transactionId: args.transactionId }, context)
  let hierarchyDesicion = false;
  if (!args.assignedUserId) {
    const systemRoles = mlHierarchyAssignment.checkSystemSystemDefinedRole(mlHierarchyAssignment.getUserRoles(context.userId));
    if (systemRoles === true) {
      const code = 200;
      const result = { status: systemRoles }
      const response = new MlRespPayload().successPayload(result, code);
      return response
    }
    const code = 400;
  ...
  if (hierarchyDesicion === true) {
    const code = 200;
    const result = { status: hierarchyDesicion }
    const response = new MlRespPayload().successPayload(result, code);
    return response
  }
  const code = 400;
...
MlResolver.MlQueryResolver.validateAssignmentsDataContext = (obj, args, context, info) => {
  ...
  if (data && userId) {
    ...
    _.each(userProfiles, (profile) => {
      ...
      const roles = _lodash.filter(profile.userRoles, { isActive: true })
      const highestRole = _lodash.maxBy(roles, 'hierarchyLevel')
    ...
    for (let i = 0; i < data.length; i++) {
      try {
        if (data[i].clusterId && !data[i].chapterId && !data[i].subChapterId && !data[i].communityId) {
          var role = _lodash.find(userRoles, (obj) => {
