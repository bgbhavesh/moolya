...
export default class MlIdeatorLookingFor extends Component {
  ...
  async fetchPortfolioDetails() {
    ...
    const empty = _.isEmpty(that.context.ideatorPortfolio && that.context.ideatorPortfolio.lookingFor)
    ...
    this.IdeatorLookingForServer = response || []
  }
  ...
  onTileClick(index, e) {
    const cloneArray = _.cloneDeep(this.state.ideatorLookingFor);
  ...
  lockPrivateKeys(selIndex) {
    const privateValues = this.IdeatorLookingForServer && this.IdeatorLookingForServer[selIndex] ? this.IdeatorLookingForServer[selIndex].privateFields : []
  ...
  onSaveAction(e) {
    ...
    let setObject = this.state.ideatorLookingFor
    if (this.context && this.context.ideatorPortfolio && this.context.ideatorPortfolio.lookingFor) { setObject = this.context.ideatorPortfolio.lookingFor }
  ...
  sendDataToParent(isSaveClicked) {
    ...
    const ideatorLookingFor1 = this.state.ideatorLookingFor;
    let ideatorLookingFor = _.cloneDeep(ideatorLookingFor1);
    data.index = this.state.selectedIndex;
    if (isSaveClicked) { ideatorLookingFor[this.state.selectedIndex] = data; }
    const arr = [];
    _.each(ideatorLookingFor, (item) => {
    ...
    this.setState({ ideatorLookingFor })
  ...
  render() {
    ...
    const that = this;
    const ideatorLookingForList = that.state.ideatorLookingForList || [];
    return (
      <div>
              ...
              >
                <div className="col-lg-12">
                  <div className="row">
                    ...
                    {ideatorLookingForList.map((details, idx) => (<div className="col-lg-2 col-md-3 col-sm-3" key={idx}>
