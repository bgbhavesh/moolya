...
export default class Step4 extends React.Component {
  ...
  updateOptions(index, did, selectedValue, selObject, callback) {
    ...
    if (index !== -1) {
      // do your stuff here
      const updatedComment = update(this.state.socialLinkArray[index], { socialLinkTypeName: { $set: selObject.label }, socialLinkType: { $set: did }, socialLinkUrl: { $set: this.refs[`socialLinkTypeUrl${index}`].value } });
      console.log(updatedComment);
      const newData = update(this.state.socialLinkArray, {
  ...
  async onDeleteSocialLink(index, value) {
    const listArray = update(this.state.socialLinkArray, {
      $splice: [[index, 1]]
    });
  ...
  componentWillReceiveProps(nextProps) {
    const socialLinkProps = nextProps.registrationData.socialLinksInfo;
  ...
  isUpdated() {
    ...
    const newArray = this.state.socialLinkArray || []
  ...
  async onSavingSocialLink(index, value) {
    ...
    const socialLinkURL = this.refs.socialLinkTypeUrl.value
    const ret = mlFieldValidations(refs)
    const isValidURL = validatedURL(socialLinkURL);
    if (ret) {
      toastr.error(ret);
    } else if (socialLinkURL && !isValidURL) {
      toastr.error("Enter valid 'Social link'");
    } else {
      const socialLinkList = this.state.socialLinkObject;
  ...
  async onUpdatingSocialLinkDetails(index, value) {
    ...
    if (index !== -1) {
      ...
      const registrationDetails = this.props.registrationInfo.socialLinksInfo
      const dbData = _underscore.pluck(registrationDetails, 'socialLinkType') || [];
      ...
      if (contactExist) {
        ...
        const ret = mlFieldValidations(refs)
        const socialLinkURL = this.refs[`socialLinkTypeUrl${index}`] && this.refs[`socialLinkTypeUrl${index}`].value ? this.refs[`socialLinkTypeUrl${index}`].value : ''
        const isValidURL = validatedURL(socialLinkURL);
        if (ret) {
          toastr.error(ret);
        } else if (socialLinkURL && !isValidURL) {
          ...
          const labelValue = this.state.selectedSocialLinkLabel ? this.state.selectedSocialLinkLabel : this.state.socialLinkArray[index].socialLinkTypeName;
          const valueSelected = this.state.selectedValue ? this.state.selectedValue : this.state.socialLinkArray[index].socialLinkType;
          const updatedComment = update(this.state.socialLinkArray[index], {
  ...
  onFileUpload(file) {
    const data = {
      moduleName: 'REGISTRATION', actionName: 'UPLOAD', documentId: null, registrationId: this.props.registrationId
    };
  ...
  render() {
    ...
    const socialLinkTypeQuery = gql`query($type:String,$hierarchyRefId:String){
     ...
     `;
    const socialLinkTypeOption = { options: { variables: { type: 'SOCIALLINKS', hierarchyRefId: this.props.clusterId } } };
    return (
              ...
              <div className="panel-body">
                ...
                <div className="ml_tabs">
                  <ul className="nav nav-pills">
                    ...
                    {that.state.socialLinkArray && (that.state.socialLinkArray.map((options, key) => (
                  ...
                  <div className="tab-content clearfix">
                    <div className={`tab-pane${this.state.activeTab}`} id="1a">
                      <div className="form-group">
                        <Moolyaselect
                          multiSelect={false} ref={'socialLinkType'} mandatory={true}
                    ...
                    {that.state.socialLinkArray && (that.state.socialLinkArray.map((options, key) => (<div className="tab-pane" id={`socialLink${key}`} key={key} >
                      <div className="form-group">
                        <Moolyaselect
                          multiSelect={false} ref={`socialLinkType${key}`}
