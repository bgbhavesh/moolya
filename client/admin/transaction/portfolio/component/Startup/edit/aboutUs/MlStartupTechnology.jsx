...
class MlStartupTechnology extends Component {
  ...
  componentWillMount() {
    const empty = _.isEmpty(this.context.startupPortfolio && this.context.startupPortfolio.technologies)
    if (!empty) {
  ...
  onSaveAction(e) {
    ...
    let setObject = this.state.startupTechnologies
  ...
  onTileClick(index, uiIndex, e) {
    const cloneArray = _.cloneDeep(this.state.startupTechnologies);
    // let details = cloneArray[index]
    let details = _.find(cloneArray, { index });
    details = _.omit(details, '__typename');
    this.setState({
      selectedIndex: index, data: details, selectedObject: uiIndex, popoverOpen: !(this.state.popoverOpen), selectedVal: details.technologyId
    });
    setTimeout(() => {
  ...
  onOptionSelected(selectedId, callback, selObject) {
    if (selectedId) {
      ...
      this.setState({
        data: details, selectedVal: selectedId, technologyName: selObject.label, technologyDescription: selObject.about
      }, () => {
        // this.sendDataToParent()
      })
    } else {
      ...
      this.setState({
        data: details, selectedVal: '', technologyName: '', technologyDescription: ''
      }, () => {
        // this.sendDataToParent()
      })
    }
  ...
  sendDataToParent(isSaveClicked) {
    ...
    const technologies = this.state.startupTechnologies;
    ...
    const arr = [];
    _.each(startupTechnologies, (item) => {
    ...
    this.setState({ startupTechnologies })
  ...
  onLogoFileUpload(e) {
    ...
    const data = {
      moduleName: 'PORTFOLIO', actionName: 'UPLOAD', portfolioDetailsId: this.props.portfolioDetailsId, portfolio: { technologies: [{ logo: { fileUrl: '', fileName }, index: this.state.selectedIndex }] }
    };
  ...
  async fetchOnlyImages() {
    ...
    if (response && response.technologies) {
      ...
      const dataDetails = this.state.startupTechnologies
      const cloneBackUp = _.cloneDeep(dataDetails);
      const specificData = cloneBackUp[thisState];
      if (specificData) {
        const curUpload = response.technologies[thisState]
  ...
  render() {
    ...
    const technologiesArray = that.state.startupTechnologiesList || [];
            ...
            >
              <div className="col-lg-12">
                <div className="row">
                  ...
                  {technologiesArray.map((details, idx) => (<div className="col-lg-2 col-md-3 col-sm-3" key={idx}>
                  ...
                  <div className="medium-popover"><div className="row">
                    <div className="col-md-12">
                      <div className="form-group">
                        <Moolyaselect
                          multiSelect={false} className="form-control float-label" valueKey={'value'} ref={'technologyId'}
