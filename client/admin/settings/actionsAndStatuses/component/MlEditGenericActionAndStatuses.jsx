...
class MlSearchDepartmentContainer extends React.Component {
  ...
  async getRoles(dId, sDId) {
    const id = FlowRouter.getParam('id');
    const response = await findRolessActionHandler(dId, sDId);
    if (response) {
      this.setState({
        rolesList: response
      });
  ...
  async getSubDepartment() {
    const did = FlowRouter.getParam('Did');
    const that = this;
    const response = await findSubDepartmentActionHandler(did);
    if (response) {
      this.setState({
  ...
  async getDepartment(id) {
    const response = await findDepartmentActionHandler(id);
    if (response) {
      this.setState({
  ...
  updateOperation(evt, index, type) {
    const operatios = this.state.operation;
  ...
  render() {
    ...
    const options = that.state.rolesList.map(data => ({
      value: data.value,
      label: data.name
    }));
    return (
      <div className="admin_main_wrap">
        <div className="admin_padding_wrap">
          ...
          <div style={{ clear: 'both' }}>
            {that.state.operation.map((data, i) => (
