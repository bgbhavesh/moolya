...
describe('MlAuditLog Module', () => {
  beforeEach(() => {
    mlAL = new mlAuditLog();
    resetDatabase();
  });
  it('should insert audit log - integration tests', () => {
    ...
    userId = Accounts.createUser({
      email: userEmail, password: userPassword, profile: { name: userName }, username: userName
    });
    //
    // Prepare request
    //
    ...
    // Create stub and spy
    //
    const stubMlAudit = stubs.create('mlAuditInsert', MlAudit, 'insert');
    const spyAsync = sinon.spy(async, 'each');
    //
