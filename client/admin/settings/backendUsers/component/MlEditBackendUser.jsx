...
class MlEditBackendUser extends React.Component {
  ...
  onBirthDateSelection(event) {
    if (event._d) {
      const value = moment(event._d).format(Meteor.settings.public.dateFormat);
  ...
  async handleSuccess(response) {
    ...
    } else { toastr.error(response.result); }
  }
  ...
  async findBackendUser() {
    ...
    const userTypeId = this.props.config;
    ...
    if (response) {
      ...
      if (dataDetails.profile.InternalUprofile.moolyaProfile.userProfiles[0]) {
        const userProfiles = dataDetails.profile.InternalUprofile.moolyaProfile.userProfiles
        const userProfilesDetails = [];
        for (let i = 0; i < userProfiles.length; i++) {
          const userRolesDetails = []
          const userRole = userProfiles[i].userRoles
  ...
  onMakeDefultChange(id, event) {
    const that = this;
    const userProfilesDetails = this.state.userProfiles;
    const userRoles = userProfilesDetails[id].userRoles;
  ...
  async updateBackendUser() {
    ...
    if (dataDetails.profile.InternalUprofile.moolyaProfile.userProfiles[0]) {
      ...
      for (let i = 0; i < userProfilesDetails.length; i++) {
        const userRolesDetails = []
        const userRole = userProfilesDetails[i].userRoles
        for (let j = 0; j < userRole.length; j++) {
          const json = {
            roleId: userRole[j].roleId,
            clusterId: userRole[j].clusterId,
    ...
    const profile = {
      ...
      email: this.refs.email.value,
      isMoolya: !!(moolyaProfile.userType && moolyaProfile.userType == 'moolya'),
    ...
    const head = first(userprofiles) || {}
    const headRole = head.userRoles
    const headProfile = first(headRole)
    let updateDetails = pick(headProfile, ['clusterId', 'chapterId', 'subChapterId', 'communityId'])
  ...
  render() {
    ...
    const UserTypeOptions = (this.state.loginUserDetails && this.state.loginUserDetails.isMoolya) ? [
...
`;
    const rolequery = gql` query{
    data:fetchActiveRoles{label:roleName,value:_id}
    }
`;
    const subChapterQuery = gql` query{
                ...
                <div className="col-md-6 nopadding-left">
                  ...
                  <div className="form_bg">
                    <form>
                      ...
                      <div className="form-group">
                        <Select
                          name="form-field-name" placeholder="Backend User Type" className="float-label" options={UserTypeOptions} value={that.state.selectedBackendUserType} onChange={that.onBackendUserTypeSelect.bind(that)} disabled="disabled"
                        />
                      </div>
                      ...
                      <div className="form-group">
                        <Select
                          name="form-field-name" placeholder="Select Role" className="float-label"
                    ...
                    <form>
                      ...
                      <div className="form-group switch_wrap inline_switch">
                        ...
                        {(that.state.selectedBackendUserType == 'non-moolya') ? <label className="switch">
                          <input
                            type="checkbox" ref="globalAssignment" checked={that.state.globalStatus}
                            disabled="disabled"/>
                      ...
                      <br className="brclear"/>
                      {that.state.userProfiles.map((userProfiles, idx) => (
                        <div key={idx}>
                          {userProfiles.userRoles.map((userRoles, RId) => (
                            <div key={RId} className="panel panel-default">
                              ...
                              <div className="panel-body">
                                ...
                                <div className="form-group">
                                  <input
                                    type="text" ref="chapter" defaultValue={(userRoles.chapterId == 'all') ? userRoles.chapterId : userRoles.chapterName} placeholder="Chapter"
