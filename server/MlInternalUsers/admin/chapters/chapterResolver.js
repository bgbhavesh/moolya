...
MlResolver.MlMutationResolver.createChapter = (obj, args, context, info) => {
  ...
  const subChapterName = Meteor.settings.private.subChapterName;
  ...
  const geoCIty = `${chapter.cityName}, ${chapter.stateName}, ${chapter.clusterName}` ? `${chapter.cityName}, ${chapter.stateName}, ${chapter.clusterName}` : '';
  geocoder.geocode(geoCIty, Meteor.bindEnvironment((err, data) => {
    if (err) {
    ...
    const id = mlDBController.insert('MlChapters', chapter, context)
    if (id) {
      ...
      const subchapterid = createSubChapter(subChapter, context);
      if (subchapterid) {
        MlResolver.MlMutationResolver.createCommunityAccess(obj, {
...
MlResolver.MlMutationResolver.updateChapter = (obj, args, context, info) => {
  ...
  const chapter = mlDBController.findOne('MlChapters', { _id: args.chapterId }, context)
  if (chapter) {
    ...
    } else if (chapter.status && chapter.status.code != 110) {
      chapter.status = {
        code: 110,
    ...
    const resp = mlDBController.update('MlChapters', args.chapterId, chapter, { $set: true }, context)
    if (resp) {
      const code = 200;
      const result = { chapter: resp }
      const response = JSON.stringify(new MlRespPayload().successPayload(result, code));
      return response
    }
  }
...
MlResolver.MlQueryResolver.fetchChapter = (obj, args, context, info) => {
  const response = mlDBController.findOne('MlChapters', { _id: args.chapterId }, context) || []
  return response
}
...
MlResolver.MlQueryResolver.fetchChaptersForMap = (obj, args, context, info) => {
  const result = mlDBController.find('MlChapters', { isActive: true }, context).fetch() || [];
  return result;
}
...
MlResolver.MlQueryResolver.fetchSubChapter = (obj, args, context, info) => {
  ...
  if (args.subChapterId) {
    const id = args.subChapterId;
    response = mlDBController.findOne('MlSubChapters', { _id: id }, context) || {}
    const stateName = mlDBController.findOne('MlStates', { _id: response.stateId }, context).name;
    response.stateName = stateName;
    if (response && !response.isDefaultSubChapter) {
      const associatedObj = MlResolver.MlQueryResolver.fetchRelatedSubChapters(obj, { subChapterId: args.subChapterId }, context, info)
      if (associatedObj) { response.associatedObj = associatedObj }
    }
    return response;
  } return response
}
...
MlResolver.MlQueryResolver.fetchSubChapters = (obj, args, context, info) => {
  ...
  const result = mlDBController.find('MlSubChapters', { chapterId: args.chapterId }, context).fetch() || [];
  return { data: result };
}

MlResolver.MlQueryResolver.fetchSubChaptersSelect = (obj, args, context, info) => {
  const chapterId = args.id;
  if (args.id && args.id === 'all') {
  }
  const result = mlDBController.find('MlSubChapters', { chapterId }, context).fetch() || [];
...
MlResolver.MlQueryResolver.fetchActiveSubChapters = (obj, args, context, info) => {
  ...
  const curUserProfile = new MlAdminUserContext().userProfileDetails(context.userId);
  let queryChange;
  ...
  const result = mlDBController.find('MlSubChapters', queryChange, context).fetch() || [];
  return result
}
...
MlResolver.MlMutationResolver.createSubChapter = (obj, args, context, info) => {
  try {
    ...
    const externalUser = args.subChapter && args.subChapter.moolyaSubChapterAccess && args.subChapter.moolyaSubChapterAccess.externalUser ? args.subChapter.moolyaSubChapterAccess.externalUser : {};
    args.subChapter.moolyaSubChapterAccess = {
      externalUser: {
    ...
    const hasPerm = MlSubChapterPreConditions.hasEditPermSubChapterAccessControl(context);
    ...
    const subChapterId = createSubChapter(args.subChapter, context)

    if (subChapterId) {
      if (args.subChapter && args.subChapter.associatedObj && args.subChapter.associatedObj.length) {
        _.each(args.subChapter.associatedObj, (item, say) => {
          item.subChapters.push({ subChapterId, chapterId: args.chapterId })
        })
      ...
      const geoCIty = `${args.subChapter.chapterName}, ${args.subChapter.stateName}, ${args.subChapter.clusterName}` ? `${args.subChapter.chapterName}, ${args.subChapter.stateName}, ${args.subChapter.clusterName}` : '';
      ...
      geocoder.geocode(geoCIty, Meteor.bindEnvironment((err, data) => {
        ...
        const subChapter = MlSubChapters.findOne({ _id: subChapterId })
        if (subChapter) {
          const latitude = data.results[0].geometry.location.lat + addNoise();
          const longitude = data.results[0].geometry.location.lng + addNoise();
      ...
      const code = 200;
      const response = new MlRespPayload().successPayload(subChapterId, code);
      return response
    }

    const code = 400;
    const response = new MlRespPayload().errorPayload('Unable To Create Subchapter', code);
    return response
  } catch (e) {
    const code = 400;
...
MlResolver.MlMutationResolver.updateSubChapter = (obj, args, context, info) => {
  ...
  if (subChapter) {
    ...
    const resp = mlDBController.update('MlSubChapters', args.subChapterId, subChapter, { $set: true }, context)
    if (resp) {
    ...
    if (resp) {
      ...
      const response = new MlRespPayload().successPayload('Updated successfully', code);
      return response
    }
...
MlResolver.MlQueryResolver.fetchActiveStatesChapters = (obj, args, context, info) => {
  ...
  if (states && states.length > 0) {
    if (states.length == 1 && states[0] == 'all' && clusters[0] == 'all') {
      ...
      clusters.map((clusterId) => {
        activeChapters = MlChapters.find({ $and: [{ clusterId, isActive: true }] }).fetch();
        if (activeChapters && activeChapters.length > 0) {
          chapters = chapters.concat(activeChapters)
        }
      })
      ...
      states.map((stateId) => {
        activeChapters = MlChapters.find({ $and: [{ stateId, isActive: true }] }).fetch();
        if (activeChapters && activeChapters.length > 0) {
          chapters = chapters.concat(activeChapters)
        }
      })
...
createSubChapter = (subChapter, context) => {
  ...
  const id = mlDBController.insert('MlSubChapters', subChapter, context)
  return id
}
...
MlResolver.MlMutationResolver.createRelatedSubChapters = (obj, args, context, info) => {
  ...
  if (args.associatedObj) {
    _.each(args.associatedObj, (item, say) => {
      const generatedCode = GenerateUniqueCode(item.subChapters[0].subChapterId, item.subChapters[1].subChapterId)
  ...
  try {
    ...
    if (args.associatedObj) {
      _.each(args.associatedObj, (item, say) => {
        ...
        const isValue = mlDBController.findOne('MlRelatedSubChapters', { relatedCode: generatedCode }, context)
        if (isValue) {
...
MlResolver.MlQueryResolver.isSubChapterMoolyaNonMoolya = (obj, args, context, info) => {
  // let result =  MlSubChapters.find({chapterId: args.id}).fetch()||[];
  if (args.id) {
    const result = mlDBController.findOne('MlSubChapters', { _id: args.id }, context) || {};
    return result;
  }
}
