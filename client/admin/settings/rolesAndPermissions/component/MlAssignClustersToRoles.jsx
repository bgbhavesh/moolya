...
export default class MlAssignClustersToRoles extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      assignRoleToClusters: [{
        cluster: '', chapter: '', subChapter: '', community: '', department: '', subDepartment: '', isActive: false
      }]
    }
  ...
  AssignassignRoleToClusters(id) {
    this.setState({
      assignRoleToClusters: this.state.assignRoleToClusters.concat([{
        cluster: '', chapter: '', subChapter: '', community: '', department: '', subDepartment: '', isActive: false
      }])
    }, function () {
      setTimeout(() => {
  ...
  RemoveAssignassignRoleToClusters(id, event) {
    ...
    assignRoleToClusters = this.state.assignRoleToClusters.filter((object, index) => id !== index);
    this.setState({
      assignRoleToClusters
    })
  }
  ...
  componentWillMount() {
    const assignedClusterDetails = this.props.assignedClusterDetails
    if (assignedClusterDetails) {
      const assignedClusterDetailsForm = []
  ...
  optionsBySelectCluster(index, selectedIndex) {
    const availabilityDetails = this.state.assignRoleToClusters
    ...
    const canAdd = this.checkAvailabilityOfRole(availabilityDetails[index])
    if (canAdd) {
  ...
  checkAvailabilityOfRole(role) {
    ...
    const roleObj = _.filter(this.state.assignRoleToClusters, role);
  ...
  addDepartmentComponent(event) {
    ...
    this.setState({
      assignRoleToClusters: this.state.assignRoleToClusters.concat([{
        cluster: '', chapter: '', subChapter: '', community: '', email: '', isActive: false
      }])
    });
  }
  onStatusChange(id, event) {
    ...
    const departmentDetails = this.state.assignRoleToClusters
  ...
  render() {
    ...
    const selectedUserType = this.props.selectedBackendUserType
    ...
    return (
      ...
      <div>
        ...
        {that.state.assignRoleToClusters.map((assignCluster, id) => {
          if (selectedUserType == 'moolya') {
          ...
          const chapterOption = { options: { variables: { id: assignCluster.cluster } } };
          ...
          const subDeparatmentOption = { options: { variables: { id: assignCluster.department } } };
          const communityOption = { options: { variables: { clusterId: assignCluster.cluster, chapterId: assignCluster.chapter, subChapterId: assignCluster.subChapter } } };
          return (
