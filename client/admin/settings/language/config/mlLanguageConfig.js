...
const mlLanguageTableConfig = new MlViewer.View({
  ...
  columns: [
    ...
    {
      dataField: 'languageInfo.languageName', title: 'Name', dataSort: true, customComponent(data) { return <div>{data.data.languageInfo.languageName}</div> }
    },
    {
      dataField: 'languageInfo.languageDisplayName', title: 'Display Name', dataSort: true, customComponent(data) { return <div>{data.data.languageInfo.languageDisplayName}</div> }
    },
    {
      dataField: 'languageInfo.aboutLanguage', title: 'About', dataSort: true, dataSort: true, customComponent(data) { return <div>{data.data.languageInfo.aboutLanguage}</div> }
    },
    {
  ...
  sizePerPage: 5,
  queryOptions: true,
  buildQueryOptions: config => ({ context: { settingsType: 'LANGUAGE' } }),
  graphQlQuery: gql`
