...
export default class MlConnectionRequest extends Component {
  ...
  async fetchConnectionDetails() {
    ...
    if (transactionId && this.props.data.transactionType == 'connectionRequest') {
      const connection = await fetchConnectionRequestHandler(transactionId);
  ...
  async acceptConnectionHandler() {
    const response = await acceptConnectionActionHandler({ connectionId: this.state.connectionId });
    if (response) {
  ...
  async rejectConnectionHandler() {
    const response = await rejectConnectionActionHandler({ connectionId: this.state.connectionId });
    if (response) {
      toastr.success(response.result);
      await this.fetchConnectionDetails();
    } else {
