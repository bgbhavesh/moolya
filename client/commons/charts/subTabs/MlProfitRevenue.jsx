...
export default class MlProfitRevenue extends React.Component {
  ...
  sendDataToParent(index) {
    if (index == null) {
      ...
      const arr = [];
      _.each(startupCompanyRevenue, (item) => {
      ...
      this.setState({ startupCompanyRevenue })
  ...
  componentWillMount() {
    const empty = _.isEmpty(this.context.startupPortfolio && this.context.startupPortfolio.profitRevenueLiabilityChart)
    if (!empty) {
