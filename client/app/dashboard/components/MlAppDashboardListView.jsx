...
export default class MlCommunityList extends Component {
  ...
  onStatusChange(userType, e) {
    if (userType) {
      ...
      const hasQueryOptions = !!(this.props.config && this.props.config.queryOptions);
      if (hasQueryOptions) {
  ...
  render() {
    ...
    const clusterId = this.props.config.params && this.props.config.params.clusterId ? this.props.config.params.clusterId : '';
    const chapterId = this.props.config.params && this.props.config.params.chapterId ? this.props.config.params.chapterId : '';
    const subChapterId = this.props.config.params && this.props.config.params.subChapterId ? this.props.config.params.subChapterId : '';
    ...
    const list = data.map((prop, idx) => {
      if (prop.communityCode == 'IDE') { icon = 'ideator' } else if (prop.communityCode == 'FUN') { icon = 'funder' } else if (prop.communityCode == 'STU') { icon = 'startup' } else if (prop.communityCode == 'CMP') { icon = 'company' } else if (prop.communityCode == 'SPS') { icon = 'users' } else if (prop.communityCode == 'INS') { icon = 'institutions' } else if (prop.profile.isInternaluser) { icon = 'moolya-symbol' }
