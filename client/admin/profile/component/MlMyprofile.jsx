...
export default class MlMyProfile extends React.Component {
  ...
  async checkExistingPassword() {
    const pwd = this.refs.existingPassword.value;
    const digest = CryptoJS.SHA256(pwd).toString()
    console.log(digest);
  ...
  async getValue() {
    ...
    const response = await findBackendUserActionHandler(userType);
    let DOB = '';
  ...
  async resetPassword() {
    if (this.state.showPasswordFields) {
      ...
      if (this.state.passwordState === 'Passwords match!') {
        ...
        if (this.state.pwdErrorMsg) { toastr.error("'Confirm Password' does not match with 'New Password'"); } else if (this.state.newpwdErrorMsg) {
  ...
  async updateProfile() {
    const existingPwdField = this.refs.existingPassword.value;
  ...
  onCheckPassword() {
    const existingPassword = this.refs.existingPassword.value;
    const password = this.refs.password.value;
    const confirmPassword = this.refs.confirmPassword.value;
  ...
  render() {
    ...
    const userContext = getAdminUserContext();
    let route;
