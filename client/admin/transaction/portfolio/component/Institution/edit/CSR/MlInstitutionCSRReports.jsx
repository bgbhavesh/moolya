...
export default class MlInstitutionCSRReports extends React.Component {
  ...
  componentWillMount() {
    const portfolioId = FlowRouter.getRouteName();
    ...
    if (portfolioId === 'library') {
      this.setState({
        explore: false, isLibrary: true, hideLock: true, deleteOption: false
      })
    }
  ...
  documentUpload(type, e) {
    ...
    const response = multipartASyncFormHandler(data, file, 'registration', this.onFileUploadCallBack.bind(this, type, file));
    // }else{
