...
class MlSubChapterAccessControl {
  ...
  static resolveNonMoolyaExternalAccess(permission, context) {
    ...
    var relatedSubChapters = MlSubChapterAccessControl.getRelatedSubChapters(isInternalUser, contextSubChapterId, permission);
    allowedSubChapters = allowedSubChapters.concat(relatedSubChapters);
  ...
  static canMoolyaSubChapterAccess(permission,requestSubChapterId) {
    ...
    if(accessCount>0){hasAccess=true};
    return hasAccess;
  }
  /**
  ...
  static getRelatedSubChapters(isInternalUser, subChapterId, permission) {
    ...
    var pipeline = [{
      ...
      {
        $project: {_id: "$subChapters.subChapterId"}
      }
    ];
