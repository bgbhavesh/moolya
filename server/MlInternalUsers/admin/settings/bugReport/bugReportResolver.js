...
MlResolver.MlMutationResolver.createBugReport = (obj, args, context, info) => {
  ...
  let bugReportData = { details: args.details.details, reportedUrl: args.details.reportedUrl };
  // todo: make a generic contextData handler
  const contextData = new MlTransactionsHandler().contextData(context.userId);
  const userAgent = new MlTransactionsHandler().resolveUserAgent(context);
