...
export default class MlChapterList extends Component {
  render() {
    ...
    const list = data.map((prop, idx) => {
      const image = prop.chapterImage && prop.chapterImage.trim() !== '' ? <img src={Meteor.settings.public.countriesFlagBaseUrl + prop.chapterImage}/> : <span className="ml my-ml-chapter"></span>;
      let icon,
        status;
