...
export default class MapDetails extends Component {
  ...
  render() {
    ...
    const mapDataArr = _.cloneDeep(mapData);
    const totalUsers = _.find(mapDataArr, { key: 'totalUsers' });
    _.remove(mapDataArr, { key: 'totalUsers' });
    const subchapter = _.find(mapDataArr, { key: 'subChapter' });
    if (subchapter) {
      _.remove(mapDataArr, { key: 'subChapter' });
    }

    const mapDataList = mapDataArr.map((m) => {
      let title = ''
